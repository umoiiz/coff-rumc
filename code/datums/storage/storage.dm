...
/atom/movable/proc/toggle_gathering_mode()
	...
	if(our_storage.collection_mode)
		to_chat(usr, span_notice("\The [our_storage.parent.name] 现在会一次性拾取同一格内的所有物品."))
	else
		to_chat(usr, span_notice("\The [our_storage.parent.name] 现在会一次拾取一件物品."))
...
/atom/movable/proc/toggle_draw_mode()
	...
	if(our_storage.draw_mode)
		...
		to_chat(usr, span_notice("空手点击[our_storage.parent.name]现在会打开袋装储物菜单."))
...
/datum/storage/proc/can_be_inserted(obj/item/item_to_insert, mob/user, warning = TRUE)
	...
	if(isitem(parent))
		...
		if(item_to_insert.w_class >= parent_storage.w_class && istype(item_to_insert, /obj/item/storage) && !is_type_in_typecache(item_to_insert.type, typecacheof(storage_type_limits)))
			if(!istype(src, /obj/item/storage/backpack/holding))	//bohs should be able to hold backpacks again. The override for putting a boh in a boh is in backpack.dm.
				if(warning)
					to_chat(user, span_notice("\The [parent.name] 无法容纳 \the [item_to_insert], 因为它是相同大小的储物物品."))
	...
	for(var/limited_type in storage_type_limits_max)
		...
		if(storage_type_limits_max[limited_type] == 0)
			if(warning)
				to_chat(user, span_warning("\The [parent.name] 再也装不下更多了.") )
			return FALSE
...
/datum/storage/proc/handle_access_delay(obj/item/accessed, mob/user, taking_out = TRUE, alert_user = TRUE)
	...
	if(!do_after(user, access_delay, IGNORE_USER_LOC_CHANGE, delay_target))
		to_chat(user, span_warning("你失手弄掉了[accessed]!"))
		return FALSE
	return TRUE
