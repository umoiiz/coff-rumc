...
/datum/game_mode/infestation/announce_bioscans(show_locations = TRUE, delta = 2, ai_operator = FALSE, announce_humans = TRUE, announce_xenos = TRUE, send_fax = TRUE)
	...
	if(announce_xenos)
		for(var/i in GLOB.alive_xeno_list_hive[XENO_HIVE_NORMAL])
			...
			to_chat(M, assemble_alert(
				...
				message = "我的孩子们和他们的女王,我[hosts_shipside ? "":"не"]在你们的金属巢穴[BIOSCAN_LOCATION(show_locations, host_location_shipside)]中感应到[hosts_shipside ? "примерно [hosts_shipside]":""]\
				个潜在宿主,在它之外有他们的["всего [numHostsPlanet]" || "нет"]\
	...
	if(ai_operator)
		...
		switch(GLOB.current_orbit)
			if(1)
				to_chat(usr, span_warning("信号分析可以获取有关敌方移动和数量的详细信息."))
				return
			if(3)
				to_chat(usr, span_warning("由于飞船升空,我们的生物扫描仪器中检测到轻微错误,部分有关敌对活动的信息可能不准确."))
				return
			if(5)
				to_chat(usr, span_warning("由于飞船的轨道高度,我们的生物扫描读数中检测到严重错误,信息可能与真实情况大相径庭."))
		return
	...
	for(var/mob/M as anything in GLOB.observer_list)
		to_chat(M, assemble_alert(
			title = "生物扫描完成",
			message = {"地面上有[numXenosPlanet]个异形.
			[numXenosShip]个异形在飞船上.
			[numXenosTransit]个异形在穿梭机上.

			[numHostsPlanet]个人类在地面上.
...
/datum/game_mode/infestation/proc/map_announce()
	...
	priority_announce(
		title = "来自最高指挥部的讯息",
		subtitle = "早上好,同志们!",
		message = "低温休眠已被总参谋部禁用.<br><br>注意: [SSmapping.configs[SHIP_MAP].map_name].<br>[SSmapping.configs[GROUND_MAP].announce_text]",
		color_override = "red"
	)
...
/datum/game_mode/infestation/proc/play_cinematic(z_level)
	...
	priority_announce("警报. 警报. 行星核弹已激活. 警报. 警报. 自毁程序进行中. 警报. 警报.", "行星弹头已成功发射", type = ANNOUNCEMENT_PRIORITY)
