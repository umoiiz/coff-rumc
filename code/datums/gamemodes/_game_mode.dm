...
/datum/game_mode/proc/CanLateSpawn(mob/new_player/NP, datum/job/job)
	...
	if(!SSticker || SSticker.current_state != GAME_STATE_PLAYING)
		to_chat(usr, span_warning("回合要么尚未准备好,要么已经结束!"))
		return FALSE
	if(!GLOB.enter_allowed || (!GLOB.xeno_enter_allowed && istype(job, /datum/job/xenomorph)))
		to_chat(usr, span_warning("当前禁止生成,请观察."))
		return FALSE
	if(!NP.client.prefs.random_name)
		...
		if(CONFIG_GET(flag/prevent_dupe_names) && GLOB.real_names_joined.Find(name_to_check))
			to_chat(usr, span_warning("已经有人使用此角色名加入了回合.请另选一个."))
			return FALSE
	if(!SSjob.AssignRole(NP, job, TRUE))
		to_chat(usr, span_warning("分配所选角色失败."))
		return FALSE
	return TRUE
...
/datum/game_mode/proc/set_valid_job_types()
	...
	if(!length(SSjob.active_occupations))
		to_chat(world, span_boldnotice("错误,游戏模式只分配了无效职业."))
		return FALSE
		...
		if(M.stat != DEAD && !isbrain(M))
			if(ishuman(M))
				...
				if(!is_mainship_level(current_turf.z) && (round_finished == MODE_INFESTATION_X_MINOR))
					...
					parts += span_greentext("你成功以[M.real_name]的身份在[SSmapping.configs[GROUND_MAP].map_name]的事件中幸存了下来.")
			else
				parts += "<div class='panel greenborder'>"
				parts += span_greentext("你成功以[M.real_name]的身份在[SSmapping.configs[GROUND_MAP].map_name]的事件中幸存了下来.")
...
/datum/game_mode/proc/check_predator_late_join(mob/pred_candidate, show_warning = TRUE)
	...
	if(!(GLOB.roles_whitelist[pred_candidate.ckey] & WHITELIST_PREDATOR))
		if(show_warning)
			to_chat(pred_candidate, span_warning("你不在白名单中!你可以在论坛上申请将你列入掠食者白名单."))
		return
	...
	if(!(round_type_flags & MODE_PREDATOR))
		if(show_warning)
			to_chat(pred_candidate, span_warning("本回合没有狩猎!也许下一回合吧."))
		return

	if(pred_candidate.ckey in predators)
		if(show_warning)
			to_chat(pred_candidate, span_warning("你已经是亚乌贾了!给别人一个机会吧."))
		return
	...
	if(get_desired_status(pred_candidate.client.prefs.yautja_status, WHITELIST_COUNCIL) == WHITELIST_NORMAL)
		...
		if(pred_current_num >= pred_max)
			if(show_warning)
				to_chat(pred_candidate, span_warning("本回合只能生成[pred_max]名掠食者,但议员和古者不计入其中."))
			return
	#endif
	return TRUE
