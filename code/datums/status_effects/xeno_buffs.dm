...
/datum/status_effect/stacking/essence_link/on_creation(mob/living/new_owner, stacks_to_apply, mob/living/carbon/link_target, expected_lifesteal_percentage, expected_revenge_modifier)
	...
	to_chat(link_owner, span_xenonotice("我们已与[link_target]建立了精华链接. 保持在[DRONE_ESSENCE_LINK_RANGE]格范围内以维持链接."))
	to_chat(link_target, span_xenonotice("[link_owner]已与我们建立了精华链接. 保持在[DRONE_ESSENCE_LINK_RANGE]格范围内以维持链接."))
	return ..()
...
/datum/status_effect/salve_regen/on_remove()
	buff_owner.balloon_alert(buff_owner, "药膏再生已结束")
	return ..()
...
/datum/status_effect/xeno_rejuvenate/tick(delta_time)
	...
	if(owner_xeno.plasma_stored < GORGER_REJUVENATE_COST)
		to_chat(owner_xeno, span_notice("没有足够的物质来维持自身..."))
	...
	to_chat(owner_xeno, span_notice("我们感觉伤口正在愈合."))
...
/datum/status_effect/xeno_carnage/on_remove()
	...
	to_chat(owner, span_notice("我们的嗜血欲望消退了..."))
...
/datum/status_effect/xeno_feast/tick(delta_time)
	...
	if(xeno_owner.plasma_stored < plasma_drain)
		to_chat(xeno_owner, span_notice("我们的盛宴已经结束..."))
...
/datum/status_effect/drain_surge/on_apply()
	...
	X.visible_message(span_danger("[X]的甲壳闪烁着凶猛的绿光!"), \
	span_notice("你为你甲壳质的护甲注入了你受害者的毒素!"), null, 5)
	X.color = "#7FFF00"
...
/datum/status_effect/drain_surge/on_remove()
	...
	span_notice("你的甲壳质护甲失去了光芒."), null, 5)
	X.color = "#FFFFFF"
	QDEL_NULL(particle_holder)
	return ..()
