...
/datum/strippable_item/proc/try_equip(atom/source, obj/item/equipping, mob/user)
	...
	if(HAS_TRAIT(equipping, TRAIT_NODROP))
		to_chat(user, span_warning("你无法将[equipping]放到[source]上,它粘在你手上了!"))
		return FALSE
...
/datum/strippable_item/proc/start_equip(atom/source, obj/item/equipping, mob/user)
	if(isclothing(source))
		source.visible_message(
			span_notice("[user]试图将[equipping]放到[source]上."),
			span_notice("[user]试图将[equipping]放到你身上."),
			ignored_mob = user
		)

	to_chat(user, span_notice("你试图将[equipping]放到[source]上..."))
...
/datum/strippable_item/proc/start_unequip(atom/source, mob/user)
	...
	source.visible_message(
		span_warning("[user]试图移除[source]的[item.name]."),
		span_userdanger("[user]试图移除你的[item.name]."),
		ignored_mob = user,
	)

	to_chat(user, span_danger("你试图移除[source]的[item.name]..."))
...
/datum/strippable_item/mob_item_slot/try_equip(atom/source, obj/item/equipping, mob/user)
	...
	if(!equipping.mob_can_equip(source, item_slot, warning = TRUE, override_nodrop = FALSE, bitslot = TRUE))
		to_chat(user, span_warning("\The [equipping]不适合放在那个位置!"))
		return FALSE
