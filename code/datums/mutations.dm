...
/datum/mutation_menu/proc/purchase_mutation(mutation_name)
	...
	if(!xeno_owner)
		to_chat(usr, span_warning("异形引用无效!"))
		return
	...
	if(xeno_owner.fortify)
		to_chat(usr, span_warning("你在筑垒状态下无法购买突变!"))
		return

	if(xeno_owner.stat == DEAD)
		to_chat(usr, span_warning("你已经死了!"))
		return

	if(!mutation_datum)
		to_chat(usr, span_warning("突变名称无效!"))
		return

	if(!mutation_datum.is_available(xeno_owner))
		to_chat(usr, span_warning("该突变不适用于你的种姓!"))
		return

	if(!mutation_datum.is_unlocked(xeno_owner))
		to_chat(usr, span_warning("该突变尚未解锁!"))
		return
	...
	if(upgrade)
		to_chat(usr, span_xenonotice("已选择现有突变。未消耗生物质。"))
		return
	...
	to_chat(usr, span_xenonotice("获得[mutation_name]突变。"))
