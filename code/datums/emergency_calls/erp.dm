...
/datum/emergency_call/erp/print_backstory(mob/living/carbon/human/H)
	to_chat(H, "<B>你是一个恶作剧者,属于紧急响应恶作剧者,一个专门响应任何需要娱乐的情况的响应者</b>")
	to_chat(H, "<B>最近从[SSmapping.configs[SHIP_MAP].map_name]发出了一个信标,一个对恶作剧和娱乐的恳求!</b>")
	to_chat(H, "<B>在那里娱乐TGMC的好人们,并确保所有非TGMC的反娱乐来源被恶作剧并立即消灭!</b>")
