...
/datum/emergency_call/som_breachers/print_backstory(mob/living/carbon/human/H)
	...
	to_chat(H, "<B>由于你的[pick("marked distinction in combat", "ruthless record fighting against TGMC", "family's connections", "previous failures in combat")],你被分配到了SOM第5特别突击部队。</b>")
	to_chat(H, "<B>加入第5 SAF被视为极大的荣誉,通常会被派往一些能想象到的最危险行动中,尤其侧重于舰船战斗。伤亡率往往极高,不过在第5部队中取得成功往往能快速推进士兵的职业生涯。</b>")
	to_chat(H, "")
...
/datum/emergency_call/som_breachers/create_member(datum/mind/M)
	...
	H.apply_assigned_role_to_spawn(selected_job)
	to_chat(H, span_notice("你是火星之子的一员,被分配到这支精英破门小队,前去响应附近发出的TGMC求救信号。你的职责是消灭你发现的所有Terra Gov走狗。绝不留情!"))
