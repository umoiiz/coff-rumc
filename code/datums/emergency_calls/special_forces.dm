...
/datum/emergency_call/special_forces/print_backstory(mob/living/carbon/human/H)
	to_chat(H, "<B>你是当地特别响应部队的一员,驻扎在这个孤立星系内,接受过良好的训练并拥有良好的装备。</b>")
	to_chat(H, "<B>通常,你大部分时间都在为这里为数不多的大人物提供安保,其间在基地休息放松,偶尔处理人质事件。</b>")
	to_chat(H, "")
	to_chat(H, "<B>然而,最近当地一个定居点突然失联,并广播了求救信号,与此同时一艘TGMC舰船[SSmapping.configs[SHIP_MAP].map_name]出现在轨道上,这两者很可能有关联。</b>")
...
/datum/emergency_call/special_forces/create_member(datum/mind/M)
	...
	if(!leader)
		...
		to_chat(H, "<p style='font-size:1.5em'><span class='notice'>你是被指派带领这支队伍响应附近发出的TGMC求救信号的特种部队队长。让你的队伍保持完整并完成任务!</notice></p>")
		return
	...
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, "<p style='font-size:1.5em'><span class='notice'>你是这支特种部队中训练有素的成员,被派去调查附近发出的TGMC求救信号。不要让你和你的队伍放松警惕!</notice></p>")
