...
/datum/emergency_call/xenomorphs_zeta/print_backstory(mob/living/carbon/xenomorph/X)
	to_chat(X, "<B>我们是来自泽塔巢穴的异形.</b>")
	to_chat(X, "<B>我们在太空中巡航了多年,直到一位新的女王联系我们并接管了我们穿梭机的控制权.</b>") // Admin-only ERT, so queens only.
	to_chat(X, "<B>扩张我们的巢穴并摧毁任何胆敢对抗我们影响力的人,尤其是其他巢穴.为了泽塔巢穴!</b>")
