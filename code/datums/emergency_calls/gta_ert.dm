...
/datum/emergency_call/gta/ballas/print_backstory(mob/living/carbon/human/H)
	to_chat(H, "<B>你代表Ballas帮 -- 紫色深深流淌在你的血液和头巾中.</B>")
	to_chat(H, "<B>街头传言[SSmapping.configs[SHIP_MAP].map_name]附近出了事. 是时候现身,拿走属于你的东西,并提醒所有人谁掌管着这些街区.</B>")
	to_chat(H, "<B>跟紧你的帮派. 别让Grove或那些公司挡你的路.</B>")
...
/datum/emergency_call/gta/ballas/create_member(datum/mind/M)
	...
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, span_danger("你是Ballas帮成员,正赶往求救信号所在地."))
...
/datum/emergency_call/gta/grove/print_backstory(mob/living/carbon/human/H)
	to_chat(H, "<B>你是Grove Street的人 -- 绿色头巾,忠诚的兄弟,以及与Ballas帮的长期恩怨.</B>")
	to_chat(H, "<B>[SSmapping.configs[SHIP_MAP].map_name]附近的混乱意味着机会. 带上你的兄弟们冲过去,把事情摆平.</B>")
	to_chat(H, "<B>Grove Street,家. 至少在所有这一切之前是.</B>")
...
/datum/emergency_call/gta/grove/create_member(datum/mind/M)
	...
	H.apply_assigned_role_to_spawn(J)
	to_chat(H, span_danger("你是Grove Street帮成员,正赶往求救信号所在地."))
