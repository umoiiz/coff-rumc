...
/datum/job/survivor/after_spawn(mob/living/carbon/C, mob/M, latejoin = FALSE)
	...
	switch(SSmapping.configs[GROUND_MAP].map_name)
		if(MAP_PRISON_STATION)
			to_chat(M, span_notice("你是菲奥里纳轨道监狱袭击的幸存者.你在监狱站工作或生活,并设法避开了异形袭击...直到现在."))
		if(MAP_ICE_COLONY)
			to_chat(M, span_notice("你是冰栖地袭击的幸存者.你在殖民地工作或生活,并设法避开了异形袭击...直到现在."))
		if(MAP_BIG_RED)
			to_chat(M, span_notice("你是殖民地袭击的幸存者.你在考古殖民地工作或生活,并设法避开了异形袭击...直到现在."))
		if(MAP_LV_624)
			to_chat(M, span_notice("你是殖民地袭击的幸存者.你怀疑出了什么问题并试图警告他人,但为时已晚..."))
		if(MAP_ICY_CAVES)
			to_chat(M, span_notice("你是冰洞系统袭击的幸存者.你在该站点工作或生活,并设法避开了异形袭击...直到现在."))
		if(MAP_RESEARCH_OUTPOST)
			to_chat(M, span_notice("你是前哨站袭击的幸存者.但你问自己:你现在真的安全吗?"))
		if(MAP_MAGMOOR_DIGSITE)
			to_chat(M, span_notice("你是马格莫尔挖掘场IV袭击的幸存者.你在挖掘场工作或生活,并设法避开了异形袭击...直到现在."))
		else
			to_chat(M, span_notice("凭借奇迹,你设法在袭击中幸存下来.但你现在真的安全吗?"))

/datum/job/survivor/radio_help_message(mob/M)
	. = ..()
	to_chat(M, {"无论你经历了什么,你来这里是为了生存并让自己获救.
如果你获救,你会感激地球政府和纳米传讯的支持.
你对TGMC没有敌意,也不应反对或破坏他们的目标,除非管理员另有说明.
如果你在该区域发现任何其他幸存者,与他们合作以提高生存几率.
根据你所从事的工作,你可能拥有在需要时帮助他人的额外技能.
祝你好运,但别指望能活下来."})
