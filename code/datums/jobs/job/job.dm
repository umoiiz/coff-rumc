...
/datum/job/proc/radio_help_message(mob/new_player)
	...
	message += span_role_body("作为 <b>[title]</b>, 你听命于 [supervisors]. 特殊情况可能会改变这一点.")
	if(!(job_flags & JOB_FLAG_NOHEADSET))
		message += separator_hr("[span_role_body("<b>Radio</b>")]")
		message += span_role_body("在你的消息前加上 <b>;</b> 以在默认无线电频道上发言, 大多数情况下这是你的小队无线电. 如需更多前缀, 请检查你的耳机.")
	if(req_admin_notify)
		message += separator_hr("[span_role_header("This is an important job.")]")
		message += span_role_body("如果你必须断开连接, 请使用低温休眠舱. 如果你无法到达那里, <b><u>adminhelp</u></b> 使用 F1 或 Adminhelp 动词.")
