...
/datum/component/grillable/proc/FinishGrilling(atom/grill_source)
	...
	grill_source.visible_message("<span class='[positive_result ? "notice" : "warning"]'>[parent]变成了\a [grilled_result]!</span>")
...
/datum/component/grillable/proc/OnExamine(atom/A, mob/user, list/examine_list)
	...
	if(!current_cook_time) //Not grilled yet
		if(positive_result)
			examine_list += span_notice("[parent]可以被<b>烤制</b>成\a [initial(cook_result.name)].")
		return

	if(positive_result)
		if(current_cook_time <= required_cook_time * 0.75)
			examine_list += span_notice("[parent]可能需要再煮一会儿!")
		else if(current_cook_time <= required_cook_time)
			examine_list += span_notice("[parent]似乎快煮好了!")
	else
		examine_list += span_danger("[parent]可能不应该再煮太久了!")
