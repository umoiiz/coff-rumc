...
/datum/component/chem_booster/proc/examine(datum/source, mob/user, list/examine_text)
	SIGNAL_HANDLER
	examine_text += span_notice("化学系统当前储存有[resource_storage_current]u的绿色血液. 其增强等级设置为[boost_amount].")
...
/datum/component/chem_booster/process()
	...
	if(vali_necro_timer > 15 SECONDS)
		...
		to_chat(wearer, span_userdanger("坏死过程开始出现. 在太晚之前关掉它!"))
...
/datum/component/chem_booster/proc/on_off(datum/source)
	...
	if(!boost_on)
		if(!COOLDOWN_FINISHED(src, chemboost_activation_cooldown))
			...
			return
		if(resource_storage_current < resource_drain_amount)
			wearer.balloon_alert(wearer, "绿色血液不足,无法开始运作")
			return
		if(wearer.stat)
	...
	if(!boost_on)
		...
		wearer.balloon_alert(wearer, "停止绿色血液注射")
	...
	to_chat(wearer, span_notice("开始绿色血液注射.<b>[(automatic_meds_use && meds_beaker.reagents.total_volume) ? " Adding additional reagents." : ""]</b>"))
...
/datum/component/chem_booster/proc/update_boost(amount)
	boost_amount = amount
	wearer?.balloon_alert(wearer, "增强等级设置为[boost_amount]")
...
/datum/component/chem_booster/proc/connect_weapon()
	...
	if(wearer.do_actions)
		wearer.balloon_alert(wearer, "你已经在忙别的事了")
		return

	var/obj/item/held_item = wearer.get_held_item()
	if(!held_item)
		wearer.balloon_alert(wearer, "你需要手持采集器")
		return
...
/datum/component/chem_booster/proc/extract(volume)
	...
	if(resource_storage_current < volume)
		wearer.balloon_alert(wearer, "绿色血液不足,无法提取")
		return

	var/obj/item/held_item = wearer.get_held_item()
	if(!held_item)
		wearer.balloon_alert(wearer, "你必须手持玻璃试剂容器")
		return
	...
	if((held_item.reagents.maximum_volume-held_item.reagents.total_volume) < volume)
		wearer.balloon_alert(wearer, "手持容器没有提取所需的空间")
		return
...
/datum/component/chem_booster/proc/load_up()
	...
	if(!istype(held_item, /obj/item/reagent_containers) && meds_beaker.reagents.total_volume)
		...
		wearer.balloon_alert(wearer, "化学系统[automatic_meds_use ? "will" : "won't"]在激活时注射已装载的试剂")
		return
	...
	if(!held_beaker.reagents.total_volume && !meds_beaker.reagents.total_volume)
		wearer.balloon_alert(wearer, "手持的试剂容器和系统的试剂储存都是空的")
		return
...
/datum/component/chem_booster/proc/steal_greenblood(owner, drain_plasma_gain, drainer)
	...
	to_chat(wearer, span_danger("未注册用户正在访问绿色血液储备!"))
