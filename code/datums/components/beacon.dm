...
/datum/component/beacon/proc/activate(atom/movable/source, mob/user)
	...
	if(A && istype(A) && A.ceiling >= CEILING_DEEP_UNDERGROUND)
		to_chat(user, span_warning("如果你站在很深的地下,这不会起作用."))
		active = FALSE
		return FALSE

	if(istype(A, /area/shuttle/dropship))
		to_chat(user, span_warning("你必须在运输机外才能使用它,否则无法传输."))
		active = FALSE
		return FALSE
	...
	if(anchor && anchor_time)
		...
		user.visible_message(span_notice("[user]开始在地面上架设[source]."),
		span_notice("你开始在地面上架设[source]并输入它所需的所有数据."))
	...
	user.show_message(span_notice("[source]发出哔声并播报,\"你当前的坐标已被补给控制台登记. 经度 [location.x]. 纬度 [location.y]. 区域ID: [get_area(source)]\""), EMOTE_AUDIBLE, span_notice("[source]震动了,但你听不到它!"))
...
/datum/component/beacon/proc/deactivate(atom/movable/source, mob/user)
	...
	if(source.anchored)
		if(user)
			...
			user.visible_message(span_notice("[user]开始从地面上拆除[source]."),
			span_notice("你开始从地面上拆除[source],将其停用."))
	...
	user.show_message(span_warning("[source]发出哔声并播报,\"你的最后位置已无法被补给控制台访问."), EMOTE_AUDIBLE, span_notice("[source]震动了,但你听不到它!"))
