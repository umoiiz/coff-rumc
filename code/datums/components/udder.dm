...
/datum/component/udder/proc/on_examine(datum/source, mob/user, list/examine_list)
	...
	switch(udder_filled_percentage)
		...
		if(11 to 99)
			examine_list += span_notice("如果你有东西可以盛装,[parent]的[udder]可以被挤奶.")
		if(100)
			examine_list += span_notice("[parent]的[udder]又圆又满,如果你有东西可以盛装,就可以挤奶.")
...
/obj/item/udder/proc/milk(obj/item/reagent_containers/glass/milk_holder, mob/user)
	if(milk_holder.reagents.total_volume >= milk_holder.volume)
		milk_holder.balloon_alert(user, "[milk_holder]满了.")
		return
	var/transfered = reagents.trans_to(milk_holder, rand(5,10))
	if(transfered)
		user.visible_message(span_notice("[user]使用\the [milk_holder]给[src]挤奶."), span_notice("你使用\the [milk_holder]给[src]挤奶."))
	else
		milk_holder.balloon_alert(user, "乳房是干的. 再等一会...")
