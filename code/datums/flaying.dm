...
/datum/flaying_datum/proc/begin_flaying(mob/living/carbon/human/target, mob/living/carbon/human/user, obj/item/tool, ongoing_attempt)
	...
	if(current_flayer)
		if(current_flayer != user)
			to_chat(user, span_warning("你无法剥皮[target],[current_flayer]已经在进行了!"))
	else
		current_flayer = user
		if(!ongoing_attempt)
			playsound(user.loc, 'sound/weapons/pierce.ogg', 25)
			user.visible_message(span_danger("<B>[user]用\a [tool]继续剥取[victim]的皮...</B>"),
				span_danger("<B>你用你的[tool.name]继续剥取[victim]的皮...</B>"))
	...
	switch(flaying_stage)
		if(FLAY_STAGE_SCALP)
			...
			if(!v_head || (v_head.limb_status & LIMB_DESTROYED)) //they might be beheaded
				...
				user.visible_message(span_danger("<B>[user]用\the [tool]将[victim]头部残端周围的皮肤剥松。</B>"),
					span_danger("<B>[victim]缺少\his 的头部。这样的毛皮就是不一样...你用你的[tool.name]将残端周围的皮肤剥松。</B>"))
			else
				...
				if(victim.h_style == "Bald") //you can't scalp someone with no hair.
					...
					user.visible_message(span_danger("<B>[user]沿着[victim]的发际线划了一圈,然后将\his 的头皮从\his 的头上撕了下来!</B>"),
						span_danger("<B>你沿着[victim]的发际线划了一圈,然后将\his 的头皮从\his 的头上撕了下来。</B>"))
		...
		if(FLAY_STAGE_STRIP)
			user.visible_message(span_danger("<B>[user]将\his [tool.name]刺入[victim]的切口中,撬动、切割,然后撕下大面积的皮肤。剩余部分松松垮垮地垂着。</B>"),
				span_danger("<B>你将你的[tool.name]刺入[victim]的切口中,撬动、切割,然后撕下大面积的皮肤。剩余部分松松垮垮地垂着。</B>"))
		...
		if(FLAY_STAGE_SKIN)
			user.visible_message(span_danger("<B>[user]将[victim]完全剥皮,像脱手套一样把\his 身上剩余的皮肤扯了下来!</B>"),
				span_danger("<B>你将[victim]完全剥皮,像脱手套一样把\his 身上剩余的皮肤扯了下来。\nUse绳子将\him 从天花板上吊起来。</B>"))
