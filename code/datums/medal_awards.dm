...
/proc/give_medal_award(medal_location)
	...
	var/posthumous = 1
	var/medal_type = tgui_input_list(usr, "你想授予哪种类型的勋章?", "勋章类型", list("distinguished conduct medal", "bronze heart medal","medal of valor", "medal of exceptional heroism", "letter of commendation"))
	if(!medal_type) 
		return
