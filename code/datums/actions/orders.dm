...
/mob/living/carbon/human/verb/issue_order(command_aura as null|text)
	...
	if(skills.getRating(SKILL_LEADERSHIP) < SKILL_LEAD_TRAINED)
		to_chat(src, span_warning("你的领导能力不足以发布命令."))
		return
	...
	if(has_status_effect(STATUS_EFFECT_MUTED))
		to_chat(src, span_warning("被禁言时无法下达命令."))
		return

	if(TIMER_COOLDOWN_RUNNING(src, COOLDOWN_SKILL_ORDERS))
		to_chat(src, span_warning("你最近已经下达过命令了.冷静一下."))
		return

	if(!command_aura)
		command_aura = tgui_input_list(src, "选择一个命令", items = command_aura_allowed + "help")
		if(command_aura == "help")
			to_chat(src, span_notice("<br>命令会在短时间内为附近的陆战队员提供增益,随后进入冷却,具体如下:<br><B>移动</B> - 提高机动性并增加闪避投射物的几率.<br><B>坚守</B> - 提高对疼痛和战斗创伤的抗性.<br><B>专注</B> - 提高枪械精准度和有效射程.<br>"))
			return
		if(!command_aura)
			return
	...
	switch(command_aura)
		if("move")
			...
			message = pick("动起来,懒鬼们!", "快动起来!", "动起来,动起来!", "走,走,走!", "前进!快点!", "动,动,动!", "跑步,前进!", "步子迈大点!", "挪动你们的爪子!", "动动你们的小腿,女士们!")
			say(message)
			add_emote_overlay(move)
		if("hold")
			var/image/hold = image('icons/mob/talk.dmi', src, icon_state = "order_hold")
			message = pick("一步也不许后退!", "死守到底!", "保持队形!", "守住阵地!", "顶住打击!", "站起来战斗!", "控制这片区域!", "准备迎接冲击!", "坚持住!", ";保护好脑袋!")
			say(message)
			add_emote_overlay(hold)
		if("focus")
			var/image/focus = image('icons/mob/talk.dmi', src, icon_state = "order_focus")
			message = pick("别打到自己人!", "集中火力!", "射击杀敌!", "上刺刀!", "随时准备开火!", "武器准备就绪!", "瞄准!", "注意!", "开火!", "准备战斗!", "给他们喂子弹!", "消灭目标!")
			say(message)
