...
/obj/effect/acid_hole/proc/use_wall_hole(mob/user)
	...
	if(do_after(user, 15, IGNORE_HELD_ITEM, src, BUSY_ICON_HOSTILE) && !T.density && !user.lying_angle && !user.buckled)
		...
		if(user.pulling)
			user.stop_pulling()
			to_chat(user, span_warning("你松开了正在拖拽的东西以便挤进隧道!"))
		user.forceMove(T)
...
/obj/effect/acid_hole/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/explosive/grenade))
		...
		user.visible_message(span_warning("[user]将[G]投掷穿过了[src]!"), \
							span_warning("你将[G]投掷穿过了[src]"))
		user.drop_held_item()
		G.forceMove(T)
	...
	//Throwing Flares and flashlights
		...
		user.visible_message(span_warning("[user]将[F]投掷穿过了[src]!"), \
							span_warning("你将[F]投掷穿过了[src]"))
		user.drop_held_item()
		F.forceMove(T)
