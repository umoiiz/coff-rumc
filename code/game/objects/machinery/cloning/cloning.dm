...
/obj/machinery/cloning/relaymove(mob/user)
	...
	user.visible_message("你听到有东西撞在\the [src]的窗户上", "门纹丝不动!")
	return FALSE
...
/obj/machinery/cloning/vats/attack_ghost(mob/dead/observer/user)
	...
	if(!occupant)
		return FALSE
	if(tgui_alert(user, "你想成为克隆人吗?", "成为克隆人", list("Yes", "No")) != "Yes")
		return FALSE
...
/obj/machinery/cloning/vats/attack_hand(mob/living/user)
	...
	if(user.a_intent == INTENT_HARM)
		user.visible_message(span_notice("[user]猛敲玻璃."), span_notice("你猛敲玻璃."))
		return TRUE
	...
	if(timerid || occupant) // You need to stop the process or remove the human first.
		to_chat(user, span_notice("机器正在培育克隆体时你无法接触到烧杯."))
		return
...
/obj/machinery/cloning/vats/attackby(obj/item/hit_by, mob/living/user, params)
	...
	if(istype(hit_by, /obj/item/reagent_containers/glass/beaker))
		if(beaker)
			to_chat(user, span_warning("烧杯中已经装入了机器."))
			return
		...
		for(var/datum/reagent/instance AS in hit_by.reagents.reagent_list)
			if(!istype(instance, /datum/reagent/medicine/biomass) && !istype(instance, /datum/reagent/medicine/biomass/xeno))
				to_chat(user, span_warning("\The [src]因内容物不兼容而拒绝该烧杯."))
				return
...
/obj/machinery/cloning/vats/proc/grow_human(instant = FALSE)
	...
	visible_message("[icon2html(src, viewers(src))] <span><b>[src]</b>开始旋转,它开始创建一个新的克隆体.</span>")
...
/obj/machinery/cloning/vats/proc/finish_growing_human()
	...
	notify_ghosts(span_boldnotice("新的克隆体已就绪! 姓名: [name]"), enter_link = "claim=[REF(occupant)]", source = src, action = NOTIFY_ORBIT, flashwindow = TRUE)
...
/obj/machinery/cloning/vats/proc/eject_user(silent = FALSE)
	...
	to_chat(occupant, {"
<span class='notice'>你是一个刚生成的克隆体,你看起来是一名小队陆战队员,但仅此而已.
你对前世毫无记忆.

你很虚弱,最好先休息并恢复体力再投入战斗.</span>"})
	occupant.vomit()
