/obj/machinery/bot
	...
	desc = "一个通用型多用途机器人,如果你在游戏中看到这个请ahelp."
...
/obj/machinery/bot/attack_hand(mob/living/user)
	...
	if(user.a_intent != INTENT_HELP)
		return
	switch(tgui_alert(user, "你想开启\the [src][is_active ? "off" : "on"]吗?" , "机器人激活", list("No", "Yes")))
		if("No")
			return
		if("Yes")
			if(is_active)
...
/obj/machinery/bot/attack_ai(mob/user)
	if(!alter_operating_mode)
		to_chat(user, "这个机器人有防火墙,无法被远程访问.")
		return
	switch(tgui_alert(user, "你想开启\the [src][is_active ? "off" : "on"]吗?" , "机器人激活", list("No", "Yes")))
		if("No")
			return
