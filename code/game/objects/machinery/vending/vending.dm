...
/obj/machinery/vending/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	if(xeno_attacker.a_intent == INTENT_HARM)
		...
		if(prob(xeno_attacker.xeno_caste.melee_damage))
			...
			span_danger("我们陷入狂乱,将\the [src]砸碎!"), null, 5)
			malfunction()
			return TRUE
		else
	...
	if(tipped_level)
		to_chat(xeno_attacker, span_warning("没必要理会那件旧垃圾."))
		return FALSE
	...
	if(do_after(xeno_attacker, shove_time, IGNORE_HELD_ITEM, src, BUSY_ICON_HOSTILE))
		xeno_attacker.visible_message(span_danger("\The [xeno_attacker]将\the [src]撞倒!"), \
		span_danger("你将\the [src]撞倒!"), null, 5)
		tip_over()
	else
		tipped_level = 0
...
/obj/machinery/vending/can_interact(mob/user)
	...
	if(tipped_level == 2)
		user.visible_message(span_notice("[user]开始将自动售货机推回原位!"), span_notice("你开始将自动售货机推回原位.."))
		if(!do_after(user, 8 SECONDS, IGNORE_HELD_ITEM, src, BUSY_ICON_FRIENDLY))
			return FALSE

		user.visible_message(span_notice("[user]将[src]扶正了!"), span_notice("你将[src]扶正了!"))
		flip_back()
	return TRUE
...
/datum/vending_product/proc/attempt_restock(obj/item/item_to_stock, mob/user, show_feedback = TRUE)
	...
	if(item_to_stock.storage_datum) //Nice try, specialists/engis
		...
		if(!(storage_to_stock.storage_flags & BYPASS_VENDOR_CHECK)) //If your storage has this flag, it can be restocked
			user?.balloon_alert(user, "无法为容器补货!")
			return FALSE
		...
		if(isitemstack(item_to_stock))
			...
			if(stack.amount != initial(stack.amount))
				user?.balloon_alert(user, "[stack]已部分使用。请补充!")
				return FALSE
...
/obj/machinery/vending/proc/throw_item()
	...
	src.visible_message(span_warning("[src]向[target]发射了[throw_item.name]!"))
	. = TRUE
