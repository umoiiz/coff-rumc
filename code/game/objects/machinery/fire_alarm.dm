...
/obj/machinery/firealarm/multitool_act(mob/living/user, obj/item/I)
	...
	detecting = !detecting
	if(detecting)
		user.visible_message(span_warning("[user]重新连接了[src]的探测单元!"), span_warning("你重新连接了[src]的探测单元."))
	else
		user.visible_message(span_warning("[user]断开了[src]的探测单元!"), span_warning("你断开了[src]的探测单元."))
...
/obj/machinery/firealarm/wirecutter_act(mob/living/user, obj/item/I)
	...
	if(buildstage != FIRE_ALARM_BUILD_SECURED)
		return
	user.visible_message(span_warning("[user]剪断了\the [src]内部的电线!"), "你剪断了\the [src]内部的电线.")
