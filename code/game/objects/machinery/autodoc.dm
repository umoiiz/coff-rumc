...
/obj/machinery/autodoc
	...
	desc = "一台精巧的机器, 旨在能够以最少的人工干预对人进行手术. 然而, 其界面相当复杂, 其中大部分只对受过训练的医务人员有用."
...
/obj/machinery/autodoc/power_change()
	...
	if(is_operational() || !occupant)
		return
	visible_message("[src]启动了安全超控, 将占用者弹出.")
	surgery = FALSE
...
/obj/machinery/autodoc/process()
	...
	if(heal_brute)
		if(occupant.get_brute_loss() > 0)
			...
			if(prob(10))
				...
				to_chat(occupant, span_info("你感觉你的伤口被缝合并封闭起来."))
		else
			heal_brute = 0
	...
	if(heal_burn)
		if(occupant.get_fire_loss() > 0)
			...
			if(prob(10))
				...
				to_chat(occupant, span_info("你感觉烧焦的血肉正被切下并替换。"))
		else
			heal_burn = 0
	...
	if(heal_toxin)
		if(occupant.get_tox_loss() > 0)
			...
			updating_health = TRUE
			if(prob(10))
				visible_message("[src]在给舱内乘员换液时发出嗡嗡声和咕噜声。")
				to_chat(occupant, span_info("你感觉稍微好受了些。"))
		else
			heal_toxin = 0
...
/obj/machinery/autodoc/proc/surgery_op()
	...
	if(!length(surgery_todo_list))
		visible_message("[src]发出嗡嗡声, 没有排入任何外科手术。")
		return

	visible_message("[src]开始手术, 医疗舱随着一声响亮的咔哒声锁死。")
	surgery = TRUE
	update_icon()
						...
						if(S.limb_ref.body_part == CHEST) //if it's the chest check for gross parasites
							...
							if(A)
								for(A in occupant)
									...
									if(!occupant)
										break
									occupant.visible_message(span_warning("[src]从[occupant]的胸腔中灵巧地取出一条蠕动的寄生虫!"))
	...
	visible_message("\The [src]咔哒一声打开, 完成了所请求的操作。")
	surgery = 0
...
/obj/machinery/autodoc/proc/autodoc_scan(mob/living/carbon/human/patient)
	...
	visible_message(span_notice("\The [src]在存储[patient.real_name]的扫描报告时发出叮声。"))
...
/obj/machinery/autodoc/proc/do_eject()
	...
	if(forceeject)
		if(!surgery)
			visible_message("\The [src]被摧毁, 将[occupant]弹出并使其被碎片覆盖。")
			occupant.take_limb_damage(rand(10,20),rand(10,20))
		else
			visible_message("\The [src]在手术中途被摧毁时发生故障, 将[occupant]带着手术伤口弹出并使其被碎片覆盖。")
	...
	if(usr == occupant)
		if(surgery)
			to_chat(usr, span_warning("这东西在给你做手术的时候你根本出不去!"))
			return
		else
	...
	if(istype(I, /obj/item/reagent_containers/glass/beaker))
		...
		if(B.reagents.has_reagent(/datum/reagent/medicine/biomass, 30))
			if(stored_matter >= stored_matter_max)
				to_chat(user, span_warning("生物质罐[src]已满!"))
				return
			...
			to_chat(user, span_notice("储罐现在含有[stored_matter]单位, 共[stored_matter_max]单位。"))
			return
		else
...
/obj/machinery/computer/autodoc_console/examine(mob/living/user)
	. = ..()
	if(locked)
		. += span_warning("它目前处于锁定状态!")
	if(release_notice)
		. += span_notice("释放通知已开启。")

/obj/machinery/autodoc/examine(mob/living/user)
	. = ..()
	to_chat(user, span_notice("生物质储罐含有[stored_matter]单位, 共[stored_matter_max]单位。"))
	...
	if(!hasHUD(user,"medical"))
		. += span_notice("它含有: [occupant]。[active]")
		return
