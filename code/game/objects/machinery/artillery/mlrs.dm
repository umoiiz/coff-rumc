/obj/item/mortar_kit/mlrs
	name = "\improper TA-40L多管火箭炮系统"
	desc = "一种手动、需要乘员操作且可拖曳的多管火箭炮系统部件, 由地球联合政府陆战队使用, 旨在用弹药饱和覆盖一片区域以累计出大量火力, 因此射击时具有较高的散布以完成此任务. 仅以最多16发火箭弹进行齐射, 总共可容纳32发火箭弹. 使用60毫米火箭弹."
	icon_state = "mlrs"
...
/obj/machinery/deployable/mortar/howitzer/mlrs/attackby(obj/item/I, mob/user, params)
	...
	var/numrockets = tgui_input_number(user, "你想装填多少火箭弹?)", "装填数量", 0, 16, 0)
	...
	while(rocketsloaded < numrockets)
		...
		if(length(chamber_items) >= max_rounds)
			user.balloon_alert(user, "无法装填更多")
			return

		if(!(mortar_shell.type in allowed_shells))
			user.balloon_alert(user, "这个炮弹不匹配")
			return
