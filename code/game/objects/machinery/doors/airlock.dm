...
/obj/machinery/door/airlock/bumpopen(mob/living/user) //Airlocks now zap you when you 'bump' them open when they're electrified. --NeoFite
	...
	if(iscarbon(user) && isElectrified())
		...
		if(!H.gloves || H.gloves.siemens_coefficient)
			to_chat(H, span_danger("你感到一股强烈的电击贯穿全身!"))
...
/obj/machinery/door/airlock/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	if(density) //Make sure it's still closed
		open(TRUE)
		xeno_attacker.visible_message(span_danger("\The [xeno_attacker]将\the [src]撬开."), \
			span_danger("我们将\the [src]撬开."), null, 5)
...
/obj/machinery/door/airlock/attack_larva(mob/living/carbon/xenomorph/larva/M)
	for(var/atom/movable/AM in get_turf(src))
		if(AM != src && AM.density && !AM.CanPass(M, M.loc))
			to_chat(M, span_warning("\The [AM]阻止你从\the [src]下方挤过去!"))
			return
	if(locked || welded) //Can't pass through airlocks that have been bolted down or welded
		to_chat(M, span_warning("\The [src]被牢牢锁死.你无法从下方挤过去!"))
		return
	M.visible_message(span_warning("\The [M]从\the [src]下方快速爬过!"), \
	span_warning("你挤过并从\the [src]下方快速爬过."), null, 5)
	M.forceMove(loc)
...
/obj/machinery/door/airlock/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/clothing/mask/cigarette) && isElectrified())
		...
		L.light(span_notice("[user]用[src]的电弧点燃了他们的[L]"))
	...
	if(iswelder(I) && !operating && density)
		...
		if(user.a_intent != INTENT_HELP)
			...
			welded = !welded
			user.visible_message("[user.name]已[welded? "welded shut":"unwelded"][src].", \
			...
			user.visible_message(span_notice("[user.name]修复了[src]。"), \
				span_notice("你完成了气闸的修复。"))
			update_icon()
		...
		if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
			user.visible_message(span_notice("[user]摸索着研究如何拆解[src]。"),
			span_notice("你摸索着研究如何拆解[src]。"))
		...
		user.visible_message("[user]开始从气闸组件上拆除电子设备。", "你开始从气闸组件上拆除电子设备。")
		...
		to_chat(user, span_warning("气闸的马达抵抗你的强行开启。"))

	else if(locked)
		to_chat(user, span_warning("气闸的螺栓阻止了强行开启。"))
...
/obj/machinery/door/airlock/proc/user_toggle_open(mob/user)
	...
	if(locked)
		to_chat(user, span_warning("门螺栓已落下。"))
		return
...
/obj/machinery/door/airlock/proc/shock_restore(mob/user)
	...
	if(wires.is_cut(WIRE_SHOCK))
		to_chat(user, span_warning("通电线路被切断了。"))
		return
...
/obj/machinery/door/airlock/proc/bolt_raise(mob/user)
	...
	if(wires.is_cut(WIRE_BOLTS))
		to_chat(user, span_warning("门螺栓线路被切断了。"))
		return
...
/obj/machinery/door/airlock/psi_act(psi_power, mob/living/user)
	if(operating)
		to_chat(user, span_warning("气闸已经在移动了。"))
		return
	if(welded)
		to_chat(user, span_warning("气闸被焊接封死了。"))
		return
