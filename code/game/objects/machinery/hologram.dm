...
/obj/machinery/holopad/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if("AIrequest")
			...
			if(last_request + 200 < world.time)
				...
				for(var/mob/living/silicon/ai/AI as anything in GLOB.ai_list)
					if(!AI.client)
						continue
					to_chat(AI, span_info("请求你前往<a href='byond://?src=[REF(AI)];jumptoholopad=[REF(src)]'>\the [area]</a>。"))
		...
		if("holocall")
			...
			var/result = tgui_input_list(ui.user, "选择要呼叫的区域", "全息呼叫", callnames)
...
/obj/machinery/holopad/Topic(href, href_list)
	...
	if(href_list["AIrequest"])
		...
		if(usr.loc == loc)
			...
			var/result = tgui_input_list(usr, "选择要呼叫的区域", "全息呼叫", callnames)
...
/obj/machinery/holopad/proc/activate_holo(mob/living/user)
	...
	if(is_operational() && (!AI || AI.eyeobj.loc == loc))//If the projector has power and client eye is on it
		if (AI && istype(AI.current, /obj/machinery/holopad))
			to_chat(user, "[span_danger("ERROR:")] \black 影像传输进行中。")
			return
