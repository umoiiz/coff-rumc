...
/obj/machinery/smartfridge/attackby(obj/item/I, mob/user, params)
	...
	if(machine_stat & NOPOWER)
		to_chat(user, span_notice("\The [src]没有供电,毫无用处."))
		return

	if(accept_check(I))
		if(length(contents) >= max_n_of_items)
			to_chat(user, span_notice("\The [src]满了."))
			return TRUE
		...
		if(plants_loaded)
			user.visible_message(span_notice("[user]将\the [P]装入\the [src]."), \
				span_notice("你将\the [P]装入\the [src]."))

			if(length(P.contents) > 0)
				to_chat(user, span_notice("一些物品被拒绝了."))
