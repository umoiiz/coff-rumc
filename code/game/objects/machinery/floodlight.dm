...
/obj/machinery/floodlight/landing/hq
	...
	desc = "一盏设在基地上以提供更好视野的强力灯。"


/obj/machinery/floodlight/landing/testroom
	name = "Ambience Light"
	desc = "一盏隐蔽设在基地上以提供更好视野的强力灯。"
	density = 0
	alpha = 0
...
/obj/machinery/deployable/floodlight/examine(mob/user)
	. = ..()
	. += span_notice("它还剩余[obj_integrity]/[max_integrity]的完整度。")
...
/obj/machinery/colony_floodlight_switch
	...
	desc = "这个开关控制考古建筑群周围的泛光灯。它只在有电力时运作。"
	density = FALSE
	anchored = TRUE
...
/obj/machinery/colony_floodlight_switch/attack_hand(mob/living/user)
	...
	if(!ishuman(user))
		...
		return FALSE
	if(machine_stat & NOPOWER)
		to_chat(user, span_notice("什么也没发生。"))
		return FALSE
	playsound(src,'sound/machines/click.ogg', 15, 1)
