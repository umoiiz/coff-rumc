/obj/machinery/suit_storage_unit
	...
	desc = "一个工业U-Stor-It存储单元,设计用于容纳各种太空服。其机载设备还允许用户通过紫外线净化循环对内容物进行净化。控制面板上悬挂着一个警告标签,上面写着\"严禁在单元范围内放置生物制品\"。"
...
/obj/machinery/suit_storage_unit/proc/start_UV(mob/user)
	...
	if(isopen)
		to_chat(user, "<font color='red'>单元存储未关闭 -- 正在中止。</font>")
		return
	...
	if(!inserted_helmet && !inserted_mask && !inserted_suit) //shit's empty yo
		to_chat(user, "<font color='red'>单元存储舱为空。没有可消毒的物品 -- 正在中止。</font>")
		return
	to_chat(user, span_notice("你启动了单元的烧灼循环。"))
	isUV = 1
	update_icon()
	updateUsrDialog()
...
/obj/machinery/suit_storage_unit/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/clothing/suit/space))
		...
		if(inserted_suit)
			to_chat(user, span_warning("单元中已经有一套服装。"))
			return

		if(!user.transferItemToLoc(S, src))
			return

		to_chat(user, span_notice("你将[S.name]装入存储隔间。"))
		inserted_suit = S

	else if(istype(I,/obj/item/clothing/head/helmet))
		var/obj/item/clothing/head/helmet/H = I
		if(inserted_helmet)
			to_chat(user, span_warning("单元中已经有一个头盔。"))
			return

		if(!user.transferItemToLoc(H, src))
			return

		to_chat(user, span_notice("你将[H.name]装入存储隔间。"))
		inserted_helmet = H

	else if(istype(I, /obj/item/clothing/mask))
		var/obj/item/clothing/mask/M = I
		if(inserted_mask)
			to_chat(user, span_warning("单元中已经有一个面罩。"))
			return

		if(!user.transferItemToLoc(M, src))
			return

		to_chat(user, span_notice("你将[M.name]装入存储隔间。"))
		inserted_mask = M

	else if(istype(I, /obj/item/tank))
		var/obj/item/tank/T = I
		if(inserted_tank)
			to_chat(user, span_warning("单元中已经有一个气罐。"))
			return

		if(!user.transferItemToLoc(T, src))
			return

		to_chat(user, span_notice("你将[T.name]装入存储隔间。"))
		inserted_tank = T
