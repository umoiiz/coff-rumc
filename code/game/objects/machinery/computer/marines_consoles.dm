...
/obj/machinery/computer/marine_card/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/card/id))
		...
		if(ACCESS_MARINE_LOGISTICS in idcard.access)
			if(scan && modify)
				to_chat(user, "两个插槽都已经满了.请先移除一张卡.")
				return
			if(!scan)
				user.drop_held_item()
	...
	switch(action)
		if("PRG_authenticate")
			if((!(authenticated) && (scan || (issilicon(user))) && (modify || mode)))
				...
				to_chat(user, "如果没有插入要修改的ID,你就无法修改ID.一旦有一张卡插入计算机上的修改插槽,你就可以登录了.")
			...
			if(authenticated && modify)
				...
				if(authenticated && modify == t2 && (in_range(src, user) || (issilicon(user))) && istype(loc, /turf))
					...
					if(temp_name)
						...
						visible_message(span_notice("[src]无礼地发出嗡嗡声."))
		...
		if("PRG_assign")
			if(authenticated)
				...
				if(t1 == "Custom")
					...
					if(!jobdatum)
						to_chat(user, span_warning("此职位没有日志记录."))
						return FALSE
	...
	switch(action)
		...
		if("PRG_squad")
			...
			if(selected)
				...
				to_chat(user, "[selected.name]小队已添加到卡中.")
			else
				to_chat(user, "未选择小队.")
			modify.name = "[modify.registered_name]'s ID Card ([modify.assignment])"
			. = TRUE
