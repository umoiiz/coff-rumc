	...
	switch(state)
		...
		if(1)
			if(iswrench(I))
				...
				to_chat(user, span_notice("你取出了电路板."))
				state = 1
				icon_state = "0"
		...
		if(2)
			if(isscrewdriver(I) && circuit)
				...
				if(C.get_amount() < 5)
					...
					return
				to_chat(user, span_notice("你开始向框架上添加线缆."))
				...
				to_chat(user, span_notice("你向框架上添加了线缆."))
				state = 3
				icon_state = "3"
		if(3)
			if(iswirecutter(I))
				playsound(loc, 'sound/items/wirecutter.ogg', 25, 1)
				to_chat(user, span_notice("你取下了线缆."))
				state = 2
				icon_state = "2"
		...
		if(4)
			if(iscrowbar(I))
				playsound(loc, 'sound/items/crowbar.ogg', 25, 1)
				to_chat(user, span_notice("你取下了玻璃面板."))
				state = 3
				icon_state = "3"
