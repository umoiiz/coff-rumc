...
/obj/machinery/computer/communications/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("setmsg1")
			if(!authenticated)
				return FALSE
			stat_msg1 = reject_bad_text(tgui_input_text(user, "第一行", "输入消息文本", stat_msg1, 40, encode = FALSE))
			. = TRUE

		if("setmsg2")
			if(!authenticated)
				return FALSE
			stat_msg2 = reject_bad_text(tgui_input_text(user, "第二行", "输入消息文本", stat_msg2, 40, encode = FALSE))
			. = TRUE
		...
		if("announce")
			...
			if(TIMER_COOLDOWN_RUNNING(user, COOLDOWN_HUD_ORDER))
				to_chat(user, span_warning("你最近刚发送过公告或消息!"))
				return FALSE
			if(world.time < cooldown_message + COOLDOWN_COMM_MESSAGE)
				to_chat(user, span_warning("请至少等待[COOLDOWN_COMM_MESSAGE * 0.1]秒\s 后再发送公告."))
				return FALSE

			var/input = tgui_input_text(user, "请撰写一条向空间站船员发布的公告.", "优先公告", "", multiline = TRUE, encode = FALSE, max_length = 100)
		...
		if("award")
			if(authenticated != 2)
				return FALSE
			if(!isliving(user))
				to_chat(user, span_warning("只有舰长才能授予勋章."))
				return FALSE
		...
		if("messageTGMC")
			...
			var/msg = tgui_input_text(user, "请选择一条要发送给TGMC最高指挥部的消息. 请注意,此过程非常昂贵,滥用将导致终止. 发送并不保证会得到回复. 在你发送另一条消息之前会有一小段延迟. 请清晰简洁.", "要中止,请发送一条空消息.", "", encode = FALSE)
		...
		if("evacuation_start")
			...
			if(!SSticker?.mode)
				to_chat(user, span_warning("在撤离进行之前,必须先激活[SSmapping.configs[SHIP_MAP].map_name]的求救信标."))
				return FALSE
			...
			if(!SSevacuation.initiate_evacuation())
				to_chat(user, span_warning("你目前无法启动撤离程序!"))
				return FALSE
		...
		if("delta_cancel")
			...
			if(!SSevacuation.cancel_evacuation())
				to_chat(user, span_warning("你目前无法取消撤离!"))
				return FALSE
