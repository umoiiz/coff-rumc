...
/obj/machinery/computer/intel_computer/process()
	...
	if(progress >= 100)
		...
		priority_announce("从[get_area(src)]收到了秘密传输.奖励已送达,形式为[supply_reward]点货运积分和[dropship_reward]点穿梭机积分.", title = "UPP情报部门", sound = 'sound/AI/bonus_climed.ogg')
...
/obj/machinery/computer/intel_computer/interact(mob/user)
	if(!active)
		to_chat(user, span_notice("这台终端上没有任何有用的东西."))
		return
	return ..()
