...
/obj/machinery/robotic_cradle
	name = "robotic cradle"
	desc = "一台高度实验性的机器人维护机器,使用工业纳米机器浴液来快速修复插入其中的任何机器人."
...
/obj/machinery/robotic_cradle/Destroy()
	if(occupant)
		visible_message("\The [src]在修复中途被摧毁时发生故障,将[occupant]弹出,其身上带有未完成的修复创口,并使其沐浴在碎片之中.")
...
/obj/machinery/robotic_cradle/proc/place_mob_inside(mob/living/target_mob, mob/operating_mob)
	...
	if(!(patient.species.species_flags & ROBOTIC_LIMBS))
		visible_message(span_warning("[src]发出蜂鸣声.对象为生物体,无法修复."))
		playsound(src, 'sound/machines/buzz-two.ogg', 50, FALSE)
		return FALSE
	if(patient.abiotic())
		visible_message(span_warning("[src]发出蜂鸣声.对象无法佩戴非生物物品."))
		playsound(src, 'sound/machines/buzz-two.ogg', 50, FALSE)
		return FALSE
	...
	if(operating_mob == patient)
		patient.visible_message(span_notice("[patient]开始爬入\the [src]."),
...
/obj/machinery/robotic_cradle/proc/start_emergency_eject(mob/mob_ejecting)
	...
	if(mob_ejecting == occupant)
		to_chat(usr, span_warning("这东西正在对你进行手术时,你是不可能出去的!"))
		return
	perform_eject(CRADLE_NOTICE_EARLY_EJECT)
