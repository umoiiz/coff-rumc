...
/obj/item/frame/camera/proc/try_build(turf/wall, mob/user)
	...
	user.visible_message("[user]将[src]安装到墙上。", \
...
/obj/structure/camera_assembly/examine(mob/user)
	...
	switch(state)
		if(STATE_WRENCHED)
			. += span_info("你可以用<b>焊接器</b>将其固定到位,或用<b>扳手</b>将其拆除。")
		if(STATE_WELDED)
			. += span_info("你可以向其中添加<b>电线</b>,或用<b>拆焊</b>将其从墙上取下。")
		if(STATE_WIRED)
			. += span_info("你可以用<b>螺丝刀</b>完成它,或用<b>拆线</b>开始拆除。")
		if(STATE_FINISHED)
			. += span_boldwarning("你不应该看到这个,请告诉程序员!")
...
/obj/structure/camera_assembly/attackby(obj/item/I, mob/living/user, params)
	...
	if(state == STATE_WELDED)
		...
		if(!C.use(2))
			to_chat(user, span_warning("你需要两段线缆才能给摄像头接线!"))
			return
		to_chat(user, span_notice("你向[src]添加了电线。"))
		state = STATE_WIRED
...
/obj/structure/camera_assembly/welder_act(mob/living/user, obj/item/I)
	...
	switch(state)
		...
		if(STATE_WELDED)
			if(!weld(I, user))
				return
			to_chat(user, span_notice("你将[src]从其位置拆焊。"))
			anchored = TRUE
...
/obj/structure/camera_assembly/wirecutter_act(mob/user, obj/item/I)
	...
	to_chat(user, span_notice("你从电路上剪断了电线。"))
	state = STATE_WELDED
	return TRUE
...
/obj/structure/camera_assembly/wrench_act(mob/user, obj/item/I)
	if(state != STATE_WRENCHED)
		return FALSE
	I.play_tool_sound(src)
	to_chat(user, span_notice("你将[src]从其位置拆下。"))
