...
/obj/machinery/nuclearbomb/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	if(!do_after(xeno_attacker, 5 SECONDS, NONE, src, BUSY_ICON_DANGER, BUSY_ICON_HOSTILE))
		return
	xeno_attacker.visible_message("[xeno_attacker]拆除了核弹",
	"你拆除了核弹.")
...
/obj/machinery/nuclearbomb/attack_hand(mob/living/user)
	...
	if(removal_stage < NUKE_STAGE_BOLTS_REMOVED)
		if(anchored)
			visible_message(span_warning("伴随着一声响亮的哔声,[src]的显示面板上灯光闪烁.它启动了!"))
		else
			anchored = TRUE
			visible_message(span_warning("伴随着一声金属脆响,螺栓从[src]中滑出,将它固定在地板上!"))
	else
		visible_message(span_warning("\The [src]发出极其刺耳的嘎吱声.看起来固定螺栓已被切断."))
	if(!lighthack)
