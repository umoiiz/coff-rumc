	...
	switch(action)
		if("message")
			...
			if(TIMER_COOLDOWN_RUNNING(operator, COOLDOWN_HUD_ORDER))
				...
				return FALSE
			var/input = tgui_input_text(operator, "请写一条向小队宣布的消息:", "小队消息", max_length = MAX_COMMAND_MESSAGE_LENGTH)
			if(input)
		...
		if("sl_message")
			...
			if(TIMER_COOLDOWN_RUNNING(operator, COOLDOWN_HUD_ORDER))
				to_chat(operator, span_warning("你最近已经发送过公告或消息了!"))
				return FALSE
			var/input = tgui_input_text(operator, "请写一条向小队长宣布的消息:", "小队长消息", max_length = MAX_COMMAND_MESSAGE_LENGTH)
			if(input)
		...
		if("set_primary")
			...
			visible_message(span_boldnotice("小队'[current_squad]'的首要目标已设定."))
			. = TRUE
		...
		if("set_secondary")
			...
			if(issilicon(operator))
				to_chat(operator, span_boldnotice("小队'[current_squad]'的次要目标已设定."))
			visible_message(span_boldnotice("小队'[current_squad]'的次要目标已设定."))
			. = TRUE
		...
		if("change_lead")
			...
			var/new_lead = tgui_input_list(operator, "选择新的小队长", null, sl_candidates)
	...
	switch(choice)
		if(MESSAGE_SINGLE)
			if(TIMER_COOLDOWN_RUNNING(operator, COOLDOWN_HUD_ORDER))
				to_chat(operator, span_warning("你最近已经发送过公告或消息了!"))
				return
			var/input = tgui_input_text(source, "请写一条向该陆战队员宣布的消息:", "CIC消息", max_length = MAX_COMMAND_MESSAGE_LENGTH)
		...
		if(MESSAGE_NEAR)
			if(TIMER_COOLDOWN_RUNNING(operator, COOLDOWN_HUD_ORDER))
				to_chat(operator, span_warning("你最近已经发送过公告或消息了!"))
				return
			var/input = tgui_input_text(source, "请写一条向附近所有陆战队员宣布的消息:", "CIC近距消息", max_length = MAX_COMMAND_MESSAGE_LENGTH)
			...
			switch(choice)
				if(MESSAGE_SQUAD)
					if(TIMER_COOLDOWN_RUNNING(operator, COOLDOWN_HUD_ORDER))
						to_chat(operator, span_warning("你最近已经发送过公告或消息了!"))
						return
					var/input = tgui_input_text(source, "请写一条向小队宣布的消息:", "小队消息", max_length = MAX_COMMAND_MESSAGE_LENGTH)
					if(input)
...
/obj/machinery/computer/camera_advanced/overwatch/military/proc/attempt_spotlight(datum/source, atom/A, params)
	...
	if(istype(place) && place.ceiling >= CEILING_UNDERGROUND)
		to_chat(source, span_warning("你无法照亮此处. 这里很可能是地下."))
		return
...
/obj/machinery/computer/camera_advanced/overwatch/military/proc/handle_bombard()
	...
	for(var/mob/living/silicon/ai/AI AS in GLOB.ai_list)
		to_chat(AI, span_warning("通知 - 轨道轰炸已从督战控制台触发. 弹头类型: [warhead_type]. 目标: [AREACOORD_NO_Z(T)]"))
...
/obj/machinery/computer/camera_advanced/overwatch/military/proc/change_lead(datum/source, mob/living/carbon/human/target)
	...
	if(target_squad.squad_leader)
		...
		if(issilicon(source))
			to_chat(source, span_boldnotice("小队'[target_squad]'的小队长[target_squad.squad_leader]已被[target.real_name][target_squad.squad_leader.stat == DEAD ? "replaced" : "demoted and replaced"]! 正在记录至入伍档案."))
		visible_message(span_boldnotice("小队'[target_squad]'的小队长[target_squad.squad_leader]已被[target.real_name][target_squad.squad_leader.stat == DEAD ? "replaced" : "demoted and replaced"]! 正在记录至入伍档案."))
		target_squad.demote_leader()
	else
		target_squad.message_squad("Acting Squad Leader updated to [target.real_name].")
		if(issilicon(source))
			to_chat(source, span_boldnotice("[target.real_name]是小队'[target_squad]'的新小队长! 正在记录至入伍档案."))
		visible_message(span_boldnotice("[target.real_name]是小队'[target_squad]'的新小队长! 正在记录至入伍档案."))

	to_chat(target, "[icon2html(src, target)] <font size='3' color='blue'><B>\[督战\]: 你已因[target_squad.name]被晋升为\'[ismarineleaderjob(target.job) ? "SQUAD LEADER" : "ACTING SQUAD LEADER"]\'. 你的耳机可以访问指挥频道(:v).</B></font>")
...
/obj/machinery/computer/camera_advanced/overwatch/military/proc/transfer_squad(datum/source, mob/living/carbon/human/transfer_marine, datum/squad/new_squad)
	...
	if(issilicon(source))
		to_chat(source, span_boldnotice("[transfer_marine]已从小队'[old_squad]'调动至小队'[new_squad]'. 正在记录至入伍档案."))
	visible_message(span_boldnotice("[transfer_marine]已从小队'[old_squad]'调动至小队'[new_squad]'. 正在记录至入伍档案."))
	to_chat(transfer_marine, "[icon2html(src, transfer_marine)] <font size='3' color='blue'><B>\[督战\]:</b> 你已被调动至[new_squad]!</font>")
