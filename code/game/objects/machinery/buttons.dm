...
/obj/machinery/medical_help_button/attack_hand(mob/living/user)
	...
	if(!istype(user))
		return
	if(machine_stat & (NOPOWER|BROKEN))
		to_chat(user, span_warning("[src] 似乎没有反应."))
		return
	if(active)
		return
	use_power(active_power_usage)
...
/obj/machinery/button/valhalla/marine_button/attack_hand(mob/living/user)
	var/xeno_wanted = tgui_input_list(user, "你想生成哪种异形?", "异形生成", GLOB.all_xeno_types)
	if(!xeno_wanted)
		return
	QDEL_NULL(linked)
	turf_check(user)
...
/obj/machinery/button/valhalla/xeno_button/proc/spawn_menu(mob/user)
	...
	var/choice = tgui_input_list(user, "那么,我们要生成什么?", "英灵殿生成器", options)
...
/obj/machinery/button/valhalla/vehicle_button/proc/spawn_vehicles(mob/living/user)
	...
	var/selected_vehicle = tgui_input_list(user, "你想生成哪辆载具?", "载具生成", spawnable_vehicles)
	if(!selected_vehicle)
		return
	QDEL_NULL(linked)
	if(!get_turf(GLOB.valhalla_button_spawn_landmark[spawn_link]))
