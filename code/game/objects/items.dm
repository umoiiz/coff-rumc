...
/obj/item/attackby(obj/item/attacking_item, mob/user, params)
	...
	if(attacked_storage.storage_datum.collection_mode) //Mode is set to collect all items on a tile and we clicked on a valid one.
		...
		if(success && !failure)
			...
			to_chat(user, span_notice("你未能用[attacked_storage]捡起任何东西."))
	...
	switch(slot)
		...
		if(SLOT_WEAR_SUIT)
			...
			if(!H.w_uniform && (SLOT_W_UNIFORM in mob_equip))
				if(warning)
					to_chat(H, span_warning("你需要一件连体服才能安装这个[name]."))
				return FALSE
			equip_to_slot = TRUE
		if(SLOT_BELT)
			if(H.belt)
		...
		if(SLOT_S_STORE) //suit storage uniquely depends on the suit allowed list, so is a bit snowflake
			...
			if(!H.wear_suit && (SLOT_WEAR_SUIT in mob_equip))
				if(warning)
					to_chat(H, span_warning("你需要一件套装才能安装这个[name]。"))
				return FALSE
...
/obj/item/proc/zoom(mob/living/user, tileoffset, viewsize) //tileoffset is client view offset in the direction the user is facing. viewsize is how far out this thing zooms. 7 is normal view
	...
	if(zoom) //If we are zoomed out, reset that parameter.
		if(TIMER_COOLDOWN_FINISHED(user, COOLDOWN_ZOOM)) //If we are spamming the zoom, cut it out
			user.visible_message(span_notice("[user]从[zoom_device]上抬起头。"),
			span_notice("你从[zoom_device]上抬起头。"))
	...
	if(is_blind(user))
		to_chat(user, span_warning("你太瞎了,什么都看不见。"))
		return
	...
	if(SEND_SIGNAL(user, COMSIG_ITEM_ZOOM) & COMSIG_ITEM_ALREADY_ZOOMED)
		to_chat(user, span_warning("你已经在使用另一个变焦设备观察了。"))
		return
	...
	if(TIMER_COOLDOWN_FINISHED(user, COOLDOWN_ZOOM))
		...
		span_notice("你透过\the [zoom_device]观察。"))
	zoom = TRUE
...
/obj/item/proc/eyecheck(mob/user)
	...
	if(safety<2)
		if(E.damage >= E.min_broken_damage)
			...
			to_chat(H, span_warning("你的眼睛开始真的很痛了。这对你没好处!"))
			H.blind_eyes(5)
		else
			switch(safety)
				if(1)
...
/obj/item/proc/extinguish(atom/target, mob/user)
	...
	user.visible_message(span_danger("[user]从[src]中喷出水!"), \
	span_warning("你从[src]中喷出水。"),)
...
/obj/item/proc/do_trick(mob/living/carbon/human/user)
	...
	if(prob(chance))
		...
		if(prob(10))
			...
			user.visible_message(span_info("<b>[user]</b>像个大蠢货一样笨拙地摆弄[src]!"))
...
/obj/item/proc/basic_spin_trick(mob/living/carbon/human/user, direction = 1, obj/item/double)
	...
	if(double)
		user.visible_message("[user]灵巧地翻转并旋转[src]和[double]!", span_notice("你翻转并旋转[src]和[double]!"))
		animation_wrist_flick(double, 1)
	else
		user.visible_message("[user]灵巧地翻转并旋转[src]!", span_notice("你翻转并旋转[src]!"))
...
/obj/item/proc/throw_catch_trick(mob/living/carbon/human/user)
	...
	user.visible_message("[user]灵巧地翻转[src]并将其抛向空中!", span_notice("你翻转[src]并将其抛向空中!"))
	...
	if(user.get_inactive_held_item())
		user.visible_message("[user]用同一只手接住了[src]!", span_notice("你在[src]旋转到你手中时接住了它!"))
		return
...
/obj/item/proc/color_item(obj/item/facepaint/paint, mob/living/carbon/human/user)
	...
	if(length(selection_list) == 1)
		...
		selection = tgui_input_list(user, "选择一个设置", name, selection_list)
	...
	switch(selection)
		if(VARIANTS)
			var/variant = tgui_input_list(user, "选择一个变体", "变体", icon_state_variants)
		...
		if(HAIR_CONCEALING_CHANGE)
			var/concealment_variant = tgui_input_list(user, "选择你想遮盖多少头发?", "头发遮盖", hair_concealing_variants)
...
/obj/item/proc/alternate_color_item(obj/item/facepaint/paint, mob/user)
	...
	if(length(secondaries) == 1)
		...
		update_icon()
		return
	var/obj/item/selection = tgui_input_list(user, "选择要着色的次要部分。", "次要颜色选择", secondaries)
	if(!selection)
		return
