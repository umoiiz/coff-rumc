...
/obj/item/weapon/baton/examine(mob/user)
	. = ..()
	if(bcell)
		. += span_notice("电击棍电量为[round(bcell.percent())]%.")
	else
...
/obj/item/weapon/baton/proc/check_user_auth(mob/user)
	...
	if(istype(H))
		...
		if(!istype(I) || !check_access(I))
			H.visible_message(span_notice("[src]在[H]捡起它时发出哔哔声"), span_danger("警告:检测到未授权用户.拒绝访问..."))
			H.Paralyze(40 SECONDS)
			H.visible_message(span_warning("[src]发出哔哔声,并向[H]的身体发送电击!"))
...
/obj/item/weapon/baton/attack(mob/M, mob/user)
	...
	if(user.a_intent == INTENT_HARM)
		...
		if (ishuman(L))
			...
			if(!target_zone)
				L.visible_message(span_danger("[user]用\the [src]攻击[L],但没有命中!"))
				return 0
			...
			if (affecting)
				if(!status)
					L.visible_message(span_warning("[L]被[user]用[src]捅了[affecting.display_name].幸好它没开."))
					return 1
				else
					H.visible_message(span_danger("[L]被[user]用[src]捅了[affecting.display_name]!"))
		else
			if(!status)
				L.visible_message(span_warning("[L]被[user]用[src]捅了.幸好它没开."))
				return 1
			else
				L.visible_message(span_danger("[L]被[user]用[src]捅了!"))
...
/obj/item/weapon/stunprod/attack(mob/M, mob/user)
	if(user.a_intent == INTENT_HARM)
		...
		M.visible_message(span_warning("[M]被[user]用未开启的[src]戳了一下."))
		return
...
/obj/item/weapon/stunprod/improved
	charges = 30
	name = "改良电击棒"
	desc = "一种专门用于制服异形生命体的电击棒.这根似乎比前代有效得多."
	color = "#FF6666"
