...
/obj/item/weapon/shield/riot/examine(mob/user, distance, infix, suffix)
	...
	if(integrity_failure && obj_integrity <= integrity_failure)
		. += span_notice("它已经完全损坏, 到处都是破洞!")
		return
	switch(health_status)
		if(0 to 20)
			. += span_notice("它正在自身重量下解体!")
		if(20 to 40)
...
/obj/item/weapon/shield/riot/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/stack/sheet/metal))
		...
		if(metal_sheets.get_amount() < 1)
			to_chat(user, span_warning("你需要一块金属板来修复[src]的结构完整性."))
			return

		visible_message(span_notice("[user]开始修复[src]的结构完整性."))
		...
		visible_message(span_notice("[user]修复了[src]的结构完整性."))
...
/obj/item/weapon/shield/riot/marine/som
	...
	desc = "一面坚固的重型盾牌, 设计用来替持盾者挡子弹. 常被SOM在登舰行动和其他近距离战斗场景中使用. 这面盾牌正面印有SOM旗帜."
	icon = 'icons/obj/items/weapons.dmi'
...
/obj/item/weapon/shield/energy
	name = "能量战斗盾"
	desc = "一面能够阻挡大多数投射物和近战攻击的盾牌. 它可以收起, 展开, 并存放在任何地方."
	icon = 'icons/obj/items/weapons.dmi'
...
/datum/action/ability/activable/weapon_skill/shield_bash/use_ability(atom/A)
	...
	carbon_owner.visible_message(span_danger("[carbon_owner]向前猛推他们的盾牌!"))
