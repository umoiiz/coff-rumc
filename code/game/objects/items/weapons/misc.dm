...
/obj/item/weapon/powerfist/examine(user)
	. = ..()
	. += span_notice("使用<b>手持</b>来更改功率设置.其功率设置为<b>[setting]</b>.")
	if(cell)
		. += span_notice("<b>点击</b>护手以弹出电池.")
...
/obj/item/weapon/powerfist/attack(mob/living/carbon/M, mob/living/carbon/user)
	...
	if(powerused > cell.charge)
		...
		if(M == user)
			...
			M.visible_message(span_danger("[user]的动力拳套在打向[M.name]时发出沉闷的砰声!"), \
	...
	M.visible_message(span_danger("[user]的动力拳套在打向[M.name]时震颤着,将他们击飞!"), \
		span_userdanger("[user]的一拳将你击飞向后!"))
	playsound(loc, 'sound/weapons/energy_blast.ogg', 50, TRUE)
	playsound(loc, 'sound/weapons/genhit2.ogg', 50, TRUE)
