...
/obj/item/toy/gun
	name = "玩具枪"
	desc = "看起来几乎和真的一样! 适合8岁及以上儿童. 当你用完火药帽后, 请将其回收进自动车床!"
	icon_state = "capgun"
	worn_icon_state = "gun"
...
/obj/item/toy/gun/examine(mob/user)
	. = ..()
	to_chat(user, "[src]里还剩[bullets]个火药帽\s .")
...
/obj/item/toy/gun/attackby(obj/item/toy/gun_ammo/A as obj, mob/user as mob)
	...
	if (istype(A, /obj/item/toy/gun_ammo))
		if (src.bullets >= 7)
			...
			return 1
		if (A.amount_left <= 0)
			to_chat(user, span_warning("没有更多火药帽了!"))
			return 1
		if (A.amount_left < (7 - bullets))
			src.bullets += A.amount_left
			to_chat(user, span_warning("你重新装填了[A.amount_left]个火药帽\s !"))
			A.amount_left = 0
		else
			to_chat(user, span_warning("你重新装填了[7 - bullets]个火药帽\s !"))
...
/obj/item/toy/gun_ammo
	name = "火药帽"
	desc = "还剩7个火药帽! 当盒子空了时, 请务必将其回收进自动车床."
	icon_state = "cap_ammo"
...
/obj/item/toy/crossbow
	name = "泡沫飞镖弩"
	desc = "一种深受许多过度活跃的孩子们喜爱的武器. 适合8岁及以上儿童."
	...
	if(istype(I, /obj/item/toy/crossbow_ammo))
		if(bullets <= 4)
			if(user.drop_held_item())
				qdel(I)
				bullets++
				to_chat(user, span_notice("你把泡沫飞镖装进弩里."))
		else
...
// ******* Check
	...
	if (bullets > 0 && M.lying_angle)
		visible_message(span_danger("[user]随意地瞄准[M]的头部并扣动了扳机!"), null, span_warning("你听到泡沫撞击头骨的声音"))
		...
		var/mob/living/L = M
		L.visible_message(span_danger("[user]随意地瞄准[L]的头部,扣动扳机,然后意识到自己没弹药了,趴到地上开始搜寻!"))
		L.Paralyze(10 SECONDS)
	return

/obj/item/toy/crossbow_ammo
	name = "泡沫飞镖"
	desc = "要么玩nerf要么什么都别玩! 8岁及以上."
...
/obj/item/toy/sword
	name = "玩具剑"
	desc = "一把廉价的塑料能量剑仿制品. 逼真的音效! 8岁及以上."
