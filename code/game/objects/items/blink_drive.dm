...
/obj/item/blink_drive
	name = "闪现驱动器"
	desc = "一种便携式蓝空间位移驱动器, 又称闪现驱动器. 可以将使用者传送到短距离外, 但有一定不可靠性, 并可能造成致命后果. 传送到超过5格的距离, 传送到视野外的格子或快速使用驱动器都会增加传送目的地的偏差."
...
/obj/item/blink_drive/examine(mob/user)
	. = ..()
	. += span_danger("<b>装备闪现驱动器时, 按住Alt右键点击或中键点击可传送到目的地.</b>")
...
/obj/item/blink_drive/proc/teleport(atom/A, mob/user)
	...
	if(pulled_target)
		...
		if(pulled_target != user.pulling)
			return
		user.balloon_alert(user, "把某人拖了过来")
...
/obj/item/blink_drive/proc/teleport_debuff_aoe(atom/movable/teleporter)
	...
	for(var/mob/living/living_target in range(1, teleporter))
		...
		to_chat(living_target, span_warning("当现实在你周围扭曲时, 你感到一阵恶心!"))
...
/datum/action/ability/activable/item_toggle/blink_drive/can_use_ability(atom/A, silent = FALSE, override_flags)
	...
	if(is_mainship_level(carbon_owner.z))
		carbon_owner.balloon_alert(carbon_owner, "无法在此使用")
		return FALSE
	return ..()
