...
/obj/item/portable_vendor
	name = "\improper 自动化储物公文包"
	desc = "一个手提箱大小的自动化存储与检索系统.设计用于高效存储和选择性分发小件物品."
...
/obj/item/portable_vendor/proc/malfunction()
	var/turf/T = get_turf(src)
	T.visible_message(span_warning("[src]颤抖着,其内部组件碎裂开来!"))
	broken = TRUE
	STOP_PROCESSING(SSobj, src)
	update_appearance()
...
/obj/item/portable_vendor/corporate
	name = "\improper 纳米千星自动化储物公文包"
	desc = "一个手提箱大小的自动化存储与检索系统.设计用于高效存储和选择性分发小件物品.这面的侧面印有纳米千星的标志."
