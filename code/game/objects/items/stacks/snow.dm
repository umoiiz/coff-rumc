...
/obj/item/stack/snow/attackby(obj/item/I, mob/user, params)
	...
	to_chat(user, span_notice("你从[src]取雪."))
	ET.update_icon()
	use(transf_amt)
	return TRUE
...
/obj/item/stack/snow/attack_self(mob/user)
	...
	if(T.get_dirt_type() != DIRT_TYPE_SNOW)
		to_chat(user, span_warning("你不能在这个位置建造雪制路障!"))
		return
	...
	if(amount < 5)
		to_chat(user, span_warning("你需要5堆雪来建造路障."))
		return
	...
	for(var/obj/O in user.loc) //Objects, we don't care about mobs. Turfs are checked elsewhere
		...
		if(O.atom_flags & ON_BORDER)
			if(O.dir == user.dir)
				to_chat(user, span_warning("这个方向已经有\a [O.name]了!"))
				return
		else
			to_chat(user, span_warning("你需要一片空旷的区域来建造沙袋路障!"))
			return
			
	user.visible_message(span_notice("[user]开始组装雪制路障."),
	span_notice("你开始组装雪制路障."))
	...
	user.visible_message(span_notice("[user]组装了一个雪制路障."),
	span_notice("你组装了一个雪制路障."))
	SB.setDir(user.dir)
	use(5)
