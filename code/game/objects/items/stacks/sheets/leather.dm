...
/obj/item/stack/sheet/hairlesshide
	name = "无毛皮"
	desc = "这张皮已被去除毛发,但仍需要鞣制."
	singular_name = "hairless hide piece"
	icon_state = "sheet-hairlesshide"

/obj/item/stack/sheet/wetleather
	name = "湿皮革"
	desc = "这张皮革已被清洗,但仍需要晾干."
...
/obj/item/stack/sheet/animalhide/attackby(obj/item/I, mob/user, params)
	...
	if(I.sharp)
		user.visible_message(span_notice("\the [user]开始从\the [src]上割下毛发"), span_notice("你开始从\the [src]上割下毛发"), "你听到刀摩擦肉的声音")
		if(!do_after(user,50, NONE, src, BUSY_ICON_HOSTILE))
			return

		to_chat(user, span_notice("你从这个[singular_name]上割下了毛发"))
