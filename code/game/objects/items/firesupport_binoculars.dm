/obj/item/binoculars/fire_support
	name = "战术双筒望远镜"
	desc = "一副双筒望远镜,用于为空袭和巡航导弹标记目标. 独特动作可切换模式. 使用时Ctrl+点击可瞄准某个目标."
...
/obj/item/binoculars/fire_support/examine(mob/user)
	...
	if(!mode)
		return
	. += span_boldnotice("它们当前设置为[mode.name]模式: 剩余[mode.uses == -1 ? "unlimited" : "[mode.uses]"]次使用.")
...
/obj/item/binoculars/fire_support/proc/bino_checks(atom/target, mob/living/user)
	...
	if(targ_area.ceiling >= CEILING_UNDERGROUND)
		to_chat(user, span_warning("深度警告: 目标太深,无法进行炮击."))
		return FALSE
	return TRUE
