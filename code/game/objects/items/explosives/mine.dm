...
/obj/item/explosive/mine
	name = "\improper M20 阔剑反人员地雷"
	desc = "M20阔剑是一种定向近炸引信反人员地雷, 由Armat Systems为地球政府陆战队设计."
...
/obj/item/explosive/mine/attack_self(mob/living/user)
	...
	if(locate(/obj/item/explosive/mine) in get_turf(src))
		to_chat(user, span_warning("这个位置已经有一颗地雷了!"))
		return
...
/obj/item/explosive/mine/proc/trip_mine(atom/movable/victim)
	...
	living_victim.visible_message(span_danger("当[victim]移动到它前面时,[icon2html(src, viewers(living_victim))]\The [src]发出咔嗒声."), \
	span_danger("当你移动到它前面时,[icon2html(src, viewers(living_victim))]\The [src]发出咔嗒声."), \
	span_danger("你听到一声咔嗒声."))
...
/obj/item/explosive/mine/pmc
	...
	desc = "M20P阔剑是一种定向近炸引信反步兵地雷,由Armat Systems设计供地球政府陆战队使用.它已被改装供NT PMC部队使用."
	icon_state = "m20p"

/obj/item/explosive/mine/anti_tank
	name = "\improper M92勇士反坦克地雷"
	desc = "M92勇士是一种反坦克地雷,由Armat Systems设计供地球政府陆战队用于对抗重型装甲,包括坦克和机甲."
	icon_state = "m92"
