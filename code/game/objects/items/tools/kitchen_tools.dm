...
/obj/item/tool/kitchen/knife
	name = "厨刀"
	icon_state = "knife"
	desc = "由SpaceCook公司制造的多用途厨师刀. 保证多年保持锋利."
	atom_flags = CONDUCT
	sharp = IS_SHARP_ITEM_ACCURATE
	edge = 1
	force = 10
...
/obj/item/tool/kitchen/knife/butcher
	name = "屠夫砍刀"
	icon_state = "butch"
	desc = "用来剁肉和把肉剁碎的大块头. 这也包括小丑和小丑副产品."
	atom_flags = CONDUCT
	force = 35
...
/obj/item/tool/kitchen/tray/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	...
	if(ishuman(M) && ((H.head && (H.head.inventory_flags & COVEREYES) ) || (H.wear_mask && (H.wear_mask.inventory_flags & COVEREYES) ) || (H.glasses && (H.glasses.inventory_flags & COVEREYES) )))
		to_chat(M, span_warning("你被托盘砸中脸部, 隔着你的面罩!"))
		if(prob(33))
			add_mob_blood(H)
			if(H.wear_mask)
		...
		to_chat(M, span_warning("你被托盘砸中脸部!"))
		if(prob(33))
			add_mob_blood(M)
			var/turf/location = H.loc
			if (istype(location, /turf))
		...
		if(prob(50))
			playsound(M, 'sound/items/trayhit1.ogg', 25, 1)
			visible_message(span_danger("[user]用托盘猛击[M]的脸部!"))
		else
