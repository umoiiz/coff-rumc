...
/obj/item/tool/surgery/scalpel/laser3
	name = "激光手术刀"
	desc = "附带了定向激光的手术刀,用于更精确的切割,同时避免血液进入术野.这把看起来是精密能量刀具的巅峰之作!"
...
/obj/item/tool/surgery/healing_gun/attackby(obj/item/O, mob/user)
	if(!HAS_TRAIT(user, TRAIT_YAUTJA_TECH))
		to_chat(user, span_warning("你完全不知道如何将\the [O]装入\the [src]!"))
		return
	if(istype(O, /obj/item/tool/surgery/healing_gel))
		if(loaded)
			to_chat(user, span_warning("治疗枪内已经有一枚胶囊了!"))
			return
