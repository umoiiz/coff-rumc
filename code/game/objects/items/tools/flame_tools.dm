...
/obj/item/clothing/mask/cigarette/attackby(obj/item/W, mob/user, params)
	...
	if(iswelder(W))
		...
		if(S.active)
			light(span_warning("[user]挥舞[user.p_their()][W],险些擦过[user.p_their()]的鼻子.在此过程中[user.p_they()]点燃了[user.p_their()][name]."))
		...
		if(istype(G, /obj/item/weapon/gun/energy/lasgun))
			...
			if(L.rounds)
				light(span_notice("[user]灵巧地用[L]的低功率设置点燃了[user.p_their()][src]."))
			else
				to_chat(user, span_warning("你试图用[L]点燃你的[src],但你的电源电池没有电了!"))
...
/obj/item/clothing/mask/cigarette/afterattack(obj/item/reagent_containers/glass/glass, mob/living/user, proximity)
	...
	if(istype(glass))	//you can dip cigarettes into beakers
		if(glass.reagents.trans_to(src, chem_volume))	//if reagents were transfered, show the message
			to_chat(user, span_notice("你把\the [src]浸入\the [glass]中."))
...
/obj/item/clothing/mask/cigarette/attack_self(mob/user)
	if(lit)
		user.visible_message(span_notice("[user]平静地丢下并踩在点燃的[src]上,瞬间将其熄灭."))
		playsound(src, 'sound/items/cig_snuff.ogg', 15, 1)
		die()
	return ..()
...
/obj/item/clothing/mask/cigarette/attack_obj(obj/target_object, mob/living/user)
	...
	if(fixture.status != LIGHT_BROKEN || !fixture.has_power())
		return ..()
	light(span_notice("[user]从破碎的灯上点燃了[user.p_their()][src]."))
	return TRUE
...
/obj/item/clothing/mask/cigarette/antitox
	name = "神经杀手香烟"
	desc = "一种新型香烟,用于抵御有毒气体,但可能仍会让你感到疲惫."
...
/obj/item/clothing/mask/cigarette/emergency
	name = "红色同志"
	desc = "一支鲜红色的香烟,上面似乎有俄罗斯品牌标识.然而\"俄罗斯红\"这几个字是明确无误的."
	icon_state = "rrcigoff"
...
/obj/item/clothing/mask/cigarette/tram
	name = "罂粟味香烟"
	desc = "地球政府的阿片类替代品,用水稀释以规避2112年阿片控制法案."
...
/obj/item/clothing/mask/cigarette/pipe/attack_self(mob/user as mob) //Refills the pipe. Can be changed to an attackby later, if loose tobacco is added to vendors or something.
	if(lit)
		...
		return
	if(smoketime <= 0)
		to_chat(user, span_notice("你重新装填了烟斗的烟草。"))
		smoketime = initial(smoketime)


/obj/item/clothing/mask/cigarette/pipe/cobpipe
	name = "玉米芯烟斗"
	desc = "一种由土气的乡巴佬推广开来的尼古丁摄取装置,在太空潮人的追捧下于现代乃至未来经久不衰。"
...
/obj/item/clothing/mask/cigarette/pipe/cobpipe/curved
	name = "弯柄玉米芯烟斗"
	desc = "通常见于将军,或无脑嗜血的突击队员手中...但很有格调!"
	...
	if(user.r_hand == src || user.l_hand == src)
		if(!heat)
			...
			if(istype(src, /obj/item/tool/lighter/zippo) )
				user.visible_message(span_rose("[user]连脚步都没停,就翻开盖子,一个流畅的动作点燃了[src]。"))
				playsound(loc, 'sound/items/zippo_on.ogg', 15, 1)
			else
				if(prob(95))
					user.visible_message(span_notice("试了几次之后,[user]总算点燃了[src]。"))
				else
					to_chat(user, span_warning("你在点打火机的时候烧到了自己。"))
					if (user.l_hand == src)
						user.apply_damage(2,BURN,"l_hand")
					else
						user.apply_damage(2,BURN,"r_hand")
					user.visible_message(span_notice("试了几次之后,[user]总算点燃了[src],不过[user.p_they()]在过程中烧伤了[user.p_their()]手指。"))
...
/obj/item/tool/lighter/proc/turn_off(mob/living/bearer, silent = TRUE)
	if(heat)
		...
		if(!silent)
			if(istype(src, /obj/item/tool/lighter/zippo) )
				bearer.visible_message(span_rose(">你听到一声轻响,[bearer]连看都没看自己在做什么,就关掉了[src]。"))
...
/obj/item/tool/lighter/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	...
	if(istype(M.wear_mask, /obj/item/clothing/mask/cigarette) && user.zone_selected == "mouth" && heat)
		...
		if(M == user)
			...
			if(istype(src, /obj/item/tool/lighter/zippo))
				cig.light(span_rose("[user]猛地掏出[name],为[M]举着。"))
			else
				cig.light(span_notice("[user]为[M]举着[name],并点燃了[cig.name]。"))
	else
		return ..()
