...
/obj/item/shard/welder_act(mob/living/user, obj/item/tool/weldingtool/WT)
	...
	for(var/obj/item/stack/sheet/G in user.loc)
		...
		if(G.amount >= G.max_amount)
			continue
		G.attackby(NG, user)
		to_chat(user, "你将新形成的玻璃加入堆中. 现在共有[NG.amount]片.")
	qdel(src)
...
/obj/item/shard/proc/on_cross(datum/source, atom/movable/AM, oldloc, oldlocs)
	...
	if(prob(20))
		to_chat(M, span_danger("[isxeno(M) ? "We" : "You"]踩到了\the [src],将其踩碎了!"))
		qdel(src)
		return
...
/obj/item/shard/shrapnel/bone_chips/xeno
	name = "异形骨骼碎片"
	desc = "尖锐、参差不齐的异形骨骼碎片. 看起来之前的主人经历了剧烈的爆炸..."
	icon_state = "alienbonechips"
	shardsize = FALSE

/obj/item/shard/phoron
	name = "塑钢碎片"
	desc = "一块塑钢玻璃碎片. 比普通玻璃碎片坚固得多. 但显然还不足以用作窗户."
	force = 8
	throwforce = 15
	icon_state = "phoron"
