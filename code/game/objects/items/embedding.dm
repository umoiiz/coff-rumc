...
/datum/limb/proc/limb_embed(obj/item/embedding, silent)
	...
	if(limb_status & LIMB_DESTROYED)
		return FALSE
	if(!silent)
		owner.visible_message(span_danger("\The [embedding]扎进了伤口!"))
	implants += embedding
...
/datum/limb/proc/process_embedded(obj/item/embedded)
	...
	switch(rand(1,3))
		if(1)
			. =span_warning("当你碰到体内的[embedded]时,一阵剧痛刺穿你的[display_name].")
		if(2)
			. =span_warning("你的动作让你[display_name]中的[embedded]痛苦地晃动.")
		if(3)
			. =span_warning("你移动时,你[display_name]中的[embedded]痛苦地扭动.")
	to_chat(owner, .)
...
/mob/living/proc/yank_out_object()
	...
	if(self)
		balloon_alert_to_viewers("将[selection]从[user.p_their()]体内拔出")
	else
		balloon_alert_to_viewers("将[selection]从[src]的体内拔出")
/*
