/obj/item/squad_transfer_tablet
	name = "小队调动平板"
	desc = "一款用于快速将小队成员从一个不称职的小队长手下转移到另一个小队长手下的平板."
...
/obj/item/squad_transfer_tablet/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(!ismarineleaderjob(human_user.job) && !ismarinecommandjob(human_user.job))
		to_chat(human_user, span_notice("只有指挥角色才能请求小队变更!"))
		return FALSE
	if(action != "transfer")
		return FALSE
	if(params["transfer_target"] in active_requests)
		to_chat(human_user, span_info("目标无法被调动: 调动请求已处于激活状态."))
		return FALSE
	var/mob/living/carbon/human/target = locate(params["transfer_target"])
	if(!istype(target))
		to_chat(human_user, span_info("目标无法被调动: 目标错误."))
		return FALSE
	var/new_squad_id = params["squad_id"]
	var/datum/squad/new_squad = SSjob.squads[new_squad_id]
	if(!new_squad)
		to_chat(human_user, span_info("目标无法被调动: 小队错误."))
		return FALSE
	to_chat(human_user, span_info("调动请求已发送."))
...
/obj/item/squad_transfer_tablet/proc/process_transfer(mob/living/carbon/human/target, datum/squad/new_squad, mob/living/carbon/human/user)
	if(tgui_alert(target, "你想要调动到[new_squad.name]吗? [new_squad.desc ? "Description: [new_squad.desc]" : ""]", "已请求调动到[new_squad.name]小队", list("Yes", "No"), 10 SECONDS) != "Yes")
