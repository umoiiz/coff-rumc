/obj/item/squad_transfer_tablet
	name = "小队调动平板"
	desc = "一块用于快速将小队成员从一个无能的小队长手下转移到另一个小队长手下的平板."
...
/obj/item/squad_transfer_tablet/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(!ismarineleaderjob(human_user.job) && !ismarinecommandjob(human_user.job))
		to_chat(human_user, span_notice("只有指挥角色才能请求小队调动!"))
		return FALSE
	if(action != "transfer")
		return FALSE
...
/obj/item/squad_transfer_tablet/proc/process_transfer(mob/living/carbon/human/target, datum/squad/new_squad, mob/living/carbon/human/user)
	if(tgui_alert(target, "你希望调动到[new_squad.name]吗? [new_squad.desc ? "Description: [new_squad.desc]" : ""]", "已请求调动到[new_squad.name]小队", list("Yes", "No"), 10 SECONDS) != "Yes")
