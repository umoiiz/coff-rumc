...
/obj/structure/closet/bodybag/MouseDrop(over_object, src_location, over_location)
	...
	if(length(contents))
		return FALSE
	visible_message(span_notice("[usr]将[name]折叠起来."))
	if(QDELETED(foldedbag_instance))
...
/obj/structure/closet/bodybag/projectile_hit(atom/movable/projectile/proj, cardinal_move, uncrossing)
	...
	if(!opened && bodybag_occupant)
		...
		balloon_alert(bodybag_occupant, "[proj]把你从袋子里震了出来")
		open()
...
/obj/structure/closet/bodybag/ex_act(severity)
	...
	if(severity <= EXPLODE_HEAVY)
		visible_message(span_danger("冲击波把[src]炸开了!"))
...
/obj/structure/closet/bodybag/proc/acidspray_act(datum/source, obj/effect/xenomorph/spray/acid_puddle)
	if(!opened && bodybag_occupant)
		...
		balloon_alert(bodybag_occupant, "酸液迫使你出来")
		open() //Get out
...
/obj/structure/closet/bodybag/effect_smoke(obj/effect/particle_effect/smoke/S)
	...
	if((CHECK_BITFIELD(S.smoke_traits, SMOKE_BLISTERING) || CHECK_BITFIELD(S.smoke_traits, SMOKE_XENO_ACID)) && !opened && bodybag_occupant)
		...
		balloon_alert(bodybag_occupant, "烟雾迫使你出来")
		open() //Get out
...
/obj/item/bodybag/cryobag
	name = "静滞袋"
	desc = "一种折叠的可重复使用袋子, 设计用于防止对内部人员造成额外伤害."
...
/obj/structure/closet/bodybag/cryobag
	name = "静滞袋"
	bag_name = "stasis bag"
	desc = "一种可重复使用的塑料袋, 设计用于防止对内部人员造成额外伤害."
	icon = 'icons/obj/cryobag.dmi'
...
/obj/item/trash/used_stasis_bag
	...
	desc = "它已被撕开. 你需要找到一台能够回收它的机器."

//MARINE SNIPER TARPS

/obj/item/bodybag/tarp
	name = "\improper V1 热抑制伪装布 (折叠)"
	desc = "TGMC狙击手携带的伪装布. 当正确使用时, 躺在伪装布下方的狙击手几乎与周围环境无法区分. 该伪装布含有热抑制编织层, 可隐藏穿戴者的热信号, 并提供光学伪装和气味抑制."
	icon = 'icons/obj/bodybag.dmi'
...
/obj/structure/closet/bodybag/tarp
	name = "\improper V1 热抑制伪装布"
	bag_name = "V1 thermal-dampening tarp"
	desc = "TGMC狙击手携带的主动伪装布. 当正确使用时, 躺在伪装布下方的狙击手几乎与周围环境无法区分. 该伪装布含有热抑制编织层, 可隐藏穿戴者的热信号, 并提供光学伪装和气味抑制."
	icon = 'icons/obj/bodybag.dmi'
