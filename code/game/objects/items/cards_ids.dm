...
/obj/item/card/emag_broken // left as decor for maps
	desc = "这是一张附有磁条并连接到某些电路的卡片. 它看起来损坏得太严重, 除了回收利用外别无他用."
	name = "损坏的加密序列器"
	icon_state = "emag"

/obj/item/card/id
	name = "身份卡"
	desc = "一张用于提供身份证明并确定对大量机械访问权限的卡片."
	icon_state = "id"
...
/obj/item/card/id/attack_self(mob/user as mob)
	user.visible_message("[user]向你显示: [icon2html(src, viewers(user))][name]: 职务: [assignment]")
...
/obj/item/card/id/silver
	name = "身份卡"
	desc = "一张银色卡片, 象征着荣誉与奉献."
	icon_state = "silver"
	worn_icon_state = "silver_id"

/obj/item/card/id/gold
	name = "身份卡"
	desc = "一张金色卡片, 象征着权力与威严."
	icon_state = "gold"
...
/obj/item/card/id/syndicate/attack_self(mob/user as mob)
	if(!src.registered_name)
		...
		switch(tgui_alert(user, "你想展示身份卡,还是重新命名?", "选择.", list("Rename","Show")))
			if("Rename")
