...
/obj/item/hand_item/circlegame/proc/wait_a_second(mob/living/owner, mob/living/sucker)
	...
	if(owner == sucker) // big mood
		...
		to_chat(sucker, span_danger("等一下...那难道是-"))
...
/obj/item/hand_item/circlegame/proc/gottem(mob/living/owner, mob/living/sucker)
	...
	if(!in_range(owner, sucker))
		to_chat(sucker, span_notice("呼...你在[owner]注意到你看到[owner.p_their()][name]之前移开了..."))
		return

	to_chat(owner, span_warning("[sucker]低头看着你的[name],然后试图移开[sucker.p_their()]的视线,但已经太晚了!"))
	to_chat(sucker, span_danger("<b>[owner]在你试图从[owner.p_their()][name]移开视线时,看到了你眼中的恐惧!</b>"))
	...
	owner.visible_message(span_danger("[owner]用[owner.p_their()][name]敲了[sucker]!"), span_danger("你用自己的[name]敲了[sucker]!"), \
