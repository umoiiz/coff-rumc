...
/obj/item/loot_box/attack_self(mob/user)
	...
	while(rolls)
		...
		if(!iseffect(loot_pick))
			user.visible_message("[user]从[src]里抽出了一个[loot_pick.name]!")
		rolls --
	qdel(src)
...
/obj/item/loot_box/supply_drop
	name = "补给空投"
	desc = "一个标有TGMC标志的箱子,里面装满了珍贵的军用战术装备."
	icon = 'icons/obj/items/items.dmi'
...
/obj/effect/supply_drop/xenomorph/proc/spawn_larva()
	...
	if(picked)
		...
		to_chat(new_xeno, span_xenoannounce("女王之母已通过蓝空间将我们抛送至此,我们为蜂巢而活!"))
...
/obj/item/loot_box/tgmclootbox
	name = "TGMC装备箱"
	desc = "一个由TGMC应请求送来的装备箱,没人知道里面装的是什么.你只知道它大概会不错."
	icon = 'icons/obj/items/items.dmi'
	icon_state = "lootbox"
	worn_icon_state = "lootbox"
...
/obj/item/storage/box/crate/loot
	name = "\improper 通用装备"
	desc = "一个装有某种装备的大箱子.将此精灵拖到你身上以打开它!\nNOTE: 你无法将物品放回此箱子内."
