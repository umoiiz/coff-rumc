...
/obj/item/implant/sandevistan
	name = "斯安威斯坦脊柱植入物"
	desc = "超载你的中枢神经系统以加快一切动作. 注意不要过度使用."
	...
	switch(time_on)
		if(1 SECONDS to 2 SECONDS)
			if(COOLDOWN_FINISHED(src, alertcooldown))
				to_chat(implant_owner, span_alert("你感到脊柱一阵刺痛."))
		...
		if(5.1 SECONDS to INFINITY)//no infinite abuse
			to_chat(implant_owner, span_userdanger("由于过度使用, 你的大脑和脊柱撕裂开来, 你感到一丝羞愧."))
			implant_owner.gib()
			return
...
/obj/item/implant/sandevistan/proc/toggle(silent = FALSE)
	...
	if(!silent)
		to_chat(implant_owner, span_notice("你将脊柱植入物 [active? "on" : "off"]."))
	update_icon()
