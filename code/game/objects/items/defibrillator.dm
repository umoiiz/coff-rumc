...
/obj/item/defibrillator/proc/charge_information(mob/living/carbon/human/user)
	...
	var/message
	message += span_info("其内部电池剩余[round(dcell.charge / charge_cost)]次使用,共[round(dcell.maxcharge / charge_cost)]次。\n")
...
/obj/item/defibrillator/proc/defib_ready(mob/living/carbon/human/patient, mob/living/carbon/human/user)
	if(!ready)
		...
		return FALSE
	if(!ishuman(patient))
		to_chat(user, span_warning("[src]上的说明没有提到如何复苏那个..."))
		return FALSE
	...
	if(patient.wear_suit && (patient.wear_suit.atom_flags & CONDUCT && !advanced)) // something conductive on their chest
		user.visible_message(span_warning("[icon2html(src, viewers(user))]\The [src]发出蜂鸣:电极板检测到电阻大于100,000欧姆。请移除干扰的服装或护甲后重试。"))
		return FALSE
	return TRUE
...
/obj/item/defibrillator/proc/defibrillate(mob/living/carbon/human/patient, mob/living/carbon/human/user)
	...
	if(dcell.charge <= charge_cost)
		user.visible_message(span_warning("[icon2html(src, viewers(user))]\The [src]发出蜂鸣:内部电池已耗尽。请寻找充电器。无法进行分析或施加电击。"))
		to_chat(user, span_boldwarning("你可以通过将除颤器点击拖拽到医护兵背包或挎包上来为其充电,或将其放入充电器中。"))
		return
	...
	if(fail_reason)
		user.visible_message(span_warning("[icon2html(src, viewers(user))]\The [src]发出蜂鸣:无法复苏 - [fail_reason]"))
		return
	...
	user.visible_message(span_notice("[user]开始在[patient]的胸口上放置电极片."),
	span_notice("你开始在[patient]的胸口上放置电极片."))
	...
	if(!do_after(user, shock_time, NONE, patient, BUSY_ICON_FRIENDLY, BUSY_ICON_MEDICAL))
		to_chat(user, span_warning("你停止在[patient]的胸口上放置电极片."))
		return
	...
	user.visible_message(span_notice("[user]用电极片电击[patient]."),
	span_notice("你用电极片电击[patient]."))
	...
	to_chat(patient, span_notice("<i><font size=4>你突然感到一阵电击,意识回归,将你拉回凡间...</font></i>"))
	user.visible_message(span_notice("[icon2html(src, viewers(user))] \The [src]发出哔声: 复苏成功."))
...
/obj/item/defibrillator/civi
	name = "应急除颤器"
	desc = "一种通过强力电击来复苏失去行动能力患者的设备. 这台似乎是民用型号."
...
/obj/item/defibrillator/advanced
	name = "高级应急除颤器"
	desc = "一种通过强力电击来复苏失去行动能力患者的设备. 可无视护甲,覆盖心脏部位,且不会损伤心脏本身,代价是消耗更多电量."
	icon_state = "civ_defib"
	charge_cost = 100
	advanced = TRUE
