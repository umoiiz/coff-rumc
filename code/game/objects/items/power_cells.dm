...
/obj/item/cell/attack_self(mob/user as mob)
	...
	if(issynth(user) && !CONFIG_GET(flag/allow_synthetic_gun_use))
		to_chat(user, span_warning("你的程序限制使用被改装的电池."))
		return
	log_bomber(user, "primed a rigged", src)
	user.visible_message(span_danger("[user]使[src]失稳;它很快就会爆炸!"),
	span_danger("你使[src]失稳;它很快就会爆炸!"))
...
/obj/item/cell/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_containers/syringe))
		...
		if(issynth(user) && !CONFIG_GET(flag/allow_synthetic_gun_use))
			to_chat(user, span_warning("你的程序限制改装电池."))
			return

		to_chat(user, "你将溶液注入电池中.")
		...
		if(!rigged)
			...
			span_notice("你用[I]将[src]改装成使用时会爆炸的炸弹."))
		else
			if(skill < SKILL_ENGINEER_ENGI)
				user.visible_message(span_notice("[user]笨手笨脚地摸索着如何操作[src]."),
				span_notice("你笨手笨脚地摸索着,试图弄清楚如何稳定[src]."))
				var/fumbling_time = SKILL_TASK_EASY
				if(!do_after(user, fumbling_time, NONE, src, BUSY_ICON_UNSKILLED))
					return
				if(prob((SKILL_ENGINEER_PLASTEEL - skill) * 20))
					to_chat(user, "<font color='danger'>在你笨拙地摆弄了几秒后,[src]愤怒地嗡嗡作响,仿佛被冒犯了.你有一种<b>非常</b>不好的预感.</font>")
					rigged = TRUE
					explode() //Oops. Now you fucked up (or succeeded only too well). Immediate detonation.
			user.visible_message(span_notice("[user]开始用[I]操作[src]。"),
			span_notice("你开始用[I]稳定[src],使其不会在使用时引爆。"))
			if(skill > SKILL_ENGINEER_ENGI)
			...
			overlays -= spark_overlay
			user.visible_message(span_notice("[user]完成了用[I]对[src]的操作。"),
			span_notice("你用[I]稳定了[src];它不会再在使用时引爆。"))
...
/obj/item/cell/crap
	...
	desc = "你无法超越等离子顶。" //TOTALLY TRADEMARK INFRINGEMENT
	maxcharge = 500
...
/obj/item/cell/mecha
	name = "小型放射性同位素电池"
	desc = "一块巨大的扭曲金属,作为机甲的能源核心。尽管它发出蓝色光芒,你大概还是不该舔它。"
	icon = 'icons/obj/stationobjs.dmi'
	icon_state = "trashmelt"
	worn_icon_state = "trashmelt"
	w_class = WEIGHT_CLASS_HUGE
...
/obj/item/cell/unmanned_vehicle
	name = "无人载具电池"
	desc = "一种致密、外观坚固的电池,用于为TGMC战斗无人载具供电。设计上可承受枪击、爆炸以及三轮滥用。"
	icon_state = "icell"
	maxcharge = 20000
