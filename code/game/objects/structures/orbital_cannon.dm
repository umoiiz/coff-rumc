...
/obj/structure/orbital_cannon
	name = "\improper 轨道炮"
	desc = "TGMC轨道炮系统.用于射击所环绕行星上的大型目标.它用固体燃料加速其弹头,在撞击时产生毁灭性的效果."
...
/obj/structure/orbital_cannon/proc/load_tray(mob/user)
	...
	if(!tray.warhead)
		if(user)
			to_chat(user, span_warning("弹托内没有弹头,装填操作已取消."))
		return

	if(tray.fuel_amt < 1)
		to_chat(user, span_warning("弹托内没有固体燃料,装填操作已取消."))
		return

	if(loaded_tray)
		to_chat(user, span_warning("弹托已装填完毕."))
		return
...
/obj/structure/orbital_cannon/proc/unload_tray(mob/user)
	...
	if(chambered_tray)
		to_chat(user, span_warning("弹托在进入膛室后无法卸载,请先开火."))
		return

	if(!loaded_tray)
		to_chat(user, span_warning("弹托未装填."))
		return
...
/obj/structure/orbital_cannon/proc/chamber_payload(mob/user)
	...
	if(!loaded_tray)
		to_chat(user, span_warning("你需要先装填弹托才能将其推入膛室."))
		return
	...
	if(!tray.warhead)
		if(user)
			to_chat(user, span_warning("弹托内没有弹头,取消推入膛室操作."))
		return

	if(tray.fuel_amt < 1)
		if(user)
			to_chat(user, span_warning("弹托内没有固体燃料,取消推入膛室操作."))
		return
...
/obj/structure/orbital_cannon/proc/handle_ob_firing_effects(turf/target, ob_sound = 'sound/effects/OB_incoming.ogg')
	...
	for(var/mob/living/current_mob AS in GLOB.mob_living_list)
		if(current_mob.z == z)
			...
			to_chat(current_mob, span_warning("[SSmapping.configs[SHIP_MAP].map_name]的甲板在轨道炮开火时剧烈震颤."))
			continue
...
/obj/structure/orbital_cannon/proc/fire_ob_cannon(turf/T, mob/user)
	...
	priority_announce(
		message = "立即离开杀伤区域!<br><br>弹头类型: [tray.warhead.warhead_kind_rus].<br>[fuel_warning]<br>目标: [get_area(T)].",
		title = "侦测到轨道轰炸发射指令!",
...
/obj/structure/orbital_tray/attack_powerloader(mob/living/user, obj/item/powerloader_clamp/attached_clamp)
	if(attached_clamp.loaded && istype(attached_clamp.loaded, /obj/structure/ob_ammo))
		...
		if(OA.is_solid_fuel)
			...
			if(warhead)
				to_chat(user, span_warning("[src]中已有弹头."))
				return
			warhead = OA
...
/obj/structure/ob_ammo/warhead/proc/impact_message(turf/target, impact_time = 10 SECONDS)
	...
	for(var/mob/living/our_mob in range(30, target))
		...
		our_mob.show_message(span_userdanger("天空闪耀<u>[relative_dir ? ("с " + dir2rutext(relative_dir) + "а от тебя") : "прямо над тобой"]</u>!"), EMOTE_VISIBLE,
			span_userdanger("天空传来一声巨响<u>[relative_dir ? ("примерно с " + dir2rutext(relative_dir) + "а от тебя") : "прямо над тобой"]</u>!"), EMOTE_AUDIBLE)
	...
	for(var/mob/living/our_mob in range(25, target))
		if(get_turf(our_mob) == target)
			...
			relative_dir = get_dir(our_mob, target)
		our_mob.show_message(span_userdanger("天空的轰鸣愈发响亮<u>[relative_dir ? ("с " + dir2rutext(relative_dir) + "а от тебя") : "прямо над тобой"]</u>!"), EMOTE_VISIBLE,
	...
	for(var/mob/living/our_mob in range(15, target))
		our_mob.show_message(span_userdanger("天哪,天空要炸开了!!!"), EMOTE_VISIBLE,
			span_userdanger("快离开这里!"), EMOTE_AUDIBLE)
...
/obj/machinery/computer/orbital_cannon_console/can_interact(mob/user)
	...
	if(!isobserver(user) && user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
		user.visible_message(span_notice("[user]摸索着弄明白如何使用控制台."),
		span_notice("你摸索着弄明白如何使用控制台."))
...
/obj/structure/ship_rail_gun
	name = "\improper 轨道炮"
	desc = "一种强大的舰对舰武器,有时以降低的效率用于地面支援."
	icon = 'icons/obj/machines/artillery.dmi'
	icon_state = "Railgun"
	density = TRUE
	anchored = TRUE
