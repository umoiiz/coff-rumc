...
/obj/structure/droppod/proc/start_launch_pod(mob/user, commanded_drop = FALSE)
	...
	if(!operation_started && world.time < SSticker.round_start_time + SSticker.mode.deploy_time_lock + DROPPOD_DEPLOY_DELAY)
		if(user)
			to_chat(user, span_notice("无法发射,飞船尚未抵达作战区域."))
		return
	#endif

	if(!locate(/obj/structure/drop_pod_launcher) in get_turf(src))
		if(user)
			to_chat(user, span_notice("错误.没有空投舱发射器无法发射[name]."))
		return
	...
	for(var/mob/living/silicon/ai/AI AS in GLOB.ai_list)
		to_chat(AI, span_notice("[user ? user : "unknown"]已将[src]发射向[target.loc],坐标X:[target_x] Y:[target_y]"))
...
/obj/structure/droppod/leader
	name = "\improper TGMC 宙斯指挥空投舱"
	desc = "一块由TGMC用于快速战术重新部署的威慑性金属钢块. 这一款具备指挥能力."
...
/obj/structure/droppod/nonmob/supply_pod
	name = "\improper TGMC 宙斯补给空投舱"
	desc = "一块由TGMC用于快速战术重新部署的威慑性金属钢块. 这一款设计用于运载补给."
...
/obj/structure/droppod/nonmob/turret_pod
	name = "\improper TGMC 宙斯哨戒炮空投舱"
	desc = "一块由TGMC用于快速战术重新部署的威慑性金属钢块. 这一款搭载了一套自部署哨戒炮系统."
...
/obj/structure/droppod/nonmob/mech_pod
	...
	desc = "一块由TGMC用于快速战术重新部署的威慑性金属钢块. 这是专为运载机甲而设计的更大型号."
...
/obj/structure/drop_pod_launcher/attack_powerloader(mob/living/user, obj/item/powerloader_clamp/attached_clamp)
	if(!istype(attached_clamp.loaded, /obj/structure/droppod))
		return ..()
	user.visible_message(span_notice("[user]将[attached_clamp.loaded]投放到[src]上, 咔哒一声就位!"),
	span_notice("你将[attached_clamp.loaded]投放到[src]上, 咔哒一声就位!"))
