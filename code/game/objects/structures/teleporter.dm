...
/obj/machinery/deployable/teleporter/examine(mob/user)
	...
	if(kit?.linked_teleporter)
		. += span_notice("它目前连接到位于[get_area(kit.linked_teleporter)]的传送器#[kit.linked_teleporter.self_tele_tag].")
	else
		. += span_notice("它没有连接到任何其他传送器.")
...
/obj/machinery/deployable/teleporter/attack_hand(mob/living/user)
	...
	if(!kit.linked_teleporter)
		to_chat(user, span_warning("\The [src]没有连接到任何其他传送器."))
		return
	...
	if(deployed_linked_teleporter.z != z)
		to_chat(user, span_warning("[src]和[deployed_linked_teleporter]距离太远了!"))
		return
	...
	if(!length(teleporting))
		to_chat(user, span_warning("在[src]上没有检测到可传送的内容!"))
		return
	...
	if(powered())
		...
		balloon_alert_to_viewers("使用了内部电量")
	...
	if(deployed_linked_teleporter.powered())
		...
		deployed_linked_teleporter.balloon_alert_to_viewers("使用了内部电量")
...
/obj/machinery/deployable/teleporter/crowbar_act(mob/living/user, obj/item/I)
	...
	if(!istype(kit))
		CRASH("A teleporter didn't have an internal item, or it was of the wrong type.")
	if(!kit.cell)
		to_chat(user, span_warning("没有电池可以取出!"))
		return
...
/obj/machinery/deployable/teleporter/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/teleporter_kit))
		if(kit.linked_teleporter)
			balloon_alert(user, "传送器已经与另一个传送器连接了!")
			return
		balloon_alert(user, "你将两个传送器互相连接了.")
...
/obj/item/teleporter_kit
	name = "\improper ASRS蓝空间传送器"
	desc = "一个蓝空间传送垫,用于将人员和设备短距离移动到另一个预先连接的传送器.如果区域未通电,则使用内置电池提供传送."
...
/obj/item/teleporter_kit/examine(mob/user)
	. = ..()
	. += span_notice("Ctrl+点击地砖进行部署,使用扳手取消部署,使用撬棍取出电池.")
...
/obj/item/teleporter_kit/attackby(obj/item/I, mob/user, params)
	...
	if(linked_teleporter == src)
		balloon_alert(user, "你不能将传送器与自身连接!")
		return
	balloon_alert(user, "你将两个传送器互相连接了.")
...
/obj/effect/teleporter_linker
	...
	desc = "两个蓝空间传送垫,用于将人员和设备短距离移动到另一个预先连接的传送器."
