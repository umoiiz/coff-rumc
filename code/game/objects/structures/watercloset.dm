...
/obj/structure/toilet/attack_hand(mob/living/user)
	...
	if(.)
		return
	if(swirlie)
		user.visible_message(span_danger("[user]把马桶圈砸在[swirlie.name]的头上!"), span_notice("你把马桶圈砸在[swirlie.name]的头上!"), "你听到瓷器回响.")
...
/obj/structure/toilet/crowbar_act(mob/living/user, obj/item/I)
	...
	user.visible_message(span_notice("[user] [cistern ? "replaces the lid on the cistern" : "lifts the lid off the cistern"]!"), span_notice("你[cistern ? "replace the lid on the cistern" : "lift the lid off the cistern"]!"), "你听到瓷器摩擦声.")
	cistern = !cistern
	update_icon()
...
/obj/structure/toilet/grab_interact(obj/item/grab/grab, mob/user, base_damage = BASE_OBJ_SLAM_DAMAGE, is_sharp = FALSE)
	...
	user.visible_message(span_danger("[user]给[grabbed_mob]洗头!"), span_notice("你给[grabbed_mob]洗头!"), "你听到马桶冲水声.")
...
/obj/machinery/shower
	name = "shower"
	desc = "HS-451.由 Nanotrasen 卫生部门于 2050 年代安装."
	icon = 'icons/obj/watercloset.dmi'
	icon_state = "shower"
	density = FALSE
	anchored = TRUE
...
/obj/machinery/shower/wrench_act(mob/living/user, obj/item/I)
	...
	to_chat(user, span_notice("你开始用\the [I]调节温度阀."))
	...
	switch(watertemp)
		...
		if(WATER_TEMP_BOILING)
			watertemp = "normal"
	user.visible_message(span_notice("[user]用\the [I]调节淋浴."), span_notice("你用\the [I]调节淋浴."))
...
/obj/item/toy/bikehorn/rubberducky
	name = "橡皮鸭"
	desc = "橡皮鸭你真好看,你让洗澡变得好好玩.橡皮鸭我超级超级喜欢~你~"	//thanks doohl
	icon = 'icons/obj/watercloset.dmi'
...
/obj/structure/sink/attack_hand(mob/living/user)
	...
	if(busy)
		balloon_alert_to_viewers("其他人正在清洗")
		return
...
/obj/structure/sink/attackby(obj/item/I, mob/user, params)
	...
	if(istype(RG) && RG.is_open_container() && RG.reagents.total_volume < RG.reagents.maximum_volume)
		...
		user.visible_message(span_notice("[user]使用\the [src]将[RG]装满\the ."),span_notice("你使用\the [src]将[RG]装满\the ."))
		return
	...
	user.visible_message( \
		span_notice("[user]使用\the [src]清洗\a [I]."), \
		span_notice("你使用\the [src]清洗\a [I]."))
