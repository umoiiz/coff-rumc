	...
	switch(state)
		if("01")
			...
			if(istype(I, /obj/item/stack/rods) && !secure)
				...
				to_chat(user, span_notice("你开始用金属棒加固风窗门."))
				...
				to_chat(user, span_notice("你加固了风窗门."))
				secure = "secure_"
				if(anchored)
			...
			//Adding cable to the assembly. Step 5 complete.
				...
				user.visible_message("[user]给风窗门组件接线.", "你开始给风窗门组件接线.")
				...
				to_chat(user, span_notice("你给风窗门接好线了!"))
				state = "02"
				if(secure)
		...
		if("02")
			...
			if(istype(I, /obj/item/circuitboard/airlock) && I.icon_state != "door_electronics_smoked")
				...
				user.drop_held_item()
				I.forceMove(src)
				to_chat(user, span_notice("你已经安装好气闸电子设备了!"))
...
/obj/structure/windoor_assembly/welder_act(mob/living/user, obj/item/I)
	...
	if(!WT.remove_fuel(0, user))
		to_chat(user, span_notice("你需要更多焊接燃料才能拆解风窗门组件."))
		return

	user.visible_message("[user]拆解了风窗门组件.", "你开始拆解风窗门组件.")
	...
	if(!src || !WT.isOn())
		return
	to_chat(user, span_notice("你拆解了风窗门组件!"))
...
/obj/structure/windoor_assembly/wrench_act(mob/living/user, obj/item/I)
	...
	if(!anchored)
		...
		user.visible_message("[user]将风窗门组件固定到地板上.", "你开始将风窗门组件固定到地板上.")

		if(!do_after(user, 4 SECONDS, NONE, src, BUSY_ICON_BUILD))
			return

		to_chat(user, span_notice("你已经固定好风窗门组件了!"))
		anchored = TRUE
		if(secure)
			name = "Secure Anchored Windoor Assembly"
		else
	...
	//Unwrenching an unsecure assembly un-anchors it. Step 4 undone
		...
		playsound(loc, 'sound/items/ratchet.ogg', 25, 1)
		user.visible_message("[user]将风窗门组件从地板上解除固定.", "你开始将风窗门组件从地板上解除固定.")

		if(!do_after(user, 4 SECONDS, NONE, src, BUSY_ICON_BUILD))
			return

		to_chat(user, span_notice("你已经解除固定风窗门组件了!"))
		anchored = FALSE
...
/obj/structure/windoor_assembly/wirecutter_act(mob/living/user, obj/item/I)
	...
	user.visible_message("[user]切断了气闸组件的电线.", "你开始切断气闸组件的电线.")

	if(!do_after(user, 4 SECONDS, NONE, src, BUSY_ICON_BUILD))
		return

	to_chat(user, span_notice("你切断了滑动门的电线.!"))
...
/obj/structure/windoor_assembly/screwdriver_act(mob/living/user, obj/item/I)
	...
	playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
	user.visible_message("[user]从气闸组件上拆除了电子设备.", "你开始从气闸组件上拆卸电子设备.")
	...
	to_chat(user, span_notice("你已经拆除了气闸电子设备!"))
	if(secure)
		name = "Secure Wired Windoor Assembly"
	else
		name = "Wired Windoor Assembly"
...
/obj/structure/windoor_assembly/crowbar_act(mob/living/user, obj/item/I)
	...
	user.visible_message("[user]将滑动门撬入框架.", "你开始将滑动门撬入框架.")

	if(!do_after(user, 4 SECONDS, NONE, src, BUSY_ICON_BUILD))
		return

	density = TRUE //Shouldn't matter but just incase
	to_chat(user, span_notice("你完成了滑动门!"))
...
/obj/structure/windoor_assembly/verb/flip()
	...
	if(facing == "l")
		to_chat(usr, "滑动门现在将向右滑动.")
		facing = "r"
	else
		facing = "l"
		to_chat(usr, "滑动门现在将向左滑动.")
