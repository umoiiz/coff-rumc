/obj/structure/bed
	name = "床"
	desc = "一个放置在矩形金属框架上的床垫.用于以舒适的方式支撑躺卧的人,尤其是用于日常睡眠.古老的技术,但仍然实用."
	icon_state = "bed"
...
/obj/structure/bed/proc/buckle_bodybag(obj/structure/closet/bodybag/B, mob/user)
	if(buckled_bodybag || buckled)
		return
	B.visible_message(span_notice("[user]将[B]扣到[src]上!"))
...
/obj/structure/bed/grab_interact(obj/item/grab/grab, mob/user, base_damage = 5, is_sharp = FALSE)
	...
	to_chat(user, span_notice("你将[grabbed_mob]放在[src]上."))
...
/obj/structure/bed/roller
	name = "担架床"
	desc = "一块基本的软垫皮革板搁在小框架上.一点也不舒服,但可以让病人在被快速转移到其他地方时躺下休息."
...
/obj/item/roller_holder/attack_self(mob/user as mob)
	...
	to_chat(user, span_notice("你展开了[R]."))
	qdel(held)
	held = null
...
/obj/structure/bed/medevac_stretcher
	name = "医疗后送担架"
	desc = "一种带有集成信标的医疗后送担架,可通过运输机吊运快速撤离受伤患者,并配有紧急蓝空间传送器,可将患者远程撤离到已链接的信标.可接收患者和尸体袋."
	icon = 'icons/obj/rollerbed.dmi'
...
/obj/structure/bed/medevac_stretcher/examine(mob/user)
	. = ..()
	. += span_warning("右键激活.独特动作可对自己激活.")

/obj/structure/bed/medevac_stretcher/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
...
/obj/structure/bed/medevac_stretcher/proc/activate_medevac_teleport(mob/user)
	...
	if(!linked_beacon)
		...
		to_chat(user, span_warning("[src]的蓝空间引擎未链接到任何医疗后送信标."))
		return

	if(user.faction != linked_beacon.faction)
		playsound(loc,'sound/machines/buzz-two.ogg', 25, FALSE)
		visible_message(span_warning("[src]的安全机制在位移前启动,因为未能检测到正确的识别码."))
		return

	if(world.time < last_teleport )
		playsound(loc,'sound/machines/buzz-two.ogg', 25, FALSE)
		to_chat(user, span_warning("[src]的蓝空间引擎仍在充能;将在[round(last_teleport - world.time) * 0.1]秒后准备就绪."))
		return

	if(user in buckled_mobs)
		to_chat(user, span_warning("当你被扣在[src]上时,你无法够到传送激活按钮."))
		return

	if(!linked_beacon.planted)
		playsound(loc,'sound/machines/buzz-two.ogg', 25, FALSE)
		to_chat(user, span_warning("[src]的蓝空间引擎所链接的医疗后送信标未部署且未激活!"))
		return

	if(!linked_beacon.check_power())
		playsound(loc,'sound/machines/buzz-two.ogg', 25, FALSE)
		to_chat(user, span_warning("[src]的蓝空间引擎所链接的医疗后送信标未通电."))
		return
	...
	user.visible_message(span_warning("[user]激活了[src]的蓝空间引擎,使其轰鸣着启动."),
	span_warning("你激活了[src]的蓝空间引擎,使其轰鸣着启动."))
...
/obj/structure/bed/medevac_stretcher/proc/on_mob_unbuckle(datum/source, mob/living/buckled_mob, force = FALSE)
	...
	playsound(loc,'sound/machines/buzz-two.ogg', 25, FALSE)
	visible_message(span_warning("[src]的安全机制启动,不再检测到被扣住的使用者."))
...
/obj/structure/bed/medevac_stretcher/proc/medevac_teleport(mob/user)
	...
	if(!linked_beacon || !linked_beacon.check_power() || !linked_beacon.planted) //Beacon has to be planted in a powered area.
		playsound(loc,'sound/machines/buzz-two.ogg', 25, FALSE)
		visible_message(span_warning("[src]的安全机制在位移前启动,因为未能检测到已通电,已链接且已部署的医疗后送信标."))
		return
	var/mob/living/M
	if(LAZYLEN(buckled_mobs))
		...
		playsound(loc,'sound/machines/buzz-two.ogg', 25, FALSE)
		visible_message(span_warning("[src]的蓝空间引擎中止了位移,无法检测到合适的撤离对象."))
		return
	...
	visible_message(span_notice("<b>[M]在一阵火花中消失,因为[src]的蓝空间引擎生成了其位移场.</b>"))
	if(buckled_bodybag)
...
/obj/item/roller/medevac
	name = "医疗后送担架"
	desc = "一张可以随身携带的折叠医疗后送担架.可用于将陆战队员瞬间传送到已链接的信标.别忘了信标!"
...
/obj/item/medevac_beacon
	name = "医疗后送信标"
	desc = "一种与医疗后送担架链接的专用传送信标;为担架的位移场提供目标目的地.警告:必须在已通电区域才能运作."
...
/obj/item/medevac_beacon/attack_self(mob/user)
	if(locked)
		to_chat(user, span_warning("[src]的界面已锁定!现在只有小队队长,医护兵或医疗官可以解锁它."))
		return
	user.drop_held_item()
	anchored = TRUE
	planted = TRUE
...
/obj/item/medevac_beacon/attack_hand(mob/living/user)
	...
	if(planted)
		anchored = FALSE
		planted = FALSE
		to_chat(user, span_warning("你取回并停用了[src]."))
		icon_state = "med_beacon0"
...
/obj/item/medevac_beacon/attack_ghost(mob/dead/observer/user)
	...
	if(length(linked_beds + linked_beds_deployed) > 1)
		...
		var/input = tgui_input_list(user, "选择要传送到的医疗后送点:", "幽灵医疗后送传送", destinations, null, 0)
		if(!input)
			return
...
/obj/item/medevac_beacon/attackby(obj/item/I, mob/user, params) //Corpsmen can lock their beacons.
	...
	if(istype(I, /obj/item/card/id))
		if(!allowed(user))
			...
			return
		locked = !locked
		user.visible_message(span_notice("[user] [locked ? "locks" : "unlocks"] [src]的界面."),
		span_notice("你[locked ? "lock" : "unlock"] [src]的界面."))
