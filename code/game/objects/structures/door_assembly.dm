...
/obj/structure/door_assembly/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/tool/pen))
		...
		if(C.get_amount() < 1)
			to_chat(user, span_warning("你需要一截线圈来给气闸组件接线."))
			return
		...
		if(istype(S, /obj/item/stack/sheet/glass/reinforced))
			...
			to_chat(user, span_notice("你将强化玻璃窗安装到了气闸组件中."))
			glass = 1
			...
			to_chat(user, span_notice("你将[M]镀层安装到了气闸组件中."))
			glass = "[M]"
	update_state()
...
/obj/structure/door_assembly/welder_act(mob/living/user, obj/item/I)
	...
	if(istext(glass))
		user.visible_message("[user]将[glass]镀层从气闸组件上焊下.", "你开始将[glass]镀层从气闸组件上焊下.")
		if(!do_after(user, 40, NONE, src, BUSY_ICON_BUILD))
			return
		if(!WT.isOn())
			return
		to_chat(user, span_notice("你焊下了[glass]镀层!"))
		var/M = text2path("/obj/item/stack/sheet/mineral/[glass]")
		new M(loc, 2)
		glass = 0
	else if(glass == 1)
		user.visible_message("[user]将玻璃板从气闸组件中焊出.", "你开始将玻璃板从气闸组件中焊出.")
		if(!do_after(user, 40, NONE, src, BUSY_ICON_BUILD))
			return
		if(!WT.isOn())
			return
		to_chat(user, span_notice("你焊出了玻璃板!"))
...
/obj/structure/door_assembly/wrench_act(mob/living/user, obj/item/I)
	...
	if(anchored)
		...
		user.visible_message("[user]将气闸组件固定到地板上.", "你开始将气闸组件固定到地板上.")
	if(!do_after(user, 40, NONE, src, BUSY_ICON_BUILD))
		return
	to_chat(user, span_notice("你[anchored ? "un" : ""]固定了气闸组件!"))
	anchored = !anchored
	update_state()
...
/obj/structure/door_assembly/wirecutter_act(mob/living/user, obj/item/I)
	...
	if(!do_after(user, 4 SECONDS, NONE, src, BUSY_ICON_BUILD))
		return
	to_chat(user, span_notice("你切断了气闸的电线!"))
...
/obj/structure/door_assembly/crowbar_act(mob/living/user, obj/item/I)
	...
	if(state != 2)
		return
	playsound(loc, 'sound/items/crowbar.ogg', 25, 1)
	user.visible_message("[user]拆除了气闸组件的电子设备.", "你开始拆除气闸组件的电子设备.")
	if(!do_after(user, 4 SECONDS, NONE, src, BUSY_ICON_BUILD))
		return
	to_chat(user, span_notice("你拆除了气闸的电子设备!"))
	state = 1
...
/obj/structure/door_assembly/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(state != 2)
		return
	playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
	to_chat(user, span_notice("现在正在完成气闸."))
	if(!do_after(user, 4 SECONDS, NONE, src, BUSY_ICON_BUILD))
		return
