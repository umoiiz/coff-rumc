...
/obj/structure/ladder/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
...
/obj/structure/ladder/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/explosive/grenade))
		...
		user.visible_message(span_warning("[user]摆好姿势投掷[G][ladder_dir_name][src]."),
		span_warning("你摆好姿势投掷[G][ladder_dir_name][src]."))

		if(!do_after(user, 10, NONE, src, BUSY_ICON_HOSTILE))
			return

		user.visible_message(span_warning("[user]投掷了[G][ladder_dir_name][src]!"),
		span_warning("你投掷了[G][ladder_dir_name][src]"))
		user.drop_held_item()
		...
		var/obj/structure/ladder/ladder_dest
		if(up && down)
			ladder_dir_name = tgui_alert(user, "向上还是向下投掷?", "梯子", list("Up", "Down", "Cancel"), 0)
			switch(ladder_dir_name)
				if("Up")
					ladder_dir_name = "up"
					ladder_dest = up
		...
		user.visible_message(span_warning("[user]摆好姿势投掷[F][ladder_dir_name][src]."),
		span_warning("你摆好姿势投掷[F][ladder_dir_name][src]."))

		if(!do_after(user, 10, NONE, src, BUSY_ICON_HOSTILE))
			return

		user.visible_message(span_warning("[user]投掷了[F][ladder_dir_name][src]!"),
		span_warning("你投掷了[F][ladder_dir_name][src]"))
		user.drop_held_item()
