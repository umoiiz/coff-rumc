/obj/structure/janitorialcart
	name = "清洁工推车"
	desc = "清洁工推车的终极之选!有空间放水、拖把、标牌、垃圾袋等等!"
...
/obj/structure/janitorialcart/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/storage/bag/trash) && !mybag)
		...
		if(I.reagents.total_volume < I.reagents.maximum_volume && mybucket)	//if it's not completely soaked we assume they want to wet it, otherwise store it
			...
			to_chat(user, span_notice("你在[mybucket]里弄湿了[I]."))
		...
		if(signs >= 4)
			to_chat(user, span_notice("[src]放不下更多标牌了."))
			return
...
/obj/structure/janitorialcart/Topic(href, href_list)
	...
	if(href_list["garbage"])
		if(mybag)
			...
			to_chat(user, span_notice("你从[src]中取出[mybag]."))
			mybag = null
	if(href_list["mop"])
		if(mymop)
			user.put_in_hands(mymop)
			to_chat(user, span_notice("你从[src]中取出[mymop]."))
			mymop = null
	if(href_list["spray"])
		if(myspray)
			user.put_in_hands(myspray)
			to_chat(user, span_notice("你从[src]中取出[myspray]。"))
			myspray = null
	if(href_list["replacer"])
		if(myreplacer)
			user.put_in_hands(myreplacer)
			to_chat(user, span_notice("你从[src]中取出[myreplacer]。"))
			myreplacer = null
	if(href_list["bucket"])
		if(mybucket)
			user.put_in_hands(mybucket)
			to_chat(user, span_notice("你从[src]中取出[mybucket]。"))
			mybucket = null
	if(href_list["sign"])
		if(signs)
			...
			if(Sign)
				...
				to_chat(user, span_notice("你从[src]中取出\a [Sign]。"))
				signs--
			else
