...
/client/verb/ooc(msg as text)
	...
	if(!check_rights(R_ADMIN, FALSE))
		...
		if(!GLOB.dooc_allowed && (mob.stat == DEAD))
			to_chat(usr, span_warning("死亡生物的OOC已被关闭."))
			return
...
/client/verb/select_ignore()
	...
	to_chat(src, span_infoplain("你现在已在OOC频道屏蔽[selection]."))
...
/client/verb/select_unignore()
	...
	if(!length(prefs.ignoring))
		to_chat(src, span_infoplain("你还没有屏蔽任何玩家!"))
		return

	var/selection = tgui_input_list(src, "选择一名玩家", "取消屏蔽", prefs.ignoring)

	if(isnull(selection))
		return

	if(!(selection in prefs.ignoring))
		to_chat(src, span_infoplain("你没有在屏蔽[selection]!"))
		return

	prefs.ignoring.Remove(selection)
	prefs.save_preferences()

	to_chat(src, span_infoplain("你不再在OOC频道屏蔽[selection]."))
