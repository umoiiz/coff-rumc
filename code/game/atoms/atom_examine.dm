...
/atom/proc/examine(mob/user)
	...
	if(user.can_use_codex() && SScodex.get_codex_entry(get_codex_value()))
		...
		. += span_notice("图鉴中有<a href='?_src_=codex;show_examined_info=[REF(src)];show_to=[REF(user)]'>相关信息</a>可供查看.")
	...
	if((get_dist(user,src) <= 2) && reagents)
		...
		if(reagents.reagent_flags & TRANSPARENT)
			...
			if(reagents.total_volume)
				. += span_notice("它还剩下[reagents.total_volume]单位\s .")
			else
				. += span_warning("它是空的.")
