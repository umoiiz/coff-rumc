...
/turf/closed/wall/Initialize(mapload, ...)
	...
	for(var/obj/item/explosive/mine/M in src)
		if(M)
			visible_message(span_warning("\The [M]在墙壁建成时被密封在里面."))
			qdel(M)
...
/turf/closed/wall/examine(mob/user)
	...
	if(wall_integrity == max_integrity)
		if (acided_hole)
			...
			. += span_notice("它看起来完好无损.")
	else
		var/integ = wall_integrity / max_integrity
		if(integ >= 0.6)
			. += span_warning("它看起来有轻微损伤.")
		else if(integ >= 0.3)
			. += span_warning("它看起来有中度损伤.")
		else
			. += span_danger("它看起来有严重损伤.")

		if(acided_hole)
			. += span_warning("墙上有一个大洞, 可能是由某种酸造成的.")

	// todo why does this not use defines?
	switch(d_state)
		if(1)
			. += span_info("外层镀层已被切开. 螺丝刀应该能拆下支撑线.")
		if(2)
			. += span_info("支撑线已被拆除. 喷灯应该能切开金属盖.")
		if(3)
			. += span_info("金属盖已被切开. 撬棍应该能把它撬开.")
		if(4)
			. += span_info("金属盖已被拆除. 扳手将拆下锚栓.")
		if(5)
			. += span_info("锚栓已被拆除. 剪线钳将处理液压管线.")
		if(6)
			. += span_info("液压管线已消失. 撬棍将撬开内护套.")
		if(7)
			. += span_info("内护套已消失. 喷灯应该能了结这堵墙.")
...
/turf/closed/wall/welder_act(mob/living/user, obj/item/tool/weldingtool/WT)
	. = ..()
	if(resistance_flags & INDESTRUCTIBLE)
		to_chat(user, span_warning("这堵墙似乎太坚固了, 你无法对它做什么!"))
		return
	if(!ishuman(user))
		to_chat(user, span_warning("你没有足够的灵巧度来做这个!"))
		return
	...
	switch(d_state)
		if(0)
			user.visible_message(span_notice("[user]开始切开外层镀层."),
			span_notice("你开始切开外层镀层."))
			if(!WT.use_tool(src, user, 6 SECONDS, 1, 25, null, BUSY_ICON_BUILD))
				return
			if(!iswallturf(src) || !WT?.isOn())
				return
			d_state = 1
			user.visible_message(span_notice("[user]切开了外层镀层."),
			span_notice("你切开了外层镀层."))
...
/turf/closed/wall/wrench_act(mob/living/user, obj/item/I)
	...
	if(d_state != 4)
		return
	user.visible_message(span_notice("[user]开始松开固定支撑杆的锚栓."),
	span_notice("你开始松开固定支撑杆的锚栓."))
...
/turf/closed/wall/wirecutter_act(mob/living/user, obj/item/I)
	...
	if(d_state != 5)
		return
	user.visible_message(span_notice("[user]开始解开液压管路的压接."),
	span_notice("你开始解开液压管路的压接."))
	...
	d_state = 6
	user.visible_message(span_notice("[user]完成了液压管路的压接解开."),
	span_notice("你完成了液压管路的压接解开."))
...
/turf/closed/wall/grab_interact(obj/item/grab/grab, mob/user, base_damage = BASE_WALL_SLAM_DAMAGE, is_sharp = FALSE)
	...
	switch(state)
		if(GRAB_PASSIVE)
			damage += base_damage
			grabbed_mob.visible_message(span_warning("[user]将[grabbed_mob]猛砸向[src]!"))
		...
		if(GRAB_NECK)
			...
			grabbed_mob.visible_message(span_danger("<big>[user]将[grabbed_mob]碾碎在[src]上!</big>"))
