/obj/structure/xeno/resin_jelly_pod
	name = "树脂凝胶荚"
	desc = "一个巨大的树脂荚. 里面是浓稠的粘性液体, 看起来不容易燃烧."
...
/obj/structure/xeno/resin_jelly_pod/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	if(xeno_attacker.status_flags & INCORPOREAL)
		return FALSE

	if((xeno_attacker.a_intent == INTENT_HARM && isxenohivelord(xeno_attacker)) || xeno_attacker.hivenumber != hivenumber)
		balloon_alert(xeno_attacker, "摧毁中...")
		if(do_after(xeno_attacker, HIVELORD_TUNNEL_DISMANTLE_TIME, IGNORE_HELD_ITEM, src, BUSY_ICON_BUILD))
			deconstruct(FALSE)
		return

	if(!chargesleft)
		balloon_alert(xeno_attacker, "没有剩余凝胶")
		to_chat(xeno_attacker, span_xenonotice("我们将手伸进\the [src], 但只找到树脂残渣. 我们应该再等等.") )
		return
