...
/obj/structure/xeno/trap/examine(mob/user)
	...
	switch(trap_type)
		...
		if(TRAP_SMOKE_NEURO)
			. += span_notice("里面有加压神经毒素.")
		if(TRAP_SMOKE_ACID)
			. += span_notice("里面有加压酸气.")
		if(TRAP_ACID_WEAK)
			. += span_notice("里面有加压弱酸.")
		if(TRAP_ACID_NORMAL)
			. += span_notice("里面有加压普通酸.")
		if(TRAP_ACID_STRONG)
			. += span_notice("里面有高压强酸。")
		else
			. += span_notice("它是空的。")
...
/obj/structure/xeno/trap/proc/trigger_trap(datum/source, atom/movable/AM, oldloc, oldlocs)
	...
	if(iscarbon(AM))
		...
		crosser.visible_message(span_warning("[crosser]绊倒在[src]上!"), span_danger("你绊倒在[src]上!"))
...
/obj/structure/xeno/trap/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	balloon_alert(xeno_attacker, "已装入[trap_type]")
