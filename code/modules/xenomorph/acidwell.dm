...
/obj/structure/xeno/acidwell/obj_destruction(damage_amount, damage_type, damage_flag, mob/living/blame_mob)
	if(!QDELETED(creator) && creator.stat == CONSCIOUS && creator.z == z)
		...
		if(A)
			to_chat(creator, span_xenoannounce("你感应到你在[A.name]的酸液池已被摧毁!") )
...
/obj/structure/xeno/acidwell/examine(mob/user)
	...
	if(!isxeno(user) && !isobserver(user))
		return
	. += span_xenonotice("一个由[creator]制造的酸液池.它目前有<b>[charges]/[XENO_ACID_WELL_MAX_CHARGES]次充能</b>.")
...
/obj/structure/xeno/acidwell/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	if(xeno_attacker.plasma_stored < XENO_ACID_WELL_FILL_COST) //You need to have enough plasma to attempt to fill the well
		balloon_alert(xeno_attacker, "还需要[XENO_ACID_WELL_FILL_COST - xeno_attacker.plasma_stored]点等离子体")
		return
	...
	charging = FALSE
	update_icon()
	balloon_alert(xeno_attacker, "现在有[charges]/[XENO_ACID_WELL_MAX_CHARGES]次充能")
	to_chat(xeno_attacker,span_xenonotice("我们向[src]添加酸液.它目前有<b>[charges]/[XENO_ACID_WELL_MAX_CHARGES]次充能</b>.") )
...
/obj/structure/xeno/acidwell/HasProximity(atom/movable/AM)
	...
	for(var/obj/item/explosive/grenade/sticky/sticky_bomb in stepper.contents)
		...
		if(sticky_bomb.stuck_to == stepper)
			...
			visible_message(span_danger("[src]嘶嘶作响,[sticky_bomb]在酸液中熔解了."))
			qdel(sticky_bomb)
			charges_used ++
