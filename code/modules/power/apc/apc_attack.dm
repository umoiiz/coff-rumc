/obj/machinery/power/apc/attack_alien(mob/living/carbon/xenomorph/X, damage_amount = X.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = "", effects = TRUE, armor_penetration = 0, isrightclick = FALSE)
	...
	if(beenhit >= pick(3, 4) && !CHECK_BITFIELD(machine_stat, PANEL_OPEN))
		...
		update_appearance()
		visible_message(span_danger("\The [src]的盖板弹开,露出了线路!"), null, null, 5)

	else if(CHECK_BITFIELD(machine_stat, PANEL_OPEN) && !allcut)
		wires.cut_all()
		update_appearance()
		visible_message(span_danger("\The [src]的线路在火花中崩断!"), null, null, 5)
		if(X.client)
...
/obj/machinery/power/apc/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/cell) && opened) //Trying to put a cell inside
		...
		if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
			user.visible_message(span_notice("[user]笨拙地摸索着如何将[C]装入[src]."),
			span_notice("你笨拙地摸索着如何将[C]装入[src]."))
		...
		if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
			user.visible_message(span_notice("[user]笨拙地摸索着该在哪里刷[I]到[src]上."),
			span_notice("你笨拙地摸索着该在哪里刷[I]到[src]上."))
		...
		if(C.get_amount() < 10)
			balloon_alert(user, "线路不足")
			return

		balloon_alert_to_viewers("开始接线[src]")
		...
		if(has_electronics)
			balloon_alert(user, "无法,电子元件仍在里面")
			return
		...
		if(((machine_stat & BROKEN)) && !opened && I.force >= 5)
			...
			balloon_alert_to_viewers("击倒[src]的面板")
			update_appearance()
		else
			if(issilicon(user))
...
/obj/machinery/power/apc/attack_hand(mob/living/user)
	...
	if(opened && cell && !issilicon(user))
		if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
			...
			if(!do_after(user, fumbling_time, NONE, src, BUSY_ICON_UNSKILLED))
				return
		balloon_alert_to_viewers("从[src]中移除[src]")
