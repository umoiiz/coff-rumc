...
/obj/machinery/power/smes/ui_act(action, list/params)
	...
	switch(action)
		...
		if("input")
			...
			if(target == "input")
				target = tgui_input_number(usr, "新的输入目标(0-[input_level_max]):", name, input_level)
				if(!isnull(target) && !..())
					. = TRUE
			else if(target == "min")
				target = 0
		...
		if("output")
			...
			var/adjust = text2num(params["adjust"])
			if(target == "input")
				target = tgui_input_number(usr, "新的输出目标(0-[output_level_max]):", name, output_level)
				if(!isnull(target) && !..())
					. = TRUE
			else if(target == "min")
				target = 0
...
/obj/machinery/power/smes/Topic(href, href_list)
	...
	if(href_list["cmode"])
		...
		switch( href_list["input"] )
			...
			if("set")
				input_level = tgui_input_number(usr, "输入新的输入水平(0-[input_level_max])", "SMES输入电力控制", input_level)
		...
		switch( href_list["output"] )
			...
			if("set")
				output_level = tgui_input_number(usr, "输入新的输出水平(0-[output_level_max])", "SMES输出电力控制", output_level)
...
/obj/machinery/power/smes/proc/ion_act()
	if(!is_ground_level(z))
		return
	if(prob(1)) //explosion
		visible_message(span_warning("\The [src]发出了奇怪的声音!"), null, span_warning("你听到了电子设备的嘶嘶声."))
...
/obj/machinery/power/smes/magical
	...
	desc = "一种高容量超导磁储能(SMES)单元.能神奇地产生电力."
	capacity = 9000000
