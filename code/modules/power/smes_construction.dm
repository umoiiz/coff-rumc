	...
	switch (intensity)
		if (0 to 15)
			...
			if (user_protected && prob(80))
				to_chat(h_user, "小电弧几乎烧到你的手.幸好你戴了手套!")
			else
				to_chat(h_user, "当你触碰[src]时,小电弧闪出火花并烧伤了你的手!")
		...
		if (16 to 35)
			...
			if (user_protected && prob(25))
				to_chat(h_user, "中等电弧闪出火花并几乎烧到你的手.幸好你戴了手套!")
			else
				to_chat(h_user, "当你触碰[src]时,中等电弧闪出火花,严重烧伤了你的手!")
		...
		if (36 to 60)
			...
			s.start()
			if (user_protected)
				to_chat(h_user, "强电弧在你和[src]之间闪出火花,无视你的手套并烧伤了你的手!")
				h_user.adjust_fire_loss(rand(25,60))
				h_user.Unconscious(16 SECONDS)
			else
				to_chat(h_user, "强电弧在你和[src]之间闪出火花,将你击晕了一段时间!")
		...
		if (61 to INFINITY)
			...
			s.set_up(10,1,src)
			s.start()
			to_chat(h_user, "巨大的电弧在你和[src]之间闪出火花.你最后能想到的是\"完蛋了...\"")
			...
			charge = 0
			apcs_overload(5, 25)
			visible_message("警告.输出调节器故障.检测到严重的失控放电.")

			if (prob(50))
				visible_message("危险!磁约束场不稳定!磁约束场即将失效!")
				failing = 1
...
/obj/machinery/power/smes/buildable/attackby(obj/item/I, mob/user, params)
	...
	if((charge > (capacity * 0.01)) && safeties_enabled && !ismultitool(I))
		to_chat(user, span_warning("[src]的安全电路阻止了在充电时进行修改!"))
		return
	...
	if(iscrowbar(I))
		if(terminal)
			to_chat(user, span_warning("你必须先拆开终端!"))
			return
		...
		to_chat(user, span_warning("你已拆解SMES电池!"))
	...
	// Multitool - Toggle the safeties.
		...
		visible_message("[icon2html(src, viewers(src))] <b>[src]</b>发出蜂鸣声: \"警告. 安全电路已: [safeties_enabled ? "re-enabled" : "disabled. Please excercise caution."]\"")
