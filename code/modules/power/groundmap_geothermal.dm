...
/obj/machinery/power/geothermal
	...
	icon_state = "weld"
	desc = "一台位于充满等离子体的钻孔上方的热电发电机.这台已严重损坏.使用喷灯,然后用剪线钳,然后用扳手来修理它."
	anchored = TRUE
	density = TRUE
...
/obj/machinery/power/geothermal/proc/check_failure()
	...
	if(rand(1, 100) < fail_rate) //Oh snap, we failed! Shut it down!
		if(rand(0, 3) == 0)
			visible_message("[icon2html(src, viewers(src))] <span class='notice'><b>[src]</b>疯狂地发出哔哔声,一个保险丝烧断了!使用剪线钳,然后用扳手来修理它.")
			buildstate = GEOTHERMAL_MEDIUM_DAMAGE
			icon_state = "wire"
		else
			visible_message("[icon2html(src, viewers(src))] <span class='notice'><b>[src]</b>疯狂地发出哔哔声,随机零件四处飞溅!使用扳手来修理它.")
...
/obj/machinery/power/geothermal/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	play_attack_sound(1)
	xeno_attacker.visible_message(span_danger("\The [xeno_attacker]劈砍\the [src],撕扯着它的组件!"),
		span_danger("我们开始劈砍\the [src],撕扯着它的组件!"))
...
/obj/machinery/power/geothermal/proc/interact_hand(mob/living/user)
	...
	if(!ishuman(user) && !issilicon(user))
		to_chat(user, span_warning("你不知道如何使用那个."))
		return FALSE
	if(corrupted)
	...
	if(buildstate == GEOTHERMAL_HEAVY_DAMAGE)
		to_chat(usr, span_info("使用喷灯,然后用剪线钳,然后用扳手来修理它."))
		return FALSE
	else if(buildstate == GEOTHERMAL_MEDIUM_DAMAGE)
		to_chat(usr, span_info("使用剪线钳,然后用扳手来修理它."))
		return FALSE
	else if(buildstate == GEOTHERMAL_LIGHT_DAMAGE)
		to_chat(usr, span_info("使用扳手来修理它."))
		return FALSE
	if(is_on)
		visible_message("[icon2html(src, viewers(src))] <span class='warning'><b>[src]</b>轻声发出哔哔声,随着[usr]关闭涡轮机,嗡鸣声停止了.")
		is_on = FALSE
		power_gen_percent = 0
		cur_tick = 0
		icon_state = "off"
		...
		update_minimap_icon()
		return TRUE
	visible_message("[icon2html(src, viewers(src))] <span class='warning'><b>[src]</b>大声发出哔哔声,随着[usr]开启涡轮机,发电机开始加速运转.")
	icon_state = "on10"
	is_on = TRUE
	cur_tick = 0
	start_processing()
...
/obj/machinery/power/geothermal/welder_act(mob/living/user, obj/item/tool/weldingtool/WT)
	if(corrupted)
		if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
			user.visible_message(span_notice("[user]摸索着弄清楚[src]上的树脂触须."),
			span_notice("你摸索着弄清楚[src]上的树脂触须."))
	...
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
		user.visible_message(span_notice("[user]摸索着弄清楚[src]的内部结构."),
		span_notice("你摸索着弄清楚[src]的内部结构."))
...
/obj/machinery/power/geothermal/wirecutter_act(mob/living/user, obj/item/I)
	if(buildstate != GEOTHERMAL_MEDIUM_DAMAGE || is_on)
		return
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
		user.visible_message(span_notice("[user]摸索着弄清楚[src]的线路."),
		span_notice("你摸索着弄清楚[src]的线路."))
		var/fumbling_time = 10 SECONDS - 2 SECONDS * user.skills.getRating(SKILL_ENGINEER)
...
/obj/machinery/power/geothermal/wrench_act(mob/living/user, obj/item/I)
	if(buildstate != GEOTHERMAL_LIGHT_DAMAGE || is_on)
		return
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_ENGI)
		user.visible_message(span_notice("[user]摸索着弄清楚[src]的管道和装甲板."),
		span_notice("你摸索着弄清楚[src]的管道和装甲板."))
		var/fumbling_time = 10 SECONDS - 2 SECONDS * user.skills.getRating(SKILL_ENGINEER)
