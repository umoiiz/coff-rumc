...
/obj/machinery/power/port_gen/pacman/examine(mob/user)
	. = ..()
	. += span_notice("发电机剩余[sheets]单位的[sheet_name]燃料,每个周期产生[DisplayPower(power_gen)].")
	if(anchored)
		. += span_notice("它被固定在地面上.")
	if(in_range(user, src) || isobserver(user))
		. += span_notice("状态显示屏显示:燃料效率提高了<b>[(consumption*100)-100]%</b>.")
...
/obj/machinery/power/port_gen/pacman/attackby(obj/item/O, mob/user, params)
	if(istype(O, sheet_path))
		...
		if(amount < 1)
			...
			return
		to_chat(user, span_notice("你向[src.name]添加了[amount]片."))
		sheets += amount
		addstack.use(amount)
		return
	else if(!active)
