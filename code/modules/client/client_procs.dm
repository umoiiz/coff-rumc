...
/client/Topic(href, href_list, hsrc, hsrc_command)
	...
	if (!holder && stl && href_list["window_id"] != "statbrowser")
		...
		if (topiclimiter[SECOND_COUNT] > stl)
			to_chat(src, span_danger("你之前的操作被忽略,因为你在一秒内执行了太多次"))
			return
...
/client/New(TopicData)
	...
	if(byond_version < REQUIRED_CLIENT_MAJOR || (byond_build && byond_build < REQUIRED_CLIENT_MINOR))
		...
		to_chat(src, span_userdanger("TGMC现在需要第一个稳定版[REQUIRED_CLIENT_MAJOR]构建,请将你的客户端更新到[REQUIRED_CLIENT_MAJOR].[MIN_RECOMMENDED_CLIENT]."))
	...
	if(byond_build < 1555)
		to_chat(src, span_userdanger("你的byond版本可能存在渲染延迟问题,如果你遇到这些问题,建议你将版本更新到Byond 1555以上."))
...
/client/Click(atom/object, atom/location, control, params)
	...
	if(scl && !check_rights(R_ADMIN, FALSE))
		...
		if(clicklimiter[SECOND_COUNT] > scl)
			to_chat(src, span_danger("你之前的点击被忽略,因为你在一秒内执行了太多次"))
			return
