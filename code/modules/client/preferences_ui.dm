...
/datum/preferences/ui_act(action, list/params, datum/tgui/ui)
	...
	switch(action)
		...
		if("name_real")
			...
			if(!newValue)
				tgui_alert(user, "无效的名字.你的名字长度应至少为2个字符,最多为[MAX_NAME_LEN]个字符.只能包含字符A-Z、a-z、-、'和 .", "无效的名字", list("Ok"))
				return
			real_name = newValue
		...
		if("synthetic_type")
			var/choice = tgui_input_list(ui.user, "你想使用哪种类型的合成人?", "合成人类型选择", SYNTH_TYPES)
			if(!choice)
				return
		...
		if("predator_name")
			...
			if(raw_name) // Check to ensure that the user entered text (rather than cancel.)
				...
				else to_chat(user, "无效的名字.你的名字长度应至少为2个字符,最多为[MAX_NAME_LEN]个字符.只能包含字符A-Z、a-z、-、'和 .")

		if("predator_gender")
			predator_gender = predator_gender == MALE ? FEMALE : MALE

		if("predator_age")
			var/new_predator_age = tgui_input_number(user, "选择你的Predator年龄(20到10000):", "角色偏好", 1234, 10000, 20)
		...
		if("predator_caster_material")
			var/new_pred_caster_mat = tgui_input_list(user, "选择你的腕刃材质:", "腕刃材质", PRED_MATERIALS + "retro")
		...
		if("squad_robot_name")
			...
			newValue = reject_bad_name(newValue, TRUE)
			if(!newValue)
				tgui_alert(user, "无效名称. 你的名称长度应至少为2个字符且最多为[MAX_NAME_LEN]个字符. 只能包含字符A-Z, a-z, -, ' 和 .", "无效名称", list("Ok"))
				return
			squad_robot_name = newValue

		if("squad_robot_type")
			var/choice = tgui_input_list(ui.user, "你想使用哪种型号的机器人?", "机器人型号选择", ROBOT_TYPES)
			if(!choice)
				return
		...
		if("loadoutadd")
			...
			if(total_cost <= MAX_GEAR_COST)
				if(!islist(gear))
					gear = list()
				gear += choice
				to_chat(user, span_notice("已为[C.cost]点数添加'[choice]' (剩余[MAX_GEAR_COST - total_cost]点数)."))
			else
				to_chat(user, span_warning("添加'[choice]'将超过[MAX_GEAR_COST]点数的最大配装费用."))
		...
		if("facial_style")
			...
			var/choice = tgui_input_list(ui.user, "你想要哪种胡须造型?", "胡须造型选择", valid_facialhairstyles)
			if(!choice)
				return
			f_style = choice
			update_preview_icon()
		...
		if("bancheck")
			...
			to_chat(user, span_danger("你, 或此电脑或连接的另一位用户 ([ban_details["key"]]) 被禁止游玩[params["role"]].<br>封禁原因为: [ban_details["reason"]]<br>此封禁 (封禁ID #[ban_details["id"]]) 由[ban_details["admin_key"]]于[ban_details["bantime"]]在回合ID [ban_details["round_id"]]期间实施.<br>[expires]"))
