...
/client/verb/middle_mousetoggle()
	...
	to_chat(src, span_notice("选定的特殊能力现在将通过[(prefs.toggles_gameplay & MIDDLESHIFTCLICKING) ? "middle button" : "shift"]点击来激活."))
...
/client/verb/toggle_ghost_hivemind()
	...
	to_chat(src, span_notice("作为幽灵,你将[(prefs.toggles_chat & CHAT_GHOSTHIVEMIND) ? "now see chatter from the Xenomorph Hivemind" : "no longer see chatter from the Xenomorph Hivemind"]."))
...
/client/verb/toggle_radial_medical()
	...
	to_chat(src, span_notice("你将[(prefs.toggles_gameplay & RADIAL_MEDICAL) ? "now" : "no longer"]出于医疗目的使用径向菜单."))
...
/client/verb/toggle_ooc_country_flag()
	...
	to_chat(src, span_notice("你的国旗将[(prefs.show_ooc_country_flag) ? "now" : "no longer"]出现在OOC聊天中你的名字前面."))
...
/client/proc/pick_form()
	...
	to_chat(src, span_notice("作为观察者开始时你将使用[new_form]幽灵形态."))
...
/client/proc/pick_ghost_orbit()
	...
	to_chat(src, span_notice("作为幽灵时你将使用[new_orbit]."))
