...
/obj/docking_port/stationary/marine_dropship/on_crash()
	...
	for(var/i in GLOB.alive_living_list) //knock down mobs
		...
		if(M.buckled)
			...
			to_chat(M, span_warning("你脚下的地板猛烈震动!"))
			shake_camera(M, 10, 1)
...
/mob/living/carbon/xenomorph/proc/hijack()
	...
	if(!SSticker?.mode)
		to_chat(src, span_warning("该能力在此游戏模式下无效."))
	...
	if(!port)
		to_chat(src, span_warning("出了些问题."))
		return
	...
	priority_announce("登陆穿梭机控制受到未知干预. 正在关闭自动驾驶...", "穿梭机故障", type = ANNOUNCEMENT_PRIORITY, color_override = "red", sound = 'sound/AI/dropship_wrong.ogg')
...
/datum/game_mode/proc/can_summon_dropship(mob/user)
	...
	if(SSticker.round_start_time + SHUTTLE_HIJACK_LOCK > world.time)
		to_chat(user, span_warning("现在呼叫还太早. 我们必须等待[DisplayTimeText(SSticker.round_start_time + SHUTTLE_HIJACK_LOCK - world.time, 1)]."))
		return FALSE
	...
	if(is_ground_level(D.z))
		...
		if(locked_sides < 3 && !isdropshiparea(get_area(user)))
			to_chat(user, span_warning("至少有一侧仍然未锁定!"))
			return FALSE
		...
		priority_announce("诺曼底号的锁定协议已受损. 外部干预正在阻止远程控制尝试.", "穿梭机已锁定", type = ANNOUNCEMENT_PRIORITY, color_override = "red", sound = 'sound/AI/dropship_block.ogg')
		return FALSE
	...
	if(length_char(GLOB.alive_human_list) && ((living_player_list[1] / length_char(GLOB.alive_human_list)) > ALIVE_HUMANS_FOR_CALLDOWN))
		to_chat(user, span_warning("地面上还有大量活体猎物. 它们干扰着我们的心灵场. 我们必须先消灭它们才能做到这一点."))
		return FALSE
	return TRUE
...
/obj/machinery/computer/shuttle/marine_dropship/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	#ifndef TESTING
	if(SSticker.round_start_time + SHUTTLE_HIJACK_LOCK > world.time)
		to_chat(xeno_attacker, span_xenowarning("现在做这个还太早!"))
		return
	#endif
	...
	if(shuttle.hijack_state != HIJACK_STATE_CALLED_DOWN && shuttle.hijack_state != HIJACK_STATE_CRASHING) //Process of corrupting the controls
		to_chat(xeno_attacker, span_xenowarning("我们侵蚀了那只鸟的控制系统,解锁了舱门并阻止它起飞."))
	...
	switch(action)
		...
		if("hijack")
			...
			if(!is_ground_level(xeno.loc.z)) // Don't hijack from the shipside
				to_chat(xeno, span_xenowarning("穿梭机已经在飞船上了!"))
				return
			if(SSmonitor.gamestate == SHIPSIDE)
				to_chat(xeno, span_xenowarning("穿梭机已经在飞船上了!"))
				return
			if(shuttle.mode == SHUTTLE_RECHARGING)
				to_chat(xeno, span_xenowarning("那只鸟还在冷却中..."))
				return
			if(shuttle.mode != SHUTTLE_IDLE)
				to_chat(xeno, span_xenowarning("我们现在不能这么做."))
				return
			var/confirm = tgui_alert(usr, "你想劫持那只金属鸟吗?", "劫持那只鸟?", list("Да", "Нет"))
			if(confirm != "Да")
				return
		...
		if("abduct")
			...
			if(groundside_humans > 5)
				to_chat(usr, span_xenowarning("还有猎物可以猎杀!"))
				return
			var/confirm = tgui_alert(usr, "你想夺取那只金属鸟吗?\n 这将结束本轮", "夺取那只鸟?", list( "Да", "Нет"))
			if(confirm != "Да")
				return
			...
			priority_announce("诺曼底号被夺取了! 失去了通往地面的主要途径, 陆战队员们被迫撤退.", title = "诺曼底号被夺取", color_override = "orange")
...
/obj/machinery/computer/shuttle/marine_dropship/proc/do_hijack(obj/docking_port/mobile/marine_dropship/crashing_dropship, obj/docking_port/stationary/marine_dropship/crash_target/crash_target, mob/living/carbon/xenomorph/user)
	...
	priority_announce("检测到诺曼底号未经计划的战斗区域外飞行. 可能遭到劫持.", title = "诺曼底号故障", type = ANNOUNCEMENT_PRIORITY, sound = 'sound/AI/hijack.ogg', color_override = "red")
	to_chat(user, span_danger("[src]中爆发出一阵刺耳的警报! 那些血肉宿主一定知道你能访问它!"))
	...
	switch(SSshuttle.moveShuttleToDock(shuttleId, crash_target, TRUE))
		...
		if(1)
			to_chat(user, span_warning("无效的穿梭机请求. 这不应该发生! 请向管理员报告此事."))
			CRASH("moveShuttleToDock() returned 1.")
		else
			to_chat(user, span_warning("错误. 这不应该发生! 请向管理员报告此事."))
			CRASH("moveShuttleToDock() returned a non-zero-nor-one value.")

/obj/machinery/computer/shuttle/marine_dropship/one
	name = "\improper 'Normandy' flight controls"
	desc = "'诺曼底'号运输机的飞行控制台. 以法国的一个省份命名, 因1944年6月6日著名的诺曼底登陆而闻名, 那是二战中一场血腥但具有决定性意义的胜利, 也是解放法国战役的一部分."
	possible_destinations = "lz1;lz2;normandy"
...
/obj/machinery/computer/shuttle/marine_dropship/two
	...
	desc = "'阿拉莫'号运输机的飞行控制台. 以阿拉莫传教站命名, 那是1836年春季美国德克萨斯州阿拉莫战役的发生地. 守军坚守到最后一刻, 鼓舞了其他德克萨斯人团结在旗帜下."
	icon_state = "console2"
		...
		if(0)
			if(previous_status != SHUTTLE_IDLE)
				...
				for(var/mob/living/silicon/ai/AI AS in GLOB.ai_list)
					if(!AI.client)
						continue
					to_chat(AI, span_info("[src]被远程操控起飞了."))
			return TRUE
		if(1)
			to_chat(usr, span_warning("无效的穿梭机请求."))
			return TRUE
		else
...
/obj/machinery/computer/shuttle/shuttle_control/attack_ghost(mob/dead/observer/user)
	...
	var/input = tgui_input_list(user, "选择传送目的地端口:", "幽灵穿梭机传送", destinations, null, 0)
	if(!input)
		return
...
/obj/machinery/computer/shuttle/shuttle_control/dropship
	...
	desc = "'诺曼底'号运输机的遥控器. 以法国的一个省份命名, 因1944年6月6日著名的诺曼底登陆而闻名, 那是二战中一场血腥但具有决定性意义的胜利, 也是解放法国战役的一部分."
...
/obj/machinery/computer/shuttle/shuttle_control/dropship/two
	...
	desc = "'阿拉莫'号运输机的遥控器. 以阿拉莫传教站命名, 那是1836年春季美国德克萨斯州阿拉莫战役的发生地. 守军坚守到最后一刻, 鼓舞了其他德克萨斯人团结在旗帜下."
...
/obj/machinery/computer/shuttle/shuttle_control/canterbury/Topic(href, href_list)
	...
	if(!length(GLOB.active_nuke_list))
		...
		if(admin_response == "deny")
			...
			priority_announce("撤离尝试被阻止. 正在重启引擎...", "撤离尝试", ANNOUNCEMENT_COMMAND)
			return TRUE
