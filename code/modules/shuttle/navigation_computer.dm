...
/obj/machinery/computer/camera_advanced/shuttle_docker/attack_hand(mob/user)
	if(jammed)
		to_chat(user, span_warning("你只能在控制台上看到一片雪花."))
		return
	if(!shuttle_port && !SSshuttle.getShuttle(shuttle_id))
		to_chat(user,span_warning("警告: 穿梭机连接已断开!"))
		return
	return ..()
	...
	if(landing_clear != SHUTTLE_DOCKER_LANDING_CLEAR)
		switch(landing_clear)
			...
			if(SHUTTLE_DOCKER_BLOCKED_BY_HIDDEN_PORT)
				to_chat(current_user, span_warning("着陆区检测到未知物体. 请指定另一个位置."))
		return
...
/datum/action/innate/camera_jump/shuttle_docker/Activate()
	...
	var/selected = tgui_input_list(usr, "选择要跳跃至的位置", "位置", sortList(L))
