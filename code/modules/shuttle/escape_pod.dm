...
/obj/machinery/computer/shuttle/escape_pod/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(action == "launch")
		...
		if(SSevacuation.evac_time + EVACUATION_MANUAL_DEPARTURE - world.time > 0)
			to_chat(usr, span_warning("逃生舱尚未完全加满燃料!"))
			return

		to_chat(usr, span_userdanger("你猛地一拳砸在发射按钮上!"))
		M.launch(TRUE)
...
/obj/machinery/door/airlock/evacuation/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
