...
/obj/structure/bigDelivery/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/destTagger))
		...
		if(sortTag == O.currTag)
			to_chat(user, span_warning("该包裹已被标记为发往[O.currTag]."))
			return
		...
		switch(tgui_alert(user, "你想更改什么?", "标签", list("Title", "Description", "Cancel")))
			if("Title")
				...
				if(!str)
					...
					return
				user.visible_message("\The [user] 在 \the [src] 上标注了 \a [I], 记下: \"[str]\"",\
				span_notice("你将 \the [src] 标注为: \"[str]\""),\
			...
			if("Description")
				...
				if(!examtext && !nameset)
					...
					examtext = str
				user.visible_message("\The [user] 在 \the [src] 上贴了 \a [I] 标签, 写下: \"[examtext]\"",\
				span_notice("你将 \the [src] 贴上标签: \"[examtext]\""),\
				"You hear someone scribbling a note.")
...
/obj/item/smallDelivery/examine(mob/user)
	...
	if(get_dist(src, user) <= 4)
		if(sortTag)
			to_chat(user, span_notice("它被标注为 \"[sortTag]\""))
		if(examtext)
			to_chat(user, span_notice("它附有一张便条, 上面写着, \"[examtext]\""))
...
/obj/item/smallDelivery/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/destTagger))
		...
		if(sortTag == O.currTag)
			to_chat(user, span_warning("包裹已经标注为送往 [O.currTag]."))
			return

		to_chat(user, span_notice("你已将目的地标注为 [O.currTag]."))
		if(!sortTag)
			sortTag = O.currTag
			update_icon()
		else
...
/obj/item/packageWrap/afterattack(obj/target, mob/user, proximity)
	...
	if(istype(target, /obj/item) && !(istype(target, /obj/item/storage) && !istype(target,/obj/item/storage/box)))
		...
		if(amount > 1)
			...
			span_notice("你包裹了 \the [target], 在 \the [src] 上留下 [amount] 单位纸张."),\
