...
/obj/machinery/disposal/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/storage/bag/trash))
		...
		to_chat(user, span_notice("你将袋子清空倒入[src]."))
...
/obj/machinery/disposal/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(mode == 0) //It's off but still not unscrewed
		...
		to_chat(user, span_notice("你移除了电源连接周围的螺丝."))
		return
	else if(mode == -1)
		mode = 0
		playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
		to_chat(user, span_notice("你安装了电源连接周围的螺丝."))
		return
...
/obj/machinery/disposal/MouseDrop_T(mob/target, mob/user)
	...
	if(target == user)
		visible_message(span_notice("[user]开始爬进处理单元."))
	else
		visible_message(span_warning("[user]开始把[target]塞进处理单元."))
	...
	if(target == user)
		...
		span_notice("你爬进了[src]."))
	else
		user.visible_message(span_danger("[user]把[target]塞进了[src]!"),
		span_warning("你把[target]塞进了[src]!"))
...
/obj/machinery/disposal/proc/go_out(mob/user)
	...
	if(!user.lying_angle)
		...
		span_warning("你从[src]里爬了出来并辨明了方向!"))
		update()
...
/obj/machinery/disposal/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("eject")
			if(mode == -1)
				to_chat(ui.user, span_warning("处理单元的电源已关闭."))
				return
			eject()
			return TRUE
...
/obj/machinery/disposal/proc/eject()
	for(var/atom/movable/AM in src)
		...
		if(isliving(AM))
			var/mob/M = AM
			if(!M.lying_angle)
				M.visible_message(span_warning("[M]突然被推出了[src]!"),
				span_warning("你被推出了[src]并辨明了方向!"))
			if(isliving(M))
				var/mob/living/L = M
				L.Stun(4 SECONDS)
	update()
...
/obj/machinery/disposal/CanAllowThrough(atom/movable/mover, turf/target)
	if(istype(mover, /obj/item) && mover.throwing)
		...
		if(prob(75))
			I.loc = src
			visible_message(span_notice("[I]落入了[src]."))
		else
...
/obj/structure/disposalpipe/welder_act(mob/living/user, obj/item/tool/weldingtool/W)
	...
	if(!W.isOn() || user.loc != uloc || wloc != W.loc)
		to_chat(user, span_warning("在焊接[src]时你必须保持不动."))
		return
	welded()
...
/obj/structure/disposalpipe/tagger/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/destTagger))
		...
		to_chat(user, span_notice("已将标签改为'[sort_tag]'."))
		updatename()
		updatedesc()
...
/obj/structure/disposalpipe/sortjunction/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/destTagger))
		...
		sortType = O.currTag
		playsound(loc, 'sound/machines/twobeep.ogg', 25, 1)
		to_chat(user, span_notice("已将过滤器改为'[sortType]'."))
		updatename()
		updatedesc()
...
/obj/structure/disposalpipe/trunk/welder_act(mob/living/user, obj/item/tool/weldingtool/W)
	...
	sleep(3 SECONDS)
	if(!W.isOn() || user.loc != uloc && wloc != W.loc)
		to_chat(user, span_warning("在焊接管道时你必须保持不动."))
		return
...
/obj/structure/disposaloutlet/screwdriver_act(mob/living/user, obj/item/I)
	...
	if(mode)
		playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
		to_chat(user, span_notice("你拧下了电源连接处的螺丝."))
	else
		playsound(loc, 'sound/items/screwdriver.ogg', 25, 1)
		to_chat(user, span_notice("你拧上了电源连接处的螺丝."))
