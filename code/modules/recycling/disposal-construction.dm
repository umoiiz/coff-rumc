...
/obj/structure/disposalconstruct/attackby(obj/item/I, mob/user)
	...
	var/turf/T = src.loc
	if(T.intact_tile)
		to_chat(user, "只有在移除地板镀层的情况下才能安装[nicetype]。")
		return
	...
	if(iswrench(I))
		if(anchored)
			...
			if(ispipe)
				...
				density = TRUE
			to_chat(user, "你从地板下拆下了[nicetype]。")
		else
			if(ptype>=6 && ptype <= 8) // Disposal or outlet
				if(CP) // There's something there
					if(!istype(CP,/obj/structure/disposalpipe/trunk))
						to_chat(user, "[nicetype]下方需要有一个主干才能工作。")
						return
				...
				if(CP)
					...
					if(istype(CP, /obj/structure/disposalpipe/broken))
						pdir = CP.dir
					if(pdir & dpdir)
						to_chat(user, "该位置已经有一个[nicetype]了。")
						return
			...
			to_chat(user, "你将[nicetype]安装到地板下。")
