/obj/vehicle/sealed/armored/multitile
	...
	desc = "一堵巨大的金属墙,专为最大化消灭异形而设计。在入口处拖拽自己上去以进入内部。"
...
/obj/vehicle/sealed/armored/multitile/attackby(obj/item/I, mob/user, params)
	...
	if(plasteel_stack.get_amount() < 50)
		balloon_alert(user, "你需要至少50块塑钢片才能修理这辆载具!")
		return FALSE
	if(user.skills.getRating(SKILL_ENGINEER) < SKILL_ENGINEER_PLASTEEL)
		user.visible_message(span_notice("[user]笨拙地摸索着如何将塑钢用于[src]。"),
		span_notice("你笨拙地摸索着如何将塑钢用于[src]。"))
		var/fumbling_time = 30 SECONDS - 2 SECONDS * user.skills.getRating(SKILL_ENGINEER)
		if(!do_after(user, fumbling_time, NONE, src, BUSY_ICON_UNSKILLED))
			return FALSE
	balloon_alert_to_viewers("正在应用塑钢加固...")
	...
	if(!plasteel_stack.use(50))
		return FALSE
	unwreck_vehicle()
	user.visible_message(span_notice("[user]成功地用塑钢加固并修理了[src]!"),
	span_notice("你成功地用塑钢加固并修理了[src]!"))
	return TRUE
