...
/datum/action/vehicle/sealed/armored/swap_seat/action_activate(trigger_flags)
	...
	switch(choice)
		...
		if(ARMOR_PASSENGER)
			chassis.balloon_alert(owner, "已进入乘客座位")
...
/datum/action/vehicle/sealed/armored/toggle_lights/action_activate(trigger_flags)
	...
	if(!(chassis.armored_flags & ARMORED_HAS_HEADLIGHTS))
		chassis.balloon_alert(owner, "载具的灯光已损坏!")
		return
	...
	chassis.balloon_alert(owner, "已切换灯光 [chassis.armored_flags & ARMORED_LIGHTS_ON ? "on":"off"]")
...
/datum/action/vehicle/sealed/armored/zoom/action_activate(trigger_flags)
	...
	to_chat(owner, "<font color='[chassis.zoom_mode?"blue":"red"]'>缩放模式已[chassis.zoom_mode?"en":"dis"].</font>")
...
/datum/action/vehicle/sealed/armored/horn/action_activate(trigger_flags)
	...
	chassis.visible_message("[chassis]鸣响了喇叭!")
...
/datum/action/vehicle/sealed/armored/smoke_screen/action_activate(trigger_flags)
	...
	shots_remaining --
	chassis.visible_message("[chassis]释放了烟雾!")
...
/datum/action/vehicle/sealed/armored/tesla/action_activate(trigger_flags)
	...
	chassis.visible_message("[chassis]带电了!")
