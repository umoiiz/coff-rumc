...
/obj/machinery/atmospherics/components/unary/vent_pump/plasmacutter_act(mob/living/user, obj/item/tool/pickaxe/plasmacutter/I)
	if(!welded)
		to_chat(user, span_warning("\The [I]只能切割焊缝!"))
		return FALSE
...
/obj/machinery/atmospherics/components/unary/vent_pump/can_unwrench(mob/user)
	...
	if(. && on && is_operational())
		to_chat(user, span_warning("你无法卸下[src],先将其关闭!"))
		return FALSE
...
/obj/machinery/atmospherics/components/unary/vent_pump/attack_facehugger(mob/living/carbon/xenomorph/facehugger/F, damage_amount = F.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = 0, isrightclick = FALSE)
	...
	if(!welded || !(do_after(F, 3 SECONDS, IGNORE_HELD_ITEM, src, BUSY_ICON_HOSTILE)))
		return
	F.visible_message("[F]愤怒地抓挠[src]!", "我们设法清除了堵塞通风口的杂物", "你听到巨大的刮擦声.")
	welded = FALSE
	update_icon()
	pipe_vision_img = image(src, loc, dir = dir)
	SET_PLANE_EXPLICIT(pipe_vision_img, ABOVE_HUD_PLANE, src)
...
/obj/machinery/atmospherics/components/unary/vent_pump/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	if(xeno_attacker.status_flags & INCORPOREAL)
		return
	if(!welded || !(do_after(xeno_attacker, 2 SECONDS, IGNORE_HELD_ITEM, src, BUSY_ICON_HOSTILE)))
		return
	xeno_attacker.visible_message("[xeno_attacker]愤怒地抓挠[src]!", "我们设法清除了堵塞通风口的杂物", "你听到巨大的刮擦声.")
	welded = FALSE
	update_icon()
	pipe_vision_img = image(src, loc, dir = dir)
	SET_PLANE_EXPLICIT(pipe_vision_img, ABOVE_HUD_PLANE, src)
