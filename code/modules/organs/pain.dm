...
/mob/living/carbon/proc/pain(partname, amount, force, burning = FALSE)
	...
	if(burning)
		switch(amount)
			if(1 to 10)
				msg = span_warning("你的[partname]有灼烧感.")
			if(11 to 90)
				flash_weak_pain()
				msg = span_danger("你的[partname]灼烧得很厉害!")
			if(91 to INFINITY)
				flash_pain()
				msg = span_userdanger("天哪!你的[partname]着火了!")
	else
		switch(amount)
			if(1 to 10)
				msg = span_warning("你的[partname]很疼.")
			if(11 to 90)
				flash_weak_pain()
				msg = span_danger("你的[partname]疼得很厉害.")
			if(91 to INFINITY)
				flash_pain()
				msg = span_userdanger("天哪!你的[partname]疼得要命!")
