...
/datum/limb/l_arm/release_restraints()
	if(!owner.handcuffed)
		return
	owner.visible_message("\The [owner.handcuffed.name]从[owner.name]上掉了下来.", "\The [owner.handcuffed.name]从你身上掉了下来.")
...
/datum/limb/head/proc/disfigure(type = BRUTE)
	if(disfigured)
		return
	if(type == BRUTE)
		owner.visible_message(span_warning("你听到从\the [owner]的脸上传来一阵令人作呕的碎裂声."),	\
		span_danger("你的脸变成了一团无法辨认的烂肉!"),	\
		span_warning("你听到一阵令人作呕的碎裂声."))
	else
		owner.visible_message(span_warning("[owner]的脸融化了,变成一团烂肉!"),	\
		span_danger("你的脸融化了!"),	\
		span_warning("你听到一阵令人作呕的嘶嘶声."))
	disfigured = 1
