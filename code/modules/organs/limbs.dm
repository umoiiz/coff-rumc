...
RU TGMC EDIT */
	...
	if(limb_status & LIMB_SPLINTED)
		if(splint_health <= 0)
			...
			to_chat(owner, span_userdanger("你的[display_name]上的夹板松开了!"))
...
/datum/limb/proc/drop_limb(amputation, delete_limb = FALSE, silent = FALSE)
	...
	if(delete_limb)
		...
		span_warning("你听到肌腱和血肉撕裂的可怕声音!"), 3)
...
/datum/limb/proc/fracture()
	...
	owner.visible_message(\
		span_warning("你听到从[owner]传来的响亮断裂声!"),
		span_userdanger("你感觉你的[display_name]里好像有什么东西碎了!"),
		span_warning("你听到一声令人作呕的断裂声!"))
...
/datum/limb/proc/apply_splints(obj/item/stack/medical/splint/S, applied_health, mob/living/user, mob/living/carbon/human/target)
	...
	target.balloon_alert_to_viewers("正在为[display_name]上夹板...")
	...
	target.balloon_alert_to_viewers("已为[display_name]上夹板")
