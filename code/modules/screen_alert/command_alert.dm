...
/datum/action/innate/message_squad/action_activate()
	...
	var/text = tgui_input_text(human_owner, "消息最大长度 [MAX_COMMAND_MESSAGE_LEN]", "向小队发送消息",  max_length = MAX_COMMAND_MESSAGE_LEN, multiline = TRUE)
	if(!text)
		return
	text = capitalize(text)
	var/filter_result = CAN_BYPASS_FILTER(human_owner) ? null : is_ic_filtered(text)
	if(filter_result)
		to_chat(human_owner, span_warning("该消息包含了一个在IC聊天中被禁止的词语! 请考虑查看服务器规则.\n<span replaceRegex='show_filtered_ic_chat'>\"[text]\"</span>"))
	...
	if(human_owner.assigned_squad)
		...
		announcement_title = "[human_owner.assigned_squad.name]小队公告"
	...
	for(var/mob/mob_receiver in alert_receivers)
		...
		to_chat(mob_receiver, assemble_alert(
			...
			subtitle = "已发送 [human_owner.get_paygrade(0) ? human_owner.get_paygrade(0) : human_owner.job.title] [human_owner.real_name]",
			message = text,
