...
/obj/machinery/botany/proc/finished_task()
	...
	if(eject_disk)
		...
		if(loaded_disk)
			...
			visible_message("[icon2html(src, viewers(src))][src]发出哔声并吐出了[loaded_disk].")
			loaded_disk = null
	stop_processing()
...
/obj/machinery/botany/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/seeds))
		...
		if(length(B.genes) && !disk_needs_genes)
			to_chat(user, "该磁盘已经载入了基因数据.")
			return
		else if(disk_needs_genes)
			to_chat(user, "该磁盘没有载入任何基因数据.")
			return
...
/obj/machinery/botany/screwdriver_act(mob/living/user, obj/item/I)
	. = ..()
	open = !open
	to_chat(user, span_notice("你[open ? "open" : "close"]了维护面板."))
...
/obj/machinery/botany/Topic(href, href_list)
	...
	if(href_list["eject_packet"])
		...
		seed.update_seed()
		visible_message("[icon2html(src, viewers(src))][src]发出哔声并吐出了[seed].")
