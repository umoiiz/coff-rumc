...
/datum/seed/proc/mutate(degree,turf/source_turf)
	...
	source_turf.visible_message(span_notice("\The [display_name]颤抖着!"))
	...
	for(var/i = 0;i<total_mutations;i++)
		switch(rand(0,11))
			if(0) //Plant cancer!
				...
				source_turf.visible_message(span_warning("\The [display_name]迅速枯萎!"))
			if(1)
			...
			if(5)
				...
				if(prob(degree*5))
					...
					if(carnivorous)
						source_turf.visible_message(span_notice("\The [display_name]饥饿地哆嗦着."))
			if(6)
				weed_tolerance = max(0,  min(10,  weed_tolerance       + (rand(-2,2)   * degree)))
			...
			if(8)
				...
				if(prob(degree*5))
					...
					source_turf.visible_message(span_notice("\The [display_name]明显抽搐着, 在托盘里挪动."))
			if(9)
			...
			if(10)
				if(prob(degree*2))
					...
					if(biolum)
						...
						if(prob(degree*2))
							biolum_colour = "#[pick(list("FF0000","FF7F00","FFFF00","00FF00","0000FF","4B0082","8F00FF"))]"
							source_turf.visible_message(span_notice("\The [display_name]的光芒<font color='[biolum_colour]'>变了颜色</font>!"))
					else
						source_turf.visible_message(span_notice("\The [display_name]的光芒变暗了..."))
			if(11)
				if(prob(degree*2))
					flowers = !flowers
					if(flowers)
						source_turf.visible_message(span_notice("\The [display_name]开出了一簇花!"))
						if(prob(degree*2))
							flower_colour = "#[pick(list("FF0000","FF7F00","FFFF00","00FF00","0000FF","4B0082","8F00FF"))]"
						source_turf.visible_message(span_notice("\The [display_name]的花朵<font=[flower_colour]>变了颜色</font>!"))
					else
						source_turf.visible_message(span_notice("\The [display_name]的花朵枯萎凋落了."))
...
/datum/seed/proc/harvest(mob/user,yield_mod,harvest_sample)
	...
	if(!got_product && !harvest_sample)
		to_chat(user, span_warning("你没能收获任何有用的东西."))
	else
		to_chat(user, "你[harvest_sample ? "take a sample" : "harvest"]了[display_name].")
		...
		for(var/i = 0;i<total_yield;i++)
			...
			if(istype(product,/mob/living))
				product.visible_message(span_notice("荚果吐出了[product]!"))
