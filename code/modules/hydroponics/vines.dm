...
/obj/effect/plantsegment/proc/manual_unbuckle(mob/user)
	...
	if(!prob(seed ? min(max(0,100 - seed.potency),100) : 50))
		...
		user.visible_message(
			span_notice("[user.name]在[src]处[text]."),
			span_notice("你在[src]处[text]."),
	...
	if(prisoner != user)
		prisoner.visible_message(
			span_notice("[user.name]将[prisoner.name]从[src]中解放出来."),
			span_notice("[user.name]将你从[src]中解放出来."),
			span_warning("你听到撕裂和扯碎的声音."))
	else
		prisoner.visible_message(
			span_notice("[prisoner.name]挣扎着从[src]中脱身."),
			span_notice("你将[src]从自己身上解开."),
			span_warning("你听到撕裂和扯碎的声音."))
...
/obj/effect/plantsegment/proc/entangle_mob()
	...
	if(!QDELETED(victim) && victim.stat != DEAD && victim.buckled != src) // If mob exists and is not dead or captured.
		...
		to_chat(victim, span_danger("藤蔓在你周围[pick("wind", "tangle", "tighten")]!"))
