...
/datum/round_event/supply_drop/proc/set_target(turf/target_turf)
	...
	priority_announce("联盟货物将在[drop_delay / 600]分钟后抵达。着陆地点 - [target_turf.loc]。", "战术蓝空间扫描仪状态", sound = 'sound/AI/sup_drop.ogg', receivers = (GLOB.alive_human_list_faction[supplying_faction] + GLOB.observer_list))
...
/datum/round_event/supply_drop/proc/alert_hostiles(turf/target_turf, supplying_faction)
	...
	priority_announce("货物[supplying_faction]正在途中。将在[(drop_delay - alert_delay) / 600]分钟后抵达。地点 - [target_turf.loc]。", "战术蓝空间扫描仪状态", sound = 'sound/AI/sup_drop_enemy.ogg', receivers = (humans_to_alert + GLOB.observer_list))


///deploys the actual supply drop
/datum/round_event/supply_drop/proc/drop_supplies(turf/target_turf, faction)
	priority_announce("在[target_turf.loc]发现了来自[faction]的补给。", "战术蓝空间扫描仪状态", sound = 'sound/AI/sup_drop_act.ogg', receivers = (GLOB.alive_human_list + GLOB.observer_list))
