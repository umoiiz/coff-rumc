...
/datum/surgery_step/bone/glue_bone/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	user.visible_message(span_notice("[user]将一些[tool]涂抹到[target]位于[affected.display_name]的骨头上."), \
	span_notice("你用\the [tool]将一些[tool]涂抹到[target]位于[affected.display_name]的骨头上."))
...
/datum/surgery_step/bone/glue_bone/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	user.visible_message(span_warning("[user]的手一滑, 将[tool]抹进了[target]的[affected.display_name]的切口里!") , \
	span_warning("你的手一滑, 将[tool]抹进了[target]的[affected.display_name]的切口里!"))
...
/datum/surgery_step/bone/set_bone/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	if(affected.body_part == HEAD)
		user.visible_message(span_notice("[user]正开始用\the [tool]拼合[target]的头骨.")  , \
		span_notice("你正开始用\the [tool]拼合[target]的头骨."))
	else
	...
	target.balloon_alert_to_viewers("复位中...")
	..()
...
/datum/surgery_step/bone/set_bone/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	if(affected.body_part == HEAD)
		user.visible_message(span_notice("[user]用\the [tool]拼合了[target]的头骨.") , \
		span_notice("你用\the [tool]拼合了[target]的头骨."))
	else
...
/datum/surgery_step/bone/set_bone/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/limb/affected)
	target.balloon_alert_to_viewers("失手!")
	if(affected.body_part == HEAD)
		user.visible_message(span_warning("[user]的手一滑, 用\the [tool]损伤了[target]的脸部!")  , \
		span_warning("你的手一滑, 用\the [tool]损伤了[target]的脸部!"))
		...
		user.visible_message(span_warning("[user]的手一滑, 用\the [tool]损伤了[target]的[affected.display_name]中的骨头!") , \
		span_warning("你的手一滑, 用\the [tool]损伤了[target]的[affected.display_name]中的骨头!"))
