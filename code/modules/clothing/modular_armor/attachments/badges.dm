/obj/item/armor_module/armor/badge
	name = "7E变色龙徽章"
	desc = "7E变色龙徽章采用全新革命性技术, 让你的装备看起来更酷. 它能够变成各种不同的形状(Alt-点击), 改变颜色(用绿色面部涂料攻击), 并能附着到几乎所有服装、头盔、贝雷帽或装甲上!"
...
/obj/item/armor_module/armor/badge/can_attach(obj/item/attaching_to, mob/user)
	...
	if((current_style == "Left Helmet" || current_style == "Right Helmet") && !(istype(attaching_to, /obj/item/clothing/head)))
		allowed = FALSE
	if(!allowed)
		to_chat(user, span_warning("当前选中的样式([current_style])与[attaching_to]不兼容."))
	return allowed
