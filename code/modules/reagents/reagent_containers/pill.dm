...
/obj/item/reagent_containers/pill/attack(mob/M, mob/user, def_zone)
	if(M == user)
		...
		visible_message(span_green("[user]强迫[M]吞下[src]."))
...
/obj/item/reagent_containers/pill/afterattack(obj/target, mob/user, proximity)
	...
	if(target.is_refillable())
		...
		if(target.is_drainable() && !target.reagents.total_volume)
			if(!R || !liquidate)
				to_chat(user, span_warning("[target]是空的! 里面没有东西可以溶解[src]."))
				return
			to_chat(user, span_notice("[target]的液化器在[src]插入后立即将其重新处理."))

		if(!R || !liquidate)
			to_chat(user, span_notice("你把药丸溶解在[target]里."))
		...
		visible_message(span_warning("[user]把一些东西放进了\the [target]里."), null, null, 2)
