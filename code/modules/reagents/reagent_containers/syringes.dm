	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(ismob(target))//Blood!
				...
				if(!target.is_drawable())
					to_chat(user, span_warning("你不能直接从这个物体中移除试剂。"))
					return

				var/trans = target.reagents.trans_to(src, amount_per_transfer_from_this) // transfer from, transfer to - who cares?

				to_chat(user, span_notice("你把注射器装入了[trans]单位的溶液。"))
			if (reagents.holder_full())
				mode=!mode
				update_icon()
		...
		if(SYRINGE_INJECT)
			...
			if(!target.is_injectable() && !ismob(target))
				to_chat(user, span_warning("你不能直接装填这个物体。"))
				return
			...
			if(ismob(target))
				if(target != user)
					...
					if(injection_time != 6 SECONDS)
						user.visible_message(span_danger("[user]正试图注射[target]!"))
					else
					...
					user.visible_message(span_warning("[user]用注射器给[target]注射了!"))
...
/obj/item/reagent_containers/syringe/proc/syringestab(mob/living/carbon/target as mob, mob/living/carbon/user as mob)
	...
	if(ishuman(target))
		...
		if (target != user && !prob(target.modify_by_armor(100, MELEE, penetration, target_zone)))
			visible_message(span_danger("[user]试图用[src]刺入[target]的\the [hit_area], 但攻击被护甲偏转了!"))
			user.temporarilyRemoveItemFromInventory(src)
			qdel(src)
			return

		visible_message(span_danger("[user]用[src]刺入了[target]的\the [hit_area]!"))

		if(affecting.take_damage_limb(3))
			target:UpdateDamageIcon()

	else
		visible_message(span_danger("[user]用[src]刺入了[target]!"))
	...
	switch(mode)
		if(SYRINGE_DRAW)
			...
			if(ismob(target))
				...
				if(!target.is_drawable())
					to_chat(user, span_warning("你无法直接从这个物体中取出试剂."))
					return

				var/trans = target.reagents.trans_to(src, amount_per_transfer_from_this) // transfer from, transfer to - who cares?

				to_chat(user, span_notice("你将注射器装满[trans]单位的溶液."))
			if (reagents.total_volume >= reagents.maximum_volume)
				mode=!mode
				update_icon()
		...
		if(SYRINGE_INJECT)
			...
			if(ismob(target) && target != user)
				user.visible_message(span_danger("[user]正试图用巨型注射器注射[target]!"))
				if(!do_after(user, 30 SECONDS, NONE, target, BUSY_ICON_DANGER, BUSY_ICON_DANGER))
					return
				user.visible_message(span_warning("[user]用巨型注射器注射了[target]!"))
			...
			if(iscarbon(target) && locate(/datum/reagent/blood) in reagents.reagent_list)
				...
				reagents.reaction(target, INJECT)
				trans = reagents.trans_to(target, amount_per_transfer_from_this)
			to_chat(user, span_notice("你注射了[trans]单位的溶液. 注射器现在含有[reagents.total_volume]单位."))
