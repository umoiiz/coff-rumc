...
/obj/item/reagent_containers/dropper/afterattack(obj/target, mob/user , flag)
	...
	if(filled)
		...
		if(ismob(target))
			...
			visible_message(span_danger("[user]正试图往[target]的眼睛里喷东西!"))
			...
			if(ishuman(target))
				...
				if(safe_thing)
					...
					visible_message(span_danger("[user]试图往[target]的眼睛里喷东西,但失败了!"))
			...
			visible_message(span_danger("[user]往[target]的眼睛里喷了东西!"))
		...
		balloon_alert(user, "将滴管装满[trans]单位")
