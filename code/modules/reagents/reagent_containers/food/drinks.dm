	...
	if(iscarbon(M))
		if(M == user)
			...
			if(ishuman(H) && (H.species.species_flags & ROBOTIC_LIMBS))
				...
				return
			M.visible_message(span_warning("[user]试图喂给[M]\the [src]."))
			if(!do_after(user, 3 SECONDS, NONE, M, BUSY_ICON_FRIENDLY))
				return
			M.visible_message(span_warning("[user]喂给[M]\the [src]."))
...
/obj/item/reagent_containers/food/drinks/afterattack(obj/target, mob/user, proximity)
	...
	if(target.is_refillable())
		if(!is_drainable())
			to_chat(user, span_notice("[src]的拉环没有打开!"))
			return
		if(!reagents.total_volume)
		...
		to_chat(user, span_notice("你用[target]中的[trans]单位内容物装满[src]."))
...
/obj/item/reagent_containers/food/drinks/tea
	name = "\improper 紫公爵茶"
	desc = "侮辱紫公爵就是侮辱太空女王!任何正派的绅士都会因为你玷污了这茶而跟你决斗."
	icon_state = "teacup"
...
/obj/item/reagent_containers/food/drinks/dry_ramen
	name = "杯面"
	desc = "只需加入10毫升水,自热!一种让你想起学生时代的美味."
	icon_state = "ramen"
