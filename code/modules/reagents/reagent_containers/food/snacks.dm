...
/obj/item/reagent_containers/food/snacks/attack(mob/M, mob/user, def_zone)
	if(!reagents.total_volume)						//Shouldn't be needed but it checks to see if it has anything left in it.
		balloon_alert(user, "没有剩余的[src]")
		M.dropItemToGround(src)	//so icons update :[
		qdel(src)
		return FALSE

	if(package)
		balloon_alert(user, "不行,包装还在上面")
		return FALSE
	...
	if(iscarbon(M))
		...
		if(M == user)								//If you're eating it yourself
			...
			if(ishuman(H) && (H.species.species_flags & ROBOTIC_LIMBS))
				...
				return
			if(fullness <= 50)
				balloon_alert(user, "饥饿地咀嚼[src]")
			if(fullness > 50 && fullness <= 150)
				balloon_alert(user, "饥饿地吃下[src]")
			if(fullness > 150 && fullness <= 350)
				balloon_alert(user, "咬了一口[src]")
			if(fullness > 350 && fullness <= 550)
				balloon_alert(user, "不情愿地咀嚼[src]")
			if(fullness > 550)
				balloon_alert(user, "无法再吃下[src]")
				return FALSE
		else
			var/mob/living/carbon/H = M
			if(ishuman(H) && (H.species.species_flags & ROBOTIC_LIMBS))
				...
				return
			if(fullness <= 550)
				balloon_alert_to_viewers("试图喂[M]")
			else
				balloon_alert_to_viewers("试图喂[M]但做不到")
				return FALSE
...
/obj/item/reagent_containers/food/snacks/attackby(obj/item/I, mob/user, params)
	...
	if(attacking_utensil.reagents.total_volume > 0)
		balloon_alert(user, "[attacking_utensil]上面已经有东西了!")
		return
...
/obj/item/reagent_containers/food/snacks/sliceable/attackby(obj/item/I, mob/user, params)
	...
	if(I.sharp == IS_NOT_SHARP_ITEM)
		...
		if(length(contents) > max_items)
			...
			return
		balloon_alert(user, "将[I]塞入[src]中.")
		return
...
/obj/item/reagent_containers/food/snacks/egg/throw_impact(atom/hit_atom)
	...
	src.visible_message(span_warning("[src.name]被压碎了."),span_warning("你听到一声拍击声."))
	qdel(src)
...
/obj/item/reagent_containers/food/snacks/lizard_sausage
	name = "\improper 蜥蜴血肠"
	desc = "一根粗糙的干腌血肠,传统上由100%有机来源的蜥蜴制成."
	icon = 'icons/obj/items/food/meat.dmi'
...
/obj/item/reagent_containers/food/snacks/cubancarp
	name = "古巴鲤鱼"
	desc = "一份绝妙的混蛋三明治,烫麻你的舌头然后让它失去知觉!"
	icon = 'icons/obj/items/food/meat.dmi'
...
/obj/item/reagent_containers/food/snacks/monkeycube/attack_self(mob/user)
	if(!package)
		return
	icon_state = "monkeycube"
	balloon_alert_to_viewers("展开[src]")
	package = FALSE
...
/obj/item/reagent_containers/food/snacks/plumphelmetbiscuit
	name = "肥硕头盔菇饼干"
	desc = "这是一块精心制作的肥硕头盔菇饼干. 原料是切得极细的肥硕头盔菇,以及切得很细的矮人小麦面粉."
...
/obj/item/reagent_containers/food/snacks/tossedsalad
	name = "拌沙拉"
	desc = "一道像样的沙拉,基本又简单,夹杂着少许胡萝卜,番茄和苹果碎块. 纯素!"
...
/obj/item/reagent_containers/food/snacks/validsalad
	name = "合格沙拉"
	desc = "这只是一份用可疑'香草'做成的沙拉,配有肉丸和炸土豆片. 没什么可疑的."
	icon = 'icons/obj/items/food/soupsalad.dmi'
...
/obj/item/reagent_containers/food/snacks/cheesewedge
	name = "奶酪楔"
	desc = "一块美味的切达奶酪. 它被切下来的那个奶酪轮肯定就在不远处."
	icon = 'icons/obj/items/food/cheeseandfries.dmi'
...
/obj/item/reagent_containers/food/snacks/baked_cheese_platter
	name = "烤奶酪拼盘"
	desc = "一个烤奶酪轮: 分享的最爱. 通常搭配酥脆的面包片蘸着吃,因为唯一比好奶酪更好的东西就是放在面包上的好奶酪."
	icon = 'icons/obj/items/food/cheeseandfries.dmi'
...
/obj/item/reagent_containers/food/snacks/egg/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/reagent_containers/food/snacks/flour))
		...
		if(!(clr in list("blue", "green", "mime", "orange", "purple", "rainbow", "red", "yellow")))
			to_chat(user, span_notice("鸡蛋拒绝染上这种颜色!"))
			return

		to_chat(user, span_notice("你给\the [src]染上[clr]色"))
		icon_state = "egg-[clr]"
		egg_color = clr
...
/obj/item/reagent_containers/food/snacks/packaged_burrito
	name = "包装墨西哥卷饼"
	desc = "一个硬邦邦的可微波墨西哥卷饼. 没有给出烹饪时间. 由纳米传讯公司包装."
	icon = 'icons/obj/items/food/mre.dmi'
	icon_state = "burrito"
	bitesize = 2
	package = TRUE
...
/obj/item/reagent_containers/food/snacks/packaged_hdogs
	name = "包装热狗"
	desc = "一个软塌塌的,室温的热狗. 没有给出烹饪时间,所以你猜它大概可以直接吃. 由纳米传讯公司包装."
	icon = 'icons/obj/items/food/mre.dmi'
	icon_state = "hot_dogs"
	bitesize = 2
	package = TRUE
...
/obj/item/reagent_containers/food/snacks/upp
	name = "\improper USL口粮"
	desc = "一个密封的、冻干的、压缩的包装,内含一份食物.在USL海盗团乃至火星居民中都很常见,尤其是那些驻扎在偏远殖民地的人.这一份是在2415年包装的."
	icon = 'icons/obj/items/food/mre.dmi'
...
/obj/item/reagent_containers/food/snacks/wrapped/chunk
	name = "大块盒"
	desc = "一条\"<b>大块</b>\"牌巧克力.<i>\"根据联邦法律允许存在的最致密的巧克力.我们依法必须请求你不要把这个钝器用于营养以外的任何用途.\"</i>"
	icon_state = "chunk"
...
/obj/item/reagent_containers/food/snacks/wrapped/barcaridine
	name = "巴卡里丁棒"
	desc = "一条巧克力棒,闻起来像医疗舱.<i>\"巧克力总能帮助止痛.\"</i>"
...
/obj/item/reagent_containers/food/snacks/wrapped/berrybar
	name = "浆果棒"
	desc = "浆果美味棒!这是世界卫生组织为外环殖民地发明的新产品.<i>\"来点浆果,把酒吧赶走!\"</i>"
	icon_state = "berrybar"
...
/obj/item/reagent_containers/food/snacks/wrapped/proteinbar
	name = "蛋白棒"
	desc = "一条巧克力蛋白棒,由在其他配方中找不到归宿的致密废弃食材制成."
...
/obj/item/reagent_containers/food/snacks/lollipop/tramadol
	name = "镇痛棒"
	desc = "你在医疗舱表现良好得到的奖励.可以食用或放入面罩槽."
...
/obj/item/reagent_containers/food/snacks/lollipop/tramadol/combat
	desc = "一种棒棒糖,是在意识到大量陆战队员最终都落得严重的阿片类药物成瘾后设计的,旨在对抗这一问题.到底有没有用,其实取决于你自己.可以食用或放入面罩槽"
...
/obj/item/reagent_containers/food/snacks/lollipop/combat
	name = "医疗棒"
	desc = "一种棒棒糖,通过将糖与双卡因和凯洛坦混合来随时间治愈伤口,试剂消耗速度较慢.可以食用或放入面罩槽"
...
/obj/item/reagent_containers/food/snacks/lollipop/tricord
	name = "三科棒"
	desc = "一种注入了三科德拉嗪的棒棒糖,一种缓慢治愈的试剂.可以食用或放入面罩槽."
...
/obj/item/reagent_containers/food/snacks/donkpocket/berry
	name = "\improper 浆果口袋"
	desc = "一种甜得没完没了的甜甜圈口袋,最初为\"甜点风暴行动\"而创造."
...
/obj/item/reagent_containers/food/snacks/donkpocket/gondola
	name = "\improper 海豚口袋"
	desc = "在配方中使用真正的海豚肉这一选择,至少可以说是颇具争议." //Only a monster would craft this.
...
/obj/item/reagent_containers/food/snacks/donkpocket/warm/gondola
	name = "热海豚口袋"
	desc = "在配方中使用真正的海豚肉这一选择,至少可以说是颇具争议."
	icon_state = "donkpocketgondola"
