	...
	if(ishuman(L))
		...
		if(H.head)
			if(prob(meltprob) && !CHECK_BITFIELD(H.head.resistance_flags, RESIST_ALL))
				if(show_message)
					to_chat(H, span_danger("你的头具融化了,但保护了你免受酸液伤害!"))
				qdel(H.head)
				H.update_inv_head(0)
				H.update_hair(0)
			else if(show_message)
				to_chat(H, span_warning("你的头具保护了你免受酸液伤害."))
			return

		if(H.wear_mask)
			if(prob(meltprob) && !CHECK_BITFIELD(H.wear_mask.resistance_flags, RESIST_ALL))
				if(show_message)
					to_chat(H, span_danger("你的面罩融化了,但保护了你免受酸液伤害!"))
				qdel(H.wear_mask)
				H.update_inv_wear_mask(0)
				H.update_hair(0)
			else if(show_message)
				to_chat(H, span_warning("你的面罩保护了你免受酸液伤害."))
			return
...
/datum/reagent/toxin/nanites/on_mob_add(mob/living/L, metabolism)
	to_chat(L, span_userdanger("你的身体开始扭曲变形! 快离开剃刀风暴!"))
	return ..()
...
/datum/reagent/toxin/nanites/on_mob_life(mob/living/L, metabolism)
	...
	if(volume > 100)
		...
		location.visible_message(span_danger("我靠! 他们刚刚炸成了一团剃刀铁丝网! 天哪!"))
		L.gib()
...
/datum/reagent/toxin/xeno_hemodile/on_mob_life(mob/living/L, metabolism)
	...
	switch(slowdown_multiplier) //Description varies in severity and probability with the multiplier
		if(0 to 1)
			if(prob(10))
				to_chat(L, span_warning("你感到双腿紧绷.") )
		if(2 to 3.9)
			if(prob(20))
				to_chat(L, span_warning("你感到双腿麻木.") )
		if(4 to INFINITY)
			if(prob(30))
				to_chat(L, span_danger("你几乎感觉不到你的双腿了!") )
...
/datum/reagent/toxin/xeno_transvitox/on_mob_life(mob/living/L, metabolism)
	if(prob(10))
		to_chat(L, span_warning("你注意到你的伤口结上了一层恶心的绿色脓痂.") )
...
/datum/reagent/toxin/xeno_ozelomelyn/on_mob_life(mob/living/L, metabolism)
	if(L.get_tox_loss() < 40) // if our toxloss is below 40
		if(prob(15))
			to_chat(L, span_warning("你的血管感觉像水一样,你能感到里面有一种不断加剧的瘙痒感!") )
		return ..()
	if(prob(15))
