...
/obj/machinery/chem_master/Topic(href, href_list)
	...
	if(beaker)
		if (href_list["analyze"])
			...
			useramount = tgui_input_number(usr, "选择转移量.", 30, useramount)
			...
			if(!condi)
				if(loaded_pill_bottle)
					balloon_alert(user, "药瓶已装入")
					return
				var/bottle_label = reject_bad_text(tgui_input_text(user, "标签:", "输入所需的瓶子标签", encode = FALSE))
			...
			if (href_list["createpill_multiple"])
				count = tgui_input_number(usr, "选择要制作的药丸数量.", 16, pillamount, max_pill_count, 0)
				if(!count)
					return
			...
			var/name = reject_bad_text(tgui_input_text(user,"名称:","给你们的药丸命名!","[reagents.get_master_reagent_name()] ([amount_per_pill] units)", encode = FALSE))
			if(!name)
				return
			...
			if(!condi)
				var/name = reject_bad_text(tgui_input_text(user,"名称:","给你们的瓶子命名!",reagents.get_master_reagent_name(), encode = FALSE))
				if(!name)
					return
			...
			if(!condi)
				var/name = reject_bad_text(tgui_input_text(user,"名称:","给你们的自动注射器命名!",reagents.get_master_reagent_name(), encode = FALSE))
				if(!name)
					return
