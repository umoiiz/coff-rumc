...
/obj/machinery/chem_dispenser/ui_act(action, list/params)
	...
	switch(action)
		...
		if("save_recording")
			...
			if(usr.client.prefs.chem_macros[name] && tgui_alert(usr, "\"[name]\"已存在,是否要覆盖它?", null, list("Yes", "No")) == "No")
				return
