/obj/item/explosive/grenade/spawnergrenade/smartdisc
	...
	deliveryamt = 1
	desc = "一件奇怪的外星科技产物. 它有许多锯齿状、旋转的刀刃和奇异的文字."
	...
	if(istype(A, /obj/item/clothing/gloves/yautja))
		...
		if(length(bracer.discs) < bracer.max_disc_cap)
			if(src in bracer.discs)
				to_chat(user, span_warning("你解除了[bracer]和[src]的链接."))
				playsound(user.loc, 'sound/items/pred_bracer.ogg', 75, 1)
				bracer.discs -= src
			else
				bracer.discs += src
				to_chat(user, span_warning("你将[src]链接到[bracer]."))
				playsound(user.loc, 'sound/items/pred_bracer.ogg', 75, 1)
		else
			if(src in bracer.discs)
				to_chat(user, span_warning("你解除了[bracer]和[src]的链接."))
				...
				to_chat(user, span_warning("你的上限是[bracer.max_disc_cap], 在飞盘前解除链接, 以添加另一个."))
...
/mob/living/simple_animal/hostile/smartdisc/Life(seconds_per_tick, times_fired)
	...
	if(lifetime <= 0 || time_idle > 3)
		visible_message("\The [src]停止旋转并旋转着落到地板上.")
		drop_real_disc()
		qdel(src)
		return
...
/mob/living/simple_animal/hostile/smartdisc/AttackingTarget()
	...
	if(isliving(target))
		...
		if(prob(5))
			...
			L.visible_message(span_danger("\The [src]恶狠狠地劈向\the [L]!"))
