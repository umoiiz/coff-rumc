...
/obj/item/clothing/gloves/yautja/pickup(mob/living/user)
	. = ..()
	if(!isyautja(user))
		to_chat(user, span_warning("护腕贴着你的皮肤感觉冰冷, 沉重, 带着一种陌生的、近乎异形的重量感."))
...
/obj/item/clothing/gloves/yautja/proc/delimb_user(mob/living/carbon/human/user)
	...
	to_chat(user, span_notice("装置发出奇怪的噪音并脱落了... 连同你的手臂一起!"))
	...
	if(get_dist(combistick, src) <= 7 && isturf(combistick.loc))
		...
		if(user.put_in_active_hand(combistick))//Try putting it in our active hand, or, if it's full...
			if(!drain_power(user, 70)) //We should only drain power if we actually yank the chain back. Failed attempts can quickly drain the charge away.
				return TRUE
			user.visible_message(span_warning("<b>[user]将[combistick]的链条猛拉回来!</b>"), span_warning("<b>你将[combistick]的链条猛拉回来!</b>"))
			...
			to_chat(user, span_warning("你需要一只空闲的手才能这样做!</b>"))
...
/obj/item/clothing/gloves/yautja/proc/call_disc_internal(mob/living/user, forced = FALSE)
	...
	if(disc_timer)
		to_chat(user, span_warning("你的护腕需要一些时间先恢复."))
		return FALSE
	...
	for(var/obj/item/explosive/grenade/spawnergrenade/smartdisc/disc in discs)
		if(disc.spawned_item)
			if(get_dist(disc.spawned_item, src) <= 7)
				to_chat(user, span_warning("[disc.spawned_item]向你跳了回来!"))
...
/obj/item/clothing/gloves/yautja/proc/translate_internal(mob/living/carbon/human/user, forced = FALSE)
	...
	if(user.client.prefs.muted & MUTE_IC)
		to_chat(user, span_danger("你无法翻译 (被禁言)."))
		return
...
/obj/item/clothing/gloves/yautja/proc/injectors_internal(mob/living/user, forced = FALSE, power_to_drain = 1000)
	...
	if(inject_timer)
		to_chat(user, span_warning("你最近激活了稳定水晶. 请耐心等待."))
		return FALSE
	...
	to_chat(user, span_notice("你听到一声微弱的嘶嘶声,一个水晶注射器落入你的手中."))
...
/obj/item/clothing/gloves/yautja/proc/injectors_ready()
	if(ismob(loc))
		to_chat(loc, span_notice("你的护腕发出微弱的哔哔声,通知你新的稳定水晶已准备好制造."))
	inject_timer = FALSE
	owner.update_action_buttons()
...
/obj/item/clothing/gloves/yautja/proc/healing_capsule_internal(mob/living/user, forced = FALSE)
	...
	if(healing_capsule_timer)
		to_chat(usr, span_warning("你的护腕仍在生成新的治疗胶囊!"))
		return FALSE
	...
	to_chat(user, span_notice("你感觉到你的护腕在翻腾,弹出了一个治疗胶囊."))
...
/obj/item/clothing/gloves/yautja/proc/healing_capsule_ready()
	if(ismob(loc))
		to_chat(loc, span_notice("你的护腕发出微弱的哔哔声,通知你新的治疗胶囊已准备好制造."))
...
/obj/item/clothing/gloves/yautja/proc/caster_internal(mob/living/carbon/human/user, forced = FALSE, power_to_drain = 50)
	...
	if(caster_deployed)
		...
		to_chat(user, span_notice("你激活了你的等离子投射器. 它处于[caster.mode]模式."))
...
/obj/item/clothing/gloves/yautja/proc/cloaker_internal(mob/living/user, forced = FALSE)
	...
	if(cloaked) //Turn it off.
		...
		M.visible_message(span_warning("[M]消失得无影无踪!"), span_notice("你现在对普通探测不可见."))
...
/obj/item/clothing/gloves/yautja/proc/wrapper_fizzle_camouflage()
	...
	wearer.visible_message(span_danger("[wearer]的隐形消失了!"), span_danger("你的隐形消失了!"))
...
/obj/item/clothing/gloves/yautja/proc/track_gear_internal(mob/user, forced = FALSE)
	...
	if(dead_on_planet || dead_on_almayer || dead_low_orbit)
		output = TRUE
		to_chat(M, span_notice("你的护腕显示出已故Yautja生物特征的读数[dead_on_planet ? ", <b>[dead_on_planet]</b> in the hunting grounds" : ""][dead_on_almayer ? ", <b>[dead_on_almayer]</b> in orbit" : ""][dead_low_orbit ? ", <b>[dead_low_orbit]</b> in low orbit" : ""]."))
	if(gear_on_planet || gear_on_almayer || gear_low_orbit)
		output = TRUE
		to_chat(M, span_notice("你的护腕显示出Yautja科技特征的读数[gear_on_planet ? ", <b>[gear_on_planet]</b> in the hunting grounds" : ""][gear_on_almayer ? ", <b>[gear_on_almayer]</b> in orbit" : ""][gear_low_orbit ? ", <b>[gear_low_orbit]</b> in low orbit" : ""]."))
	if(closest < 900)
		output = TRUE
		var/areaName = get_area_name(areaLoc)
		if(closest == 0)
			to_chat(M, span_notice("你正位于最近特征的正上方."))
		else
			to_chat(M, span_notice("最近的特征在<b>[areaName]</b><b>[dir2text(direction)]</b>[closest > 10 ? "approximately <b>[round(closest, 10)]</b>" : "<b>[closest]</b>"]步处."))
	if(!output)
...
/obj/item/clothing/gloves/yautja/proc/change_explosion_type()
	if(explosion_type == SD_TYPE_SMALL && exploding)
		to_chat(usr, span_warning("你为什么要这样做?"))
		return
...
/obj/item/clothing/gloves/yautja/proc/activate_suicide_internal(mob/user, forced = FALSE)
	...
	if(cloaked)
		to_chat(M, span_warning("隐形时不行. 这可能会干扰序列."))
		return
	if(M.stat == DEAD)
		to_chat(M, span_warning("现在做这个太晚了!"))
		return
	if(M.health < -50)
		to_chat(M, span_warning("当你陷入昏迷时,你未能在倒下前激活自毁装置."))
		return
	if(M.stat)
	...
	if(exploding)
		if(forced || alert("Are you sure you want to stop the countdown?","Bracers", "Yes", "No") == "Yes")
			...
			if(M.stat)
				...
				return
			exploding = FALSE
			to_chat(M, span_notice("你的护腕停止了哔哔声."))
	...
	if(forced || alert("Detonate the bracers? Are you sure?\n\nNote: If you activate SD for any non-accidental reason during or after a fight, you commit to the SD. By initially activating the SD, you have accepted your impending death to preserve any lost honor.","Explosive Bracers", "Yes", "No") == "Yes")
		...
		if(exploding)
			return
		to_chat(M, span_danger("你设置了计时器. 愿你前往伟大狩猎场的旅程迅速."))
		var/area/A = get_area(M)
		var/turf/T = get_turf(M)
...
/obj/item/clothing/gloves/yautja/proc/drain_power(mob/living/carbon/human/human, amount)
	if(!human)
		return FALSE
	if(charge < amount)
		to_chat(human, span_warning("你的护腕能量不足. 它们仅剩<b>[charge]/[charge_max]</b>,需要<B>[amount]</b>."))
		return FALSE
...
/obj/item/clothing/gloves/yautja/proc/shock_user(mob/living/carbon/human/M)
	if(!HAS_TRAIT(M, TRAIT_YAUTJA_TECH) && !M.hunter_data.thralled)
		...
		s.set_up(2, 1, src)
		s.start()
		M.visible_message(span_warning("[src]发出哔哔声并向[M]的身体发送电击!"))
...
/obj/item/clothing/gloves/yautja/examine(mob/user)
	. = ..()
	. += span_notice("他们当前有<b>[charge]/[charge_max]</b>电量.")
...
/obj/item/clothing/gloves/yautja/proc/buy_gear(mob/living/carbon/human/wearer)
	if(wearer.gloves != src)
		to_chat(wearer, span_warning("你需要佩戴你的仆从护腕才能这样做."))
		return
...
/obj/item/clothing/gloves/yautja/hunter
	name = "氏族护腕"
	desc = "一套极其复杂但操作简单的装甲护腕,由Yautja佩戴. 它有许多功能,激活它们以使用其中一些."
...
/obj/item/clothing/gloves/yautja/hunter/emp_act(severity)
	...
	if(ishuman(loc))
		...
		if(wearer.gloves == src)
			wearer.visible_message(span_danger("你听到一声嘶嘶声和噼啪声!"), span_danger("你的护腕发出嘶嘶声和火花!"), span_danger("你听到一声嘶嘶声和噼啪声!"))
			if(cloaked)
				decloak(wearer)
		else
			var/turf/our_turf = get_turf(src)
			our_turf.visible_message(span_danger("你听到一声嘶嘶声和噼啪声!"), span_danger("你听到一声嘶嘶声和噼啪声!"))
...
/obj/item/clothing/gloves/yautja/hunter/proc/check_auto_targets_text(mob/user, forced = FALSE)
	...
	if(length(user.hunter_data.targets))
		for(var/datum/huntdata/data in user.hunter_data.targets)
			. += span_warning("[data.owner.real_name]位于[get_area_name(data.owner)]并有[data.owner.life_kills_total + data.owner.life_value + 3]荣誉\n")
	else
		. += span_notice("无")
	to_chat(user, .)
...
/obj/item/clothing/gloves/yautja/hunter/decloak(mob/user, forced)
	...
	user.visible_message(span_warning("[user]闪烁着现形了!"), span_warning("你的隐形装置已停用."))
...
/obj/item/clothing/gloves/yautja/hunter/proc/add_tracked_item_internal(mob/living/user, forced = FALSE)
	...
	if(untracked_item in GLOB.tracked_yautja_gear)
		to_chat(user, span_warning("\The [untracked_item]已在追踪中."))
		return FALSE
	untracked_item.AddElement(/datum/element/yautja_tracked_item)
	to_chat(user, span_notice("你将\the <b>[untracked_item]</b>添加到追踪系统中."))
	playsound(user.loc, 'sound/items/pred_bracer.ogg', 75, 1)
	return TRUE
...
/obj/item/clothing/gloves/yautja/hunter/verb/bracername()
	...
	to_chat(usr, span_notice("\The [src]将[name_active ? "now" : "no longer"]在同族Yautja检查你时显示你的名字."))
...
/obj/item/clothing/gloves/yautja/hunter/verb/toggle_lock()
	...
	if(usr.stat)
		to_chat(usr, span_warning("你现在不能那么做..."))
		return FALSE
...
/obj/item/clothing/gloves/yautja/proc/attempt_toggle_lock(mob/user, force_lock)
	...
	if(isyautja(victim) && !(victim.stat == DEAD))
		to_chat(user, span_warning("你不能解锁活着的猎人的护腕!"))
		return FALSE

	if(!istype(bracer))
		to_chat(user, span_warning("<b>这个[victim.species]没有佩戴护腕.</b>"))
		return FALSE
	...
	if(user.get_active_held_item() == held_mob && victim && victim.gloves == bracer)
		user.visible_message(span_warning("[user]在[victim]的腕部护腕上按了几个按钮."), span_danger("你解开了护腕."))
...
/obj/item/clothing/gloves/yautja/proc/toggle_lock_internal(mob/wearer, force_lock)
	if(HAS_TRAIT(src, TRAIT_NODROP) && !force_lock)
		...
		if(!isyautja(wearer))
			to_chat(wearer, span_warning("护腕发出悦耳的哔哔声,松开了对你前臂的抓握."))
		else
			to_chat(wearer, span_warning("随着一声愤怒的刺耳鸣响,护腕松开了你的前臂."))
		playsound(src, 'sound/items/air_release.ogg', 15, 1)
		return TRUE

	ADD_TRAIT(src, TRAIT_NODROP, TRAIT_GENERIC)
	if(isyautja(wearer))
		to_chat(wearer, span_warning("护腕牢牢扣住你的前臂,并以舒适而熟悉的方式哔哔作响."))
	else
		to_chat(wearer, span_warning("护腕痛苦地扣紧你的前臂,并愤怒地哔哔作响.它取不下来!"))
	playsound(src, 'sound/machines/click.ogg', 15, 1)
	return TRUE
