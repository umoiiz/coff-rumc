...
/obj/item/stack/yautja_rope/attack(mob/living/mob_victim, mob/living/carbon/human/user)
	...
	if(!HAS_TRAIT(user, TRAIT_SUPER_STRONG))
		to_chat(user, span_warning("你力气不够大,无法用绳子把[mob_victim]吊起来.而且,这也有点太变态了."))
		return TRUE
	...
	user.visible_message(span_notice("[user]开始把\his 绳子固定在天花板上..."),
		span_notice("你开始把绳子固定在天花板上..."))
	...
	if(do_after(user, 4 SECONDS, NONE, victim, BUSY_ICON_HOSTILE))
		...
		if(rarea.ceiling == CEILING_NONE)
			to_chat(user, span_warning("没有天花板可以把他们吊起来!"))
			return TRUE
...
/mob/living/carbon/human/proc/cut_down(mob/living/carbon/human/target, mob/living/user, obj/item/source)
	...
	if(user)
		if(user.a_intent != INTENT_HELP)
			return
		user.visible_message(span_warning("[user]用\the [source]割断了[src]."), span_warning("你用\the [source]割断了[src]."))
		user.do_attack_animation(src)
		playsound(src, 'sound/effects/vegetation_hit.ogg', 25, TRUE)
	else
		visible_message(span_danger("[src]的尸体从吊绳上掉落下来!"))
