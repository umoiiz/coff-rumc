...
/obj/item/clothing/suit/armor/yautja
	name = "远古异星护甲"
	desc = "由奇异合金制成的远古护甲.它带着异星的沉重感,触感冰冷."
...
/obj/item/clothing/shoes/marine/yautja
	name = "远古异星护胫"
	desc = "由碎布和奇异合金制成的护胫.它们带着异星的沉重感,触感冰冷."
...
/obj/item/clothing/shoes/marine/yautja/hunter
	name = "部落护胫"
	desc = "一双带有装甲且完美平衡的靴子. 非常适合在丛林中奔跑."
...
/obj/item/clothing/under/chainshirt
	name = "远古异形网衣"
	desc = "一种以背心形式存在的奇异合金编织物. 它摸起来冰冷, 带着异形的重量感."
...
/obj/item/radio/headset/yautja
	name = "\improper 通讯器"
	desc = "一种奇异的Yautja装置, 用于将Yautja的声音投射给其族群中的其他成员. 功能类似于标准的人类无线电."
...
/obj/item/storage/belt/yautja
	name = "狩猎袋"
	desc = "一个系在腰间的Yautja狩猎袋, 由厚实的鞣制兽皮制成. 能够容纳各种装置和工具, 并用于运输战利品."
...
/obj/item/yautja_teleporter/attack_self(mob/user)
	...
	user.visible_message(span_info("[user]开始变得闪烁不定..."))
...
/obj/item/scalp/Initialize(mapload, mob/living/carbon/human/scalpee, mob/living/carbon/human/user)
	...
	if(scalpee.hunter_data.hunter == user) //You don't get your name on it unless you hunted them yourself.
		...
		switch(worth)
			if(-1)
				true_desc += span_blue("\n[user.real_name]承担了将[them]追捕到底的不愉快任务.")
			if(0) //You hunted someone with no kills for no real reason.
				true_desc += span_blue("\nAn对于一个真正早熟的孩子来说, 是光荣的第一个战利品. [user.real_name]的父母一定非常骄傲.")
			if(1)
				true_desc += span_blue("\nThis战利品由[user.real_name]在一次成功的狩猎后取得.")
			if(2)
				true_desc += span_blue("\nThis精美的战利品由[user.real_name]在一次成功的狩猎后取得.")
...
/obj/item/scalp/examine(mob/user)
	...
	if(isyautja(user) || isobserver(user))
		...
		. += span_warning("收集头皮本应是个<i>玩笑</i>. 难道真有人到处干这种破事? 接下来是什么, 一串割下的耳朵项链? 老天爷啊.")
...
/obj/item/weapon/sword/ceremonial
	name = "仪式剑"
	desc = "一把世代相传的精美仪式剑. 尽管如此, 它被保养得非常好, 状态极佳."
...
/obj/item/hunting_trap/verb/configure_trap()
	...
	if(!HAS_TRAIT(H, TRAIT_YAUTJA_TECH))
		to_chat(H, span_warning("你不知道如何配置陷阱."))
		return
	var/range = tgui_input_list(H, "你想将狩猎陷阱设置为哪个范围?", "狩猎陷阱范围", list(2, 3, 4, 5, 6, 7))
	if(isnull(range))
		return
	tether_range = range
...
/obj/item/clothing/shoes/yautja_flavor
	name = "异形石护胫"
	desc = "一双带有装甲且完美平衡的靴子. 非常适合在水泥地上奔跑, 因为它们极其沉重."
...
/obj/item/reagent_containers/glass/rag/polishing_rag/afterattack(obj/potential_limb, mob/user, proximity_flag, click_parameters)
	...
	if(current_limb.polished)
		to_chat(user, span_notice("这个肢体已经抛光过了."))
		return ..()
	...
	if(!do_after(user, 5 SECONDS, NONE, current_limb, BUSY_ICON_HOSTILE, BUSY_ICON_HOSTILE))
		to_chat(user, span_notice("你停止了抛光[current_limb]."))
		return
...
/obj/item/armor_module/limb/skeleton/examine(mob/living/user)
	...
	if(HAS_TRAIT(user, TRAIT_YAUTJA_TECH))
		if(polished)
			...
			. += span_notice("[src]还是脏的.")

/obj/item/storage/belt/utility/pred
	name = "\improper 亚乌查工具腰带"
	desc = "一条带有各种卡扣的模块化腰带. 这个版本没有任何狩猎功能, 通常被工程师用来运送重要工具."
	icon = 'icons/obj/hunter/pred_gear.dmi'
...
/obj/item/tool/wrench/yautja
	name = "亚乌查扳手"
	desc = "一把有许多常见用途的扳手. 由某种奇异的异星骨骼制成."
	icon = 'icons/obj/hunter/pred_gear.dmi'
	icon_state = "wrench"
...
/obj/item/tool/wirecutters/yautja
	name = "亚乌查剪线钳"
	desc = "用来剪断电线, 也能剪断血肉. 由某种锋利如剃刀的动物牙齿制成."
	icon = 'icons/obj/hunter/pred_gear.dmi'
...
/obj/item/weapon/sword/machete/arnold
	name = "\improper 达奇的砍刀"
	desc = "由一位长老在年轻时狩猎的日子里赢得. 任何人都不允许触碰它."
