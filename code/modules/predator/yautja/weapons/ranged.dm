...
/obj/item/weapon/gun/energy/yautja/spike
	name = "尖刺发射器"
	desc = "一个新月形的紧凑Yautja装置.它能快速发射具有破坏性的尖刺并自动充能."
	icon = 'icons/obj/hunter/pred_gear.dmi'
...
/obj/item/weapon/gun/energy/yautja/spike/examine(mob/user)
	if(isyautja(user))
		. = ..()
		. += span_notice("它当前有<b>[rounds]/[max_rounds]</b>根尖刺.")
	else
		. = list()
		. += span_notice("看起来像某种...机械甜甜圈.")
...
/obj/item/weapon/gun/energy/yautja/plasmarifle
	name = "等离子步枪"
	desc = "一把长管重型等离子武器.用于战斗,而非狩猎.内置电池,可发射几乎无限量的射击.配备内部陀螺稳定器,使操作者可以在需要时单手射击"
...
/obj/item/weapon/gun/energy/yautja/plasmarifle/process()
	if(rounds < max_rounds)
		rounds++
		if(rounds == max_rounds)
			if(ismob(loc)) to_chat(loc, span_notice("[src]在达到最大充能时发出嗡鸣声."))
		update_icon()

/obj/item/weapon/gun/energy/yautja/plasmarifle/examine(mob/user)
	if(isyautja(user))
		. = ..()
		. += span_notice("它当前有<b>[rounds]/[max_rounds]</b>充能.")
	else
		. = list()
		. += span_notice("这东西看起来像某种外星步枪.奇怪.")
...
/obj/item/weapon/gun/energy/yautja/plasmapistol
	name = "等离子手枪"
	desc = "一把能够快速射击的等离子手枪.它有一个内置电池.可用于点火,无论是火盆还是人."
...
/obj/item/weapon/gun/energy/yautja/plasma_caster/proc/laser_on(atom/target, mob/user)
	if(user?.client)
		...
		to_chat(user, span_notice("<b>你激活目标标记并仔细瞄准.</b>"))
