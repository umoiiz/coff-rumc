...
/obj/item/clothing/gloves/yautja/hunter/verb/link_bracer()
	...
	if(linked_bracer)
		to_chat(user, span_yautjabold("[icon2html(src)] \The <b>[src]</b>发出哔声: 链接已建立!"))
		return
	...
	if(!T)
		to_chat(user, span_warning("你没有可链接的奴仆!"))
		return
		...
		to_chat(user, span_yautjabold("[icon2html(src)] \The <b>[src]</b>发出哔声: 你的护腕现已链接到你的奴仆."))
		if(notification_sound)
			playsound(loc, 'sound/items/pred_bracer.ogg', 75, 1)

		to_chat(T, span_warning("\The [thrall_gloves]伴随着一声清脆的咔嗒声锁在你的手腕上."))
		to_chat(T, span_yautjabold("[icon2html(thrall_gloves)] \The <b>[thrall_gloves]</b>发出哔声: 你的主人已将他们的护腕链接到你的."))
...
/obj/item/clothing/gloves/yautja/verb/bracer_message()
	...
	if(!istype(receiver))
		to_chat(messenger, span_warning("你没有人可以发送消息!"))
		return
	...
	to_chat(receiver, span_yautjabold("\The <b>[receiver_gloves]</b>发出哔声, 来自你的[messenger_title]的消息: [message]"))
	to_chat(messenger, span_yautjabold("\The <b>[src]</b>发出哔声: 你已将'[message]'发送给你的[receiver_title]."))
