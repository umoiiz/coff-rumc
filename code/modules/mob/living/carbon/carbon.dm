...
/mob/living/carbon/slip(slip_source_name, stun_time, paralyze_time, run_only, override_noslip, slide_steps, slip_xeno)
	...
	stop_pulling()
	visible_message(span_warning("[src]滑倒在\the [slip_source_name]上!"), span_warning("你滑倒在\the [slip_source_name]上!"))
...
/mob/living/carbon/examine(mob/user)
	...
	if(isyautja(user))
		...
		if(hunter_data.automatic_target)
			. += span_red("[src]被[hunter_data.targeted.real_name]标记为目标")
		if(hunter_data.hunted)
			. += span_orange("[src]正在被[hunter_data.hunter.real_name]猎杀.")
		...
		if(hunter_data.thralled)
			. += span_green("[src]被[hunter_data.thralled_set.real_name]因'[hunter_data.thralled_reason]'奴役了.")
