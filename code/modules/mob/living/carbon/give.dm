...
/mob/living/carbon/proc/do_give()
	if(stat != CONSCIOUS)
		return
	if(!hand && r_hand == null)
		to_chat(usr, span_warning("你右手没有任何东西可以给予."))
		return
	if(hand && l_hand == null)
		to_chat(usr, span_warning("你左手没有任何东西可以给予."))
		return
	...
	if(!Adjacent(to_give_to))
		to_chat(src, span_warning("给予物品时你需要保持在可触及的距离内."))
	...
	if(to_give_to.r_hand != null && to_give_to.l_hand != null)
		...
		to_chat(to_give_to, span_warning("你的手已经满了."))
		return
	if(!drop_held_item())
		return
	if(!to_give_to.put_in_hands(item))
		return
	visible_message(span_notice("[src]把[item]递给[to_give_to]."),
	span_notice("你把[item]递给[to_give_to]."), null, 4)
