...
/mob/living/carbon/human/examine(mob/user)
	...
	if(stat)
		...
		if(ishuman(user) && !user.stat && Adjacent(user))
			user.visible_message("<b>[user]</b>检查了[src]的脉搏.", "你检查了[src]的脉搏.", null, 4)
	...
	if(isyautja(user))
		...
		if(istype(bracers) && bracers.name_active)
			msg += span_blue("他们的护腕显示其身份为<b>[real_name]</b>.")
		msg += span_blue("[src]带有[life_kills_total]击败猎物的气味.")
		if(hunter_data.hunted)
			msg += span_orange("[src]正被[hunter_data.hunter.real_name]猎杀.")

		if(hunter_data.dishonored)
			msg += span_red("[src]因'[hunter_data.dishonored_reason]'被标记为不光彩.")
		else if(hunter_data.honored)
			msg += span_green("[src]因'[hunter_data.honored_reason]'获得了荣誉.")
...
/mob/living/carbon/human/proc/take_pulse(mob/user)
	...
	if(pulse_taken == PULSE_NONE)
		...
		to_chat(user, span_deadsay("[p_their(TRUE)]的脉搏为[pulse_taken]."))
