...
/mob/living/carbon/human/attacked_by(obj/item/I, mob/living/user, def_zone)
	...
	switch(percentage_penetration)
		if(-INFINITY to 0)
			visible_message(span_danger("[user]用[user.p_their()] [I.name][attack_verb][src]的[hit_area], 但攻击被[p_their()]护甲挡开了!"),\
	...
	visible_message(span_danger("[user]用[user.p_their()] [I.name][attack_verb][src]的[hit_area]![armor_verb]"),\
...
/mob/living/carbon/human/screech_act(mob/living/carbon/xenomorph/queen/Q, screech_range = WORLD_VIEW, within_sight = TRUE)
	...
	to_chat(src, span_danger("一声刺耳的嘶吼撕裂你的脑海, 让你的世界剧烈震荡!"))
	Stun(stun_duration)
...
/mob/living/carbon/human/attackby(obj/item/I, mob/living/user, params)
	...
	if(!HAS_TRAIT(src, TRAIT_UNDEFIBBABLE))
		to_chat(user, span_warning("你无法下定决心摧毁[src]的心脏, 因为[p_they()]仍可被救回!"))
		return
	to_chat(user, span_notice("你开始摘除[src]的心脏, 阻止[p_them()]再次起身!"))
	...
	visible_message(span_notice("[user]扯下了[src]的心脏!"), span_notice("你扯下了[src]的心脏!"))
