	...
	switch(human_user.a_intent)
		if(INTENT_HELP)
			...
			if(HAS_TRAIT(src, TRAIT_UNDEFIBBABLE))
				to_chat(human_user, span_boldnotice("帮不了这个了. 尸体已经凉了."))
				return FALSE
			...
			human_user.visible_message(span_danger("[human_user]正在对[src]进行心肺复苏!"), null, null, 4)
			...
			if(health > get_death_threshold() && health < get_crit_threshold())
				...
				update_health()
				visible_message(span_warning("[human_user]对[src]进行心肺复苏!"),
					span_boldnotice("你感到一股新鲜空气进入你的肺部. 感觉很好."),
					vision_distance = 3)
				...
				dead_ticks -= 5
				visible_message(span_warning("[human_user]对[src]进行心肺复苏!"), vision_distance = 3)
		...
		if(INTENT_DISARM)
			...
			if(randn <= 25)
				apply_effect(modify_by_armor(6 SECONDS, MELEE, def_zone = target_zone), EFFECT_PARALYZE)
				playsound(loc, 'sound/weapons/thudswoosh.ogg', 25, 1, 7)
				visible_message(span_danger("[human_user]推倒了[src]!"), null, null, 5)
			...
			if(randn <= 60)
				...
				if(pulling)
					visible_message(span_danger("[human_user]挣脱了[src]对[pulling]的抓握!"), null, null, 5)
					stop_pulling()
				else
					drop_held_item()
...
/mob/living/carbon/human/help_shake_act(mob/living/carbon/M)
	if(src == M)
		if(holo_card_color) //if we have a triage holocard printed on us, we remove it.
			...
			visible_message(span_notice("[src]摘下了[p_them()]自己的全息卡."),
				span_notice("你摘下了自己的全息卡."), null, 3)
			return
