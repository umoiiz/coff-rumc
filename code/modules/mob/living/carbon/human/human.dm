...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if(href_list["squadfireteam"])
		...
		if(!(assigned_squad == H.assigned_squad)) //still same squad
			return
		var/newfireteam = tgui_input_list(usr, "将这名陆战队员分配到火力小组.", "火力小组分配", list("None", "Fire Team 1", "Fire Team 2", "Fire Team 3"))
	...
	if(href_list["medical"])
		...
		for(var/datum/data/record/general_record in GLOB.datacore.general)
			if(!(general_record.fields["name"] == perpname) || !(general_record.fields["id"] == general_record.fields["id"]))
				continue
			var/setmedical = tgui_input_list(usr, "为这个人指定新的医疗状态.", "医疗HUD", list("*SSD*", "*Deceased*", "Physically Unfit", "Active", "Disabled"))
			if(!setmedical)
				return
	...
	if(href_list["medholocard"])
		if(!species?.count_human)
			to_chat(usr, span_warning("分诊全息卡只对有机人形实体有效."))
			return
		var/newcolor = tgui_input_list(usr, "选择一张分诊全息卡添加到病人身上:", "分诊全息卡", list("black", "red", "orange", "none"))
		if(!newcolor)
			return
	...
	if(href_list["scanreport"])
		if(!hasHUD(usr,"medical"))
			return
		if(!ishuman(src))
			to_chat(usr, span_warning("这只对人形生物有效."))
			return
		if(get_dist(usr, src) > 7)
			to_chat(usr, span_warning("[src]太远了."))
			return
...
/mob/living/carbon/human/proc/fireman_carry(mob/living/carbon/target)
	if(!can_be_firemanned(target) || incapacitated(restrained_flags = RESTRAINED_NECKGRAB))
		to_chat(src, span_warning("[target]站着的时候你无法用消防员扛法扛起他们!"))
		return
	visible_message(span_notice("[src]开始把[target]抬到[p_their()]背上..."),
	span_notice("你开始把[target]抬到你背上..."))
	...
	if(!do_after(src, delay, NONE, target, target_display = BUSY_ICON_HOSTILE))
		visible_message(span_warning("[src]没能用消防员扛法扛起[target]!"))
		return
...
/mob/living/carbon/human/verb/check_pulse()
	...
	if(!self)
		usr.visible_message(span_notice("[usr]跪下,把[usr.p_their()]的手放在[src]的手腕上,开始数他们的脉搏."),
	...
	to_chat(usr, "你必须[self ? "" : " both"]保持不动直到计数完成.")

	if(!do_after(usr, 6 SECONDS, NONE, src))
		to_chat(usr, span_warning("你未能检查到脉搏.再试一次."))
		return

	to_chat(usr, span_notice("[self ? "Your" : "[src]'s"]的脉搏是[get_pulse(GETPULSE_HAND)]."))
