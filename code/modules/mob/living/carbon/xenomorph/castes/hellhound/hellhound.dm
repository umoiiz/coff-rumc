...
/mob/living/carbon/xenomorph/hellhound/Login()
	. = ..()
	to_chat(src, "<span style='font-weight: bold; color: red;'>注意!! 你正在扮演一只地狱猎犬. 如果你表现糟糕, 你可能会被服务器封禁, 所以听好了!</span>")
	to_chat(src, "<span style='color: red;'>你必须在任何时候听从并服从掠食者的命令. 如果他们要求你死, 你就得死. 对地狱猎犬来说, 不服从他们是不可想象的.</span>")
	to_chat(src, "<span style='color: red;'>你来这里不是为了像兰博一样横冲直撞. 你来这里是为了参与一件罕见的事, 一场掠食者狩猎.</span>")
	to_chat(src, "<span style='color: red;'>掠食者玩家必须遵守严格的角色扮演准则, 你也同样被期望如此.</span>")
	to_chat(src, "<span style='color: red;'>掠食者无法理解你的语言. 他们只能给你下达命令并期望你服从.</span>")
	to_chat(src, "<span style='color: red;'>地狱猎犬会拼命保护它们的主人, 如果主人受到攻击, 它们绝不会离开主人身边.</span>")
	to_chat(src, "<span style='color: red;'>注意, 任何掠食者都可以给你下达命令. 如果命令冲突, 服从最新的命令. 如果他们不喜欢你的表现, 他们可以要求换一个幽灵, 所有人都会嘲笑你. 所以好好干!</span>")
