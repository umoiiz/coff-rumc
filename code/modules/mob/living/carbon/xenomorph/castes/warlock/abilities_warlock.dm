...
/datum/action/ability/activable/xeno/psychic_shield
	name = "Psychic Shield"
	desc = "在你当前位置引导一个灵能护盾,可以反射大多数投射物.护盾激活时再次激活可强行引爆护盾,产生击退效果.使用时必须保持静止."
...
/datum/action/ability/activable/xeno/psychic_shield/use_ability(atom/targetted_atom)
	if(active_shield)
		if(ability_cost > xeno_owner.plasma_stored)
			owner.balloon_alert(owner, "还需要[ability_cost - xeno_owner.plasma_stored]等离子体!")
			return FALSE
...
/datum/action/ability/activable/xeno/psychic_shield/proc/shield_blast(atom/targetted_atom)
	...
	owner.visible_message(span_xenowarning("[owner]释放出一股巨大的灵能冲击!"), span_xenowarning("我们释放出一股巨大的灵能冲击!"))
...
/datum/action/ability/activable/xeno/psy_crush
	name = "Psychic Crush"
	desc = "引导一个不断扩大的范围碾压效果,提前再次激活可碾压一片区域内的敌人.引导时间越长,影响范围越大,但会消耗更多等离子体."
...
/datum/action/ability/activable/xeno/psy_crush/use_ability(atom/target)
	...
	owner.visible_message(span_xenowarning("\The [owner]开始引导他们的灵能力量!"), span_xenowarning("我们开始引导我们的灵能力量!"))
	REMOVE_TRAIT(xeno_owner, TRAIT_IMMOBILE, PSYCHIC_CRUSH_ABILITY_TRAIT)
...
/datum/action/ability/activable/xeno/psy_crush/proc/check_distance(atom/target, sight_needed)
	...
	if(sight_needed && !line_of_sight(owner, target, 11))
		owner.balloon_alert(owner, "超出视线!")
		return FALSE
	return TRUE
...
/datum/action/ability/activable/xeno/psy_crush/proc/crush(turf/target)
	...
	if(crush_cost > xeno_owner.plasma_stored)
		owner.balloon_alert(owner, "还需要[crush_cost - xeno_owner.plasma_stored]等离子体!")
		stop_crush()
		return
...
/datum/action/ability/activable/xeno/psy_crush/on_cooldown_finish()
	owner.balloon_alert(owner, "碾压就绪")
	return ..()
...
/datum/action/ability/activable/xeno/psy_blast
	name = "Psychic Blast"
	desc = "发射一道灵能冲击,造成轻微伤害并击退其范围内的敌人.必须保持静止几秒钟才能使用."
...
/datum/action/ability/activable/xeno/psy_blast/on_cooldown_finish()
	owner.balloon_alert(owner, "灵能冲击就绪")
	return ..()
...
/datum/action/ability/activable/xeno/psy_blast/can_use_ability(atom/A, silent = FALSE, override_flags)
	...
	if(selected_ammo.ability_cost > xeno_owner.plasma_stored)
		if(!silent)
			owner.balloon_alert(owner, "还需要[selected_ammo.ability_cost - xeno_owner.plasma_stored]等离子体!")
		return FALSE

/datum/action/ability/activable/xeno/psy_blast/use_ability(atom/A)
	owner.balloon_alert(owner, "我们引导我们的灵能力量")
