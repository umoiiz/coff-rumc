...
/datum/action/ability/activable/xeno/recycle
	name = "Recycle"
	desc = "我们拆解一名倒下的同类异形的尸体,以免陆战队员收割我们并肩作战的姐妹."
...
/datum/action/ability/activable/xeno/recycle/can_use_ability(atom/target, silent = FALSE, override_flags)
	...
	if(victim.stat != DEAD)
		if(!silent)
			xeno_owner.balloon_alert(xeno_owner, "姐妹还没死")
		return FALSE
...
/datum/action/ability/activable/xeno/recycle/use_ability(atom/target)
	...
	span_danger("我们慢慢拆解 \the [recycled_xeno] 的残骸!"), null, 20)
	...
	xeno_owner.visible_message(span_xenowarning("\The [xeno_owner] 把异形的碎屑从爪子上刷掉."), \
	span_danger("我们把异形的碎屑从爪子上刷掉."), null, 20)
...
/datum/action/ability/xeno_action/build_tunnel/can_use_action(silent = FALSE, override_flags)
	...
	if(locate(/obj/structure/xeno/tunnel) in T)
		if(!silent)
			T.balloon_alert(owner, "这里已经有隧道了")
		return
	if(!T.can_dig_xeno_tunnel())
		if(!silent)
			T.balloon_alert(owner, "无法挖掘,地形不佳")
		return FALSE
...
/datum/action/ability/xeno_action/build_tunnel/action_activate()
	...
	xeno_owner.visible_message(span_xenonotice("\The [xeno_owner] 挖出一个隧道入口."), \
	span_xenonotice("我们挖出一条隧道,并将它连接到我们的网络中."), null, 5)
	...
	to_chat(xeno_owner, span_xenonotice("我们现在拥有 <b>[LAZYLEN(xeno_owner.tunnels)] / [HIVELORD_TUNNEL_SET_LIMIT]</b> 条隧道."))
...
/datum/action/ability/xeno_action/create_jelly/can_use_action(silent = FALSE, override_flags)
	...
	if(owner.l_hand || owner.r_hand)
		if(!silent)
			owner.balloon_alert(owner, "无法制作凝胶,需要空手")
		return FALSE
...
/datum/action/ability/activable/xeno/healing_infusion
	...
	desc = "以精神力量向一名友方异形注入再生能量,大幅提升其自然愈合能力. 如果目标无法自然愈合则无效."
...
/datum/action/ability/activable/xeno/healing_infusion/can_use_ability(atom/target, silent = FALSE, override_flags)
	...
	if(!isxeno(target))
		if(!silent)
			target.balloon_alert(owner, "无法治疗,只能治疗异形")
		return FALSE
	...
	if(HAS_TRAIT(patient, TRAIT_HEALING_INFUSION))
		if(!silent)
			patient.balloon_alert(owner, "无法治疗,已注入")
		return FALSE
...
/datum/action/ability/activable/xeno/healing_infusion/use_ability(atom/target)
	...
	owner.visible_message(span_xenodanger("\the [owner] 向 [target] 注入了神秘能量!"), \
	...
	to_chat(target, span_xenodanger("随着 [owner] 的治疗能量注入我们体内,我们的伤口开始迅速愈合.")) //Let the target know.
