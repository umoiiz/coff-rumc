...
/datum/action/ability/activable/xeno/pounce/runner
	desc = "跃向你的目标,扑倒并解除他们的武装.交替使用可切换野蛮关闭或开启."
...
/datum/action/ability/activable/xeno/pounce/runner/alternate_action_activate()
	...
	owner.balloon_alert(owner, "野蛮[savage_activated ? "activated" : "deactivated"]")
...
/datum/action/ability/activable/xeno/pounce/runner/trigger_pounce_effect(mob/living/living_target)
	...
	if(!COOLDOWN_FINISHED(src, savage_cooldown))
		owner.balloon_alert(owner, "野蛮冷却中([COOLDOWN_TIMELEFT(src, savage_cooldown) * 0.1]秒)")
		return
	...
	if(xeno_owner.plasma_stored < savage_cost)
		owner.balloon_alert(owner, "等离子体不足,无法使用野蛮([savage_cost])")
		return
...
/datum/action/ability/xeno_action/evasion
	name = "Evasion"
	desc = "采取闪避行动,只要保持移动,就能在短时间内使本会命中你的非友方投射物落空.交替使用可切换自动闪避关闭或开启.闪避时你无法闪避贴脸射击."
...
/datum/action/ability/xeno_action/evasion/alternate_action_activate()
	...
	owner.balloon_alert(owner, "自动闪避[auto_evasion ? "activated" : "deactivated"]")
...
/datum/action/ability/xeno_action/evasion/action_activate()
	...
	owner.balloon_alert(owner, "开始闪避:[evasion_duration]秒.")
	to_chat(owner, span_userdanger("我们采取闪避行动,使我们无法被命中."))
	START_PROCESSING(SSprocessing, src)
...
/datum/action/ability/xeno_action/evasion/proc/evasion_flamer_hit(datum/source, atom/movable/projectile/proj)
	...
	if(evasion_stacks)
		owner.balloon_alert(owner, "闪避降低,已受伤")
		to_chat(owner, span_danger("灼热的火焰损害了我们闪避的能力![RUNNER_EVASION_COOLDOWN_REFRESH_THRESHOLD - evasion_stacks > 0 ? " We must dodge [RUNNER_EVASION_COOLDOWN_REFRESH_THRESHOLD - evasion_stacks] more projectile damage before Evasion's cooldown refreshes." : ""]"))
...
/datum/action/ability/xeno_action/evasion/proc/evasion_deactivate()
	...
	xeno_owner.balloon_alert(xeno_owner, "闪避结束")
...
/datum/action/ability/xeno_action/evasion/proc/evasion_dodge_fx(atom/movable/proj)
	xeno_owner.visible_message(span_warning("[xeno_owner]轻松闪避了[proj.name]!"), \
	span_xenodanger("我们轻松闪避了[proj.name]![(RUNNER_EVASION_COOLDOWN_REFRESH_THRESHOLD - evasion_stacks) > 0 && evasion_stacks > 0 ? " We must dodge [RUNNER_EVASION_COOLDOWN_REFRESH_THRESHOLD - evasion_stacks] more projectile damage before [src]'s cooldown refreshes." : ""]"))
...
/datum/action/ability/activable/xeno/snatch/use_ability(atom/A)
	...
	if(isyautja(victim))
		...
		victim.balloon_alert(owner, "夺取失败,我们被抓住了!")
		to_chat(xeno_owner, span_xenodanger("[victim]在我们试图夺取时反击!"))
		to_chat(victim, span_danger("[xeno_owner]试图偷取我们的装备,但失败了!"))
		return FALSE
	...
	if(!stolen_item)
		...
		for(var/slot in slots_to_steal_from)
			...
			if(stolen_item)
				break
	if(!stolen_item)
		victim.balloon_alert(owner, "抢夺失败, 没有物品")
...
/datum/action/ability/activable/xeno/acidic_missile
	...
	action_icon = 'icons/Xeno/actions/runner.dmi'
	desc = "缓慢积聚酸液, 准备将自己作为酸性导弹发射. 如有需要可以提前发射. 最初会减速, 但在最大酸液量5x5时会加速."
	ability_cost = 100
