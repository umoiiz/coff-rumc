...
/datum/action/ability/activable/xeno/pounce/predalien
	name = "Leap"
	desc = "扑向你的目标,将其眩晕并斩击.眩晕持续时间和伤害随每层猎物的叠加而增加."
...
/datum/action/ability/activable/xeno/predalien_roar
	name = "Roar"
	desc = "以增加的斩击伤害和移动速度强化附近的异形,同时移除附近任何猎物的隐身状态.增益强度和持续时间随每层猎物的叠加而增加."
...
/datum/action/ability/activable/xeno/smash
	name = "Smash"
	desc = "眩晕你前方的一个猎物,并麻痹目标周围的任何猎物.麻痹持续时间随每层猎物的叠加而增加."
...
/datum/action/ability/activable/xeno/smash/can_use_ability(atom/target, silent = FALSE, override_flags)
	...
	if(get_dist(target, owner) > 1)
		to_chat(owner, span_xenowarning("[target]距离太远!"))
		return FALSE

	var/mob/living/carbon/carbon = target
	if(carbon.stat == DEAD)
		to_chat(owner, span_xenowarning("[carbon]已经死了,你为什么还想碰它们?"))
		return FALSE
...
/datum/action/ability/activable/xeno/devastate
	name = "Devastate"
	desc = "扯出你猎物的内脏和器官,造成残酷的伤害.伤害随每层猎物的叠加而增加."
...
/datum/action/ability/activable/xeno/devastate/use_ability(atom/target)
	...
	if(do_after(xeno_owner, activation_delay, NONE, carbon, BUSY_ICON_HOSTILE, BUSY_ICON_HOSTILE))
		xeno_owner.visible_message(span_xenouserdanger("[xeno_owner]撕开了[carbon]的内脏!"), span_xenouserdanger("你撕开了[carbon]的内脏!"))
		carbon.spawn_gibs()
	...
	xeno_owner.visible_message(span_xenodanger("[xeno_owner]快速切割[carbon]!"))
