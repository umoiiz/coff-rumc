...
/datum/action/ability/activable/xeno/defile
	name = "Defile"
	desc = "引导注入相邻目标一种加速剂,该加速剂会与异形毒素剧烈反应,释放气体并根据其体内毒素量造成大量毒素伤害."
...
/datum/action/ability/activable/xeno/defile/on_cooldown_finish()
	...
	to_chat(owner, span_xenodanger("你感到你的毒素加速剂腺体重新充盈.你可以再次使用Defile."))
	return ..()
...
/datum/action/ability/activable/xeno/defile/use_ability(atom/A)
	...
	if(living_target.status_flags & GODMODE)
		owner.balloon_alert(owner, "无法Defile")
	...
	to_chat(living_target, span_danger("你感到剧烈疼痛,有什么锋利的东西强行刺穿了你的胸腔."))
	...
	switch(defile_power) //Description varies in severity and probability with the multiplier
		...
		if(50 to 99)
			to_chat(living_target, span_danger("你的内脏在剧痛中!"))
		if(100 to INFINITY)
			to_chat(living_target, span_userdanger("你的内脏感觉像着了火!!"))
...
/datum/action/ability/xeno_action/emit_neurogas
	...
	desc = "引导3秒释放一团有毒烟雾,基于所选试剂,并跟随Defiler.引导期间你必须保持静止;移动将取消技能但仍会消耗等离子."
...
/datum/action/ability/xeno_action/emit_neurogas/on_cooldown_finish()
	...
	to_chat(owner, span_xenodanger("我们感到背部排气孔因加热气体而竖起.我们可以再次释放有毒气体."))
	return ..()
...
/datum/action/ability/xeno_action/emit_neurogas/action_activate()
	...
	xeno_owner.visible_message(span_danger("烟雾开始从[xeno_owner]滚滚涌出!"), \
	span_xenodanger("我们的背部排气孔张开,准备释放有毒烟雾.我们必须保持静止!"))
	...
	if(!do_after(xeno_owner, DEFILER_GAS_CHANNEL_TIME, NONE, null, BUSY_ICON_HOSTILE))
		if(!QDELETED(src))
			to_chat(xeno_owner, span_xenodanger("我们中止了释放烟雾,消耗的等离子白费了."))
	...
	if(xeno_owner.IsStaggered()) //If we got staggered, return
		to_chat(xeno_owner, span_xenowarning("我们试图释放毒素但被硬直了!"))
		return fail_activate()
	...
	xeno_owner.visible_message(span_xenodanger("[xeno_owner]释放出有毒气体!"), \
...
/datum/action/ability/xeno_action/emit_neurogas/proc/dispense_gas(time_left = 3, datum/effect_system/smoke_spread/emitted_gas)
	...
	emitted_gas.start()
	T.visible_message(span_danger("有毒烟雾从庞大的异形身上滚滚涌出!"))
	toggle_particles(FALSE)
...
/datum/action/ability/activable/xeno/inject_egg_neurogas
	name = "Inject Gas"
	desc = "向一颗卵注入毒素,杀死里面的幼虫,但将其充满随时爆炸的气体."
...
/datum/action/ability/activable/xeno/inject_egg_neurogas/on_cooldown_finish()
	playsound(owner.loc, 'sound/effects/alien/newlarva.ogg', 50, 0)
	to_chat(owner, span_xenodanger("我们感到毒刺充满毒素.我们可以再次向卵注入气体."))
	return ..()
...
/datum/action/ability/activable/xeno/inject_egg_neurogas/use_ability(atom/A)
	...
	xeno_owner.visible_message(span_danger("[xeno_owner]开始向卵注入神经毒气,杀死里面的小家伙!"), \
		span_xenodanger("我们将毒刺伸入卵中,将其充满气体,杀死里面的小家伙!"))
	if(!do_after(xeno_owner, 2 SECONDS, NONE, alien_egg, BUSY_ICON_HOSTILE))
		alien_egg.balloon_alert_to_viewers("注入取消")
		xeno_owner.visible_message(span_danger("毒刺从[xeno_owner]收回,留下卵和小家伙存活."), \
			span_xenodanger("我们的毒刺收回,留下卵和小家伙存活."))
		return fail_activate()
...
/datum/action/ability/xeno_action/select_reagent
	name = "Select Reagent"
	desc = "选择用于试剂斩击和有毒气体的试剂.Hemodile减缓目标速度,每种其他异形毒素都会使其效果倍增.Transvitox将灼伤转化为毒素,并在目标受到 brute 伤害时造成额外毒素伤害,两种效果都会被其他异形毒素倍增.Ozelomelyn快速清除目标体内所有药物并造成少量毒素伤害.酸液透明但造成大量灼伤伤害."
...
/datum/action/ability/xeno_action/reagent_slash/on_cooldown_finish()
	to_chat(owner, span_xenodanger("我们能够再次为尖刺注入毒素."))
