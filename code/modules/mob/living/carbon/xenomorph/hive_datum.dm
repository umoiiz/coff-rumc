...
/datum/hive_status/proc/attempt_deevolve(mob/living/carbon/xenomorph/devolver, mob/living/carbon/xenomorph/target)
	...
	if(isnull(reason))
		to_chat(devolver, span_xenonotice("需要退化原因."))
		return
	...
	if(HAS_TRAIT(target, TRAIT_MOVE_VENTCRAWLING))
		to_chat(devolver, span_xenonotice("无法退化,[target]正在爬通风管."))
		return

	if(target.xeno_flags & XENO_AGILITY || target.fortify || target.crest_defense || target.status_flags & INCORPOREAL)
		to_chat(devolver, span_xenonotice("无法退化,当[target]处于此姿态时."))
		return FALSE

	if(!isturf(target.loc))
		to_chat(devolver, span_xenonotice("无法在此处退化[target]."))
		return

	if((target.health < target.maxHealth) || (target.plasma_stored < (target.xeno_caste.plasma_max * target.xeno_caste.plasma_regen_limit)))
		to_chat(devolver, span_xenonotice("无法退化,[target]太虚弱了."))
		return

	target.balloon_alert(target, "强制退化")
	to_chat(target, span_xenowarning("[devolver]因以下原因将我们退化:[reason]."))
...
/datum/hive_status/proc/get_deevolve_caste(mob/living/carbon/xenomorph/devolver, mob/living/carbon/xenomorph/target)
	...
	var/castepick = tgui_input_list(devolver, "选择你想要退化成的种姓.", null, castes_to_pick)
...
/datum/hive_status/proc/target_status_check(mob/living/carbon/xenomorph/user, mob/living/carbon/xenomorph/target)
	if(HAS_TRAIT(target, TRAIT_MOVE_VENTCRAWLING))
		to_chat(user, span_xenonotice("无法惩罚,[target]正在爬通风管."))
		return FALSE
...
/datum/hive_status/proc/can_spawn_as_hugger(mob/dead/observer/user)
	...
	if(GLOB.key_to_time_of_death[user.key] + TIME_BEFORE_TAKING_BODY > world.time && !user.started_as_observer)
		to_chat(user, span_warning("你死亡时间太短,无法成为新的抱脸虫."))
		return FALSE

	TIMER_COOLDOWN_START(src, COOLDOWN_SENTIENT_HUGGER, 5 SECONDS)
	if(tgui_alert(user, "你确定要成为抱脸虫吗?", "成为虫群的一部分!", list("Yes", "No"), 5 SECONDS) != "Yes")
...
/datum/hive_status/proc/attempt_to_spawn_larva(client/xeno_candidate, larva_already_reserved = FALSE)
	...
	if(length(possible_mothers) > 1)
		chosen_mother = tgui_input_list(xeno_candidate.mob, "可用母体", null, possible_mothers)
	else
...
/datum/hive_status/proc/attempt_to_spawn_larva_in_silo(client/xeno_candidate, possible_silos, larva_already_reserved = FALSE)
	...
	if(length(possible_silos) > 1)
		chosen_silo = tgui_input_list(xeno_candidate.mob, "可用卵仓", "生成位置", possible_silos, timeout = 20 SECONDS)
...
/datum/hive_status/proc/do_spawn_larva(client/xeno_candidate, turf/spawn_point, larva_already_reserved = FALSE)
	...
	span_xenodanger("我们从地下钻出,从沉睡中苏醒.为了蜂巢!"))
	...
	to_chat(new_xeno, span_xenoannounce("我们是从沉睡中苏醒的异形幼虫!"))
	if(!larva_already_reserved)
