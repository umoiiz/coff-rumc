...
/datum/action/ability/xeno_action/mutation/pheromones/proc/apply_pheromones(phero_choice)
	if(mutation_aura && mutation_aura.aura_types[1] == phero_choice)
		xeno_owner.balloon_alert(xeno_owner, "停止释放变异信息素")
	...
	xeno_owner.balloon_alert(xeno_owner, "[phero_choice] (变异)")
...
/datum/action/ability/xeno_action/mutation/toxin/action_activate()
	...
	xeno_owner.balloon_alert(xeno_owner, "[toxin_choice] (变异)")
	update_button_icon()
...
/mob/living/carbon/xenomorph/proc/handle_mutation_toxin_attack(mob/living/target)
	...
	if(iscarbon(target))
		...
		to_chat(src, span_xenonotice("我们向[target]注入了[toxin_amount]单位的[initial(selected_toxin.name)]。"))
	else
		to_chat(src, span_xenonotice("我们尝试向[target]注入[initial(selected_toxin.name)]。"))
