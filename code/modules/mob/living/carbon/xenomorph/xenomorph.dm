...
/mob/living/carbon/xenomorph/pull_response(mob/puller)
	...
	H.visible_message(span_warning("[H]试图拉拽[src],却被尾巴扫中了头部!"))
	H.stop_pulling()
	return FALSE
...
/mob/living/carbon/xenomorph/proc/replace_by_ai()
	to_chat(src, span_warning("抱歉,我们的自动技能检测系统认为你的技能水平过低.因此你的身体已被交给一个更有能力的大脑,我们最先进的人工智能技术产物.在你提升之后,请随时取回你的身体!"))
...
/mob/living/carbon/xenomorph/vv_do_topic(list/href_list)
	...
	if(href_list[VV_HK_SET_XENO_SKIN])
		...
		if(length(available_skins) < 2)
			...
			return
		var/answer = tgui_input_list(usr, "选择一种外观设置", "选择一种外观设置", available_skins)
