...
/obj/alien/egg/facehugger/attack_facehugger(mob/living/carbon/xenomorph/facehugger/F, isrightclick = FALSE)
	...
	if(tgui_alert(F, "你想进入这颗蛋吗?", "进入蛋内", list("Yes", "No")) != "Yes")
		return
	...
	if(!insert_new_hugger(new /obj/item/clothing/mask/facehugger/larval()))
		F.balloon_alert(F, span_xenowarning("我们无法使用这颗蛋"))
		return

	F.visible_message(span_xenowarning("[F]滑回[src]."),span_xenonotice("你滑回了[src]。"))
	F.ghostize()
...
/obj/alien/egg/facehugger/attack_alien(mob/living/carbon/xenomorph/xenomorph, damage_amount = xenomorph.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = 0, isrightclick = FALSE)
	...
	if(!issamexenohive(xenomorph))
		...
		xenomorph.visible_message(span_xenowarning("[xenomorph]碾碎了\the [src]。"), span_xenowarning("我们碾碎了\the [src]。"))
		burst(TRUE)
		return

	switch(maturity_stage)
		if(1)
			to_chat(xenomorph, span_xenowarning("这个幼体尚未发育完全。"))
		if(2)
			to_chat(xenomorph, span_xenonotice("我们取回了幼体。"))
			burst(FALSE)
		if(3, 4)
			xenomorph.visible_message(span_xenonotice("\The [xenomorph]清理了已孵化的卵。"), \
			span_xenonotice("我们清理了已孵化的卵。"))
...
/obj/alien/egg/facehugger/proc/insert_new_hugger(obj/item/clothing/mask/facehugger/facehugger, mob/user)
	...
	if(maturity_stage != stage_ready_to_burst + 1)
		if(user)
			to_chat(user, span_xenowarning("这枚卵无法使用。"))
		return FALSE

	if(hugger_type)
		if(user)
			to_chat(user, span_xenowarning("这枚卵里已经有一个幼体了。"))
		return FALSE
	if(user)
		user.visible_message(span_xenowarning("[user]将[facehugger]滑回了[src]。"),span_xenonotice("你将幼体放入了[src]。"))
...
/obj/alien/egg/gas/attack_alien(mob/living/carbon/xenomorph/xenomorph, damage_amount = xenomorph.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = 0, isrightclick = FALSE)
	...
	if(!issamexenohive(xenomorph) || xenomorph.a_intent != INTENT_HELP)
		...
		burst(TRUE)
		return
	to_chat(xenomorph, span_warning("那枚卵充满了气体,没有幼体可取回。"))
