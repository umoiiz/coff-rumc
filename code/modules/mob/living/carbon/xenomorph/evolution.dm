...
/mob/living/carbon/xenomorph/verb/caste_swap()
	...
	if(world.time - (GLOB.key_to_time_of_caste_swap[key] ? GLOB.key_to_time_of_caste_swap[key] : -INFINITY) < (10 MINUTES))
		to_chat(src, span_warning("你的 caste 转换计时器尚未完成。"))
		return
...
/mob/living/carbon/xenomorph/proc/do_evolve(datum/xeno_caste/caste_type, regression = FALSE)
	...
	if(caste_type == /datum/xeno_caste/hivemind && tgui_alert(src, "你即将进化为 hivemind,进化时其核心会放置在你所在的格子上。该核心无法移动,你也无法退化。你确定要在此处放置你的核心吗?", "进化为 hivemind", list("Yes", "No"), FALSE) != "Yes")
		return
	...
	if(!new_mob_type)
		...
		var/castepick = tgui_input_list(src, "我们正在成长为一头美丽的异形!是时候选择一个 caste 了。", null, castes_to_pick)
...
/mob/living/carbon/xenomorph/proc/finish_evolve(new_mob_type, keep_evolution_stored = FALSE)
	...
	new_xeno.visible_message(span_xenodanger("一个[new_xeno.xeno_caste.caste_name]从\the [src]的残骸中破出。"), \
	span_xenodanger("我们从旧躯体的残骸中以更高等的形态破出。为了巢穴!"))
...
/mob/living/carbon/xenomorph/proc/generic_evolution_checks()
	if(HAS_TRAIT(src, TRAIT_BANISHED))
		balloon_alert(src, span_warning("你已被放逐,无法触及 hivemind。"))
		return FALSE
	...
	if(HAS_TRAIT(src, TRAIT_MOVE_VENTCRAWLING))
		balloon_alert(src, "这个地方太过狭窄,无法进化")
		return FALSE
	...
	if(is_banned_from(ckey, ROLE_XENOMORPH))
		...
		balloon_alert(src, "你已被禁止担任异形职位,无法进化。你到底是怎么变成异形的?")
		return FALSE

	if(incapacitated(TRUE))
		balloon_alert(src, "我们无法在当前状态下进化")
		return FALSE

	if(handcuffed)
		balloon_alert(src, "束缚太过限制,无法让我们进化")
		return FALSE
...
/mob/living/carbon/xenomorph/proc/caste_evolution_checks(new_mob_type, regression = FALSE)
	if(!regression && !(new_mob_type in get_evolution_options()))
		balloon_alert(src, "我们无法从当前 caste 进化到那个 caste")
		return FALSE

	if(new_mob_type in SSticker.mode.restricted_castes)
		balloon_alert(src, "我们弱小的巢穴无法支撑那个 caste!")
		return FALSE
	...
	if(CHECK_BITFIELD(new_caste_flags, CASTE_LEADER_TYPE))
		if(is_banned_from(ckey, ROLE_XENO_QUEEN))
			balloon_alert(src, "你已被禁止担任异形领袖职位")
			return FALSE
		var/datum/job/xenojob = SSjob.GetJobType(/datum/job/xenomorph/queen)
		if(xenojob.required_playtime_remaining(client))
			to_chat(src, span_warning("需要[get_exp_format(xenojob.required_playtime_remaining(client))]作为[xenojob.get_exp_req_type()]才能扮演皇后类角色."))
			return FALSE
	...
	if(CHECK_BITFIELD(new_caste_flags, CASTE_CANNOT_EVOLVE_IN_CAPTIVITY) && isxenoresearcharea(get_area(src)))
		to_chat(src, "这个地方有什么东西在隔绝我们与皇后母亲的心灵感应.我们应该趁还来得及赶紧离开!")
		return FALSE
	...
	if(new_caste.death_evolution_delay)
		...
		if(death_timer)
			to_chat(src, span_warning("虫群意识仍在从上一个[initial(new_caste.display_name)]的死亡中恢复.我们必须等待[DisplayTimeText(timeleft(death_timer))]才能进化."))
			return FALSE
	...
	if(maximum_active_caste != INFINITY && maximum_active_caste <= active_caste)
		to_chat(src, span_warning("蜂巢中已经有一个[initial(new_caste.display_name)]了.我们必须等它死亡."))
		return FALSE
	var/turf/T = get_turf(src)
	if(CHECK_BITFIELD(new_caste_flags, CASTE_REQUIRES_FREE_TILE) && T.check_alien_construction(src))
		balloon_alert(src, "我们需要一个空地块来进化")
		return FALSE

	if(!regression)
		if(new_caste.tier == XENO_TIER_TWO && no_room_tier_two)
			balloon_alert(src, "蜂巢无法再支撑一个二级异形,等待更多异形出生或有人死亡")
			return FALSE
		if(new_caste.tier == XENO_TIER_THREE && no_room_tier_three)
			balloon_alert(src, "蜂巢无法再支撑一个三级异形,等待更多异形出生或有人死亡")
			return FALSE
