...
/mob/living/proc/IgniteMob()
	...
	if(fire_stacks > 0 && !on_fire)
		...
		visible_message(span_danger("[src]突然起火!"), isxeno(src) ? span_xenodanger("你突然起火!") : span_userdanger("你突然起火!"))
		update_fire()
...
/mob/living/fire_act(burn_level, flame_color)
	...
	if(hard_armor.getRating(FIRE) >= 100)
		to_chat(src, span_warning("你未被火焰波及."))
		return FALSE
...
/mob/living/proc/resist_fire(datum/source)
	...
	if(istype(T, /turf/open/floor/plating/ground/snow))
		visible_message(span_danger("[src]在雪地里打滚,扑灭了自己身上的火!"), \
		span_notice("你在雪地里扑灭了自己身上的火!"), null, 5)
		ExtinguishMob()
	else
		visible_message(span_danger("[src]在地板上打滚,试图扑灭自己身上的火!"), \
		span_notice("你停下,卧倒并翻滚!"), null, 5)
		if(fire_stacks <= 0)
...
/mob/living/proc/smoke_contact(obj/effect/particle_effect/smoke/S)
	...
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_BLISTERING))
		...
		to_chat(src, span_danger("感觉就像你被扔进了熊熊烈火中!"))
...
/mob/living/proc/apply_radiation(rad_strength = 7, sound_level = null)
	...
	to_chat(src, span_warning("你的身体刺痛,你突然感到力量从体内流失!"))
...
/mob/living/post_crush_act(mob/living/carbon/xenomorph/charger, datum/action/ability/xeno_action/ready_charge/charge_datum)
	...
	switch(charge_datum.charge_type)
		if(CHARGE_CRUSH, CHARGE_BULL)
			...
			charger.visible_message(span_danger("[charger]猛撞[src]!"),
			span_xenodanger("我们猛撞[src]!"))
		...
		if(CHARGE_BULL_HEADBUTT)
			...
			charger.visible_message(span_danger("[charger]撞上[src]并将[p_them()]甩飞!"),
				span_xenowarning("我们撞上[src]并滑行停下!"))
