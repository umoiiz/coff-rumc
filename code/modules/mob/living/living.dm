...
/mob/living/proc/do_resist_grab()
	...
	if(pulledby.grab_state >= GRAB_AGGRESSIVE)
		visible_message(span_danger("[src]抵抗着[pulledby]的抓握!"))
	return resist_grab()
...
/mob/living/resist_grab()
	...
	if(pulledby.grab_state >= GRAB_AGGRESSIVE)
		visible_message(span_danger("[src]已挣脱了[pulledby]的抓握!"), null, null, 5)
	pulledby.stop_pulling()
	grab_resist_level = 0 //zero it out.
	return TRUE
...
/mob/living/Bump(atom/A)
	...
	if(isliving(A))
		...
		if(L.pulledby && L.pulledby != src && L.restrained())
			if(!(world.time % 5))
				to_chat(src, span_warning("[L]被束缚住了, 你无法推过去."))
			return
		...
		if(L.pulling)
			if(ismob(L.pulling))
				...
				if(P.restrained())
					if(!(world.time % 5))
						to_chat(src, span_warning("[L]正在束缚[P], 你无法推过去."))
					return
...
/mob/living/proc/take_over(mob/M, bypass)
	...
	if(!bypass)
		if(client)
			to_chat(M, span_warning("那个生物已经被接管了."))
			GLOB.offered_mob_list -= src
			return FALSE

		if(job && is_banned_from(M.ckey, job.title))
			to_chat(M, span_warning("你被禁止担任那个职业."))
			return FALSE
...
/mob/living/proc/admin_give_speech_impediment(mob/admin)
	...
	var/duration = tgui_input_number(admin, "应该持续多久(以秒为单位)? 最大值为无限时长.", "持续时间", 0, INFINITY, 0 SECONDS)
