...
/mob/living/simple_animal/parrot/attack_alien(mob/living/carbon/xenomorph/xeno_attacker, damage_amount = xeno_attacker.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = MELEE, effects = TRUE, armor_penetration = xeno_attacker.xeno_caste.melee_ap, isrightclick = FALSE)
	...
	if(parrot_state == PARROT_PERCH)
		...
		if(Adjacent(parrot_interest))
			if(!isliving(parrot_interest) && (!parrot_perch || parrot_interest.loc != parrot_perch.loc))
				...
				visible_message("[src]抓住了[held_item]!", span_notice("你抓住了[held_item]!"), span_italics("你听到翅膀猛烈拍打的声音."))
...
/datum/strippable_item/parrot_headset/try_equip(atom/source, obj/item/equipping, mob/user)
	...
	if(!istype(equipping, /obj/item/radio/headset))
		to_chat(user, span_warning("[equipping]放不进去!"))
		return FALSE
...
/datum/strippable_item/parrot_headset/finish_equip(atom/source, obj/item/equipping, mob/user)
	...
	to_chat(user, span_notice("你把[radio]装到了[source]上"))
