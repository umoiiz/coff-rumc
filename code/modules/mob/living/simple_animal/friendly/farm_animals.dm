...
/mob/living/simple_animal/hostile/retaliate/goat/Life(seconds_per_tick, times_fired)
	...
	if(length(enemies) && prob(10))
		enemies = list()
		LoseTarget()
		visible_message(span_notice("[src]冷静下来."))
...
/mob/living/simple_animal/cow/attack_hand(mob/living/user)
	if(!stat && user.a_intent == INTENT_DISARM && icon_state != icon_dead)
		user.visible_message(span_warning("[user]弄翻了[src]."),
			span_notice("你弄翻了[src]."))
		to_chat(src, span_userdanger("你被[user]弄翻了!"))
		Paralyze(20 SECONDS)
...
/obj/item/reagent_containers/food/snacks/egg/process()
	if(isturf(loc))
		amount_grown += rand(1, 2)
		if(amount_grown >= 100)
			visible_message("[src]随着一声轻轻的裂开声孵化了.")
