/mob/living/silicon/ai/verb/ai_network_change()
	...
	var/new_network = tgui_input_list(src, "你想查看哪个网络?", "跳转到网络", available_networks)
	if(!new_network)
		return
...
/mob/living/silicon/ai/verb/display_status()
	...
	var/emote = tgui_input_list(usr, "请选择一个状态!", "AI状态", ai_emotions)
	if(!emote)
		return
...
/mob/living/silicon/ai/verb/change_hologram()
	...
	var/hologram = tgui_alert(src, "你想根据一名船员、一只动物来选择全息影像,还是切换到一个独特的形象?", "全息影像", list("Crew Member", "Unique", "Animal"))
	switch(hologram)
		if("Crew Member")
...
/mob/living/silicon/ai/verb/make_announcement()
	...
	if(last_announcement + 60 SECONDS > world.time)
		to_chat(src, span_warning("你必须等待一段时间才能再次发布公告."))
		return
...
/mob/living/silicon/ai/verb/ai_core_display()
	...
	var/ai_core_icon = tgui_input_list(src, "选择你的AI核心显示图标.", "AI核心显示", iconstates)
...
/mob/living/silicon/ai/verb/shutdown_systems()
	...
	if(tgui_alert(src, "你确定要关闭你的系统吗? 你将无法返回你的躯体. 你无法改变主意,所以请明智地选择!", "确认关闭系统", list("Yes", "No")) != "Yes")
		return
	...
	priority_announce("[src]已被断开. 正在等待加载新的人格...", "AI故障", sound = 'sound/AI/aileft.ogg')
	ghostize(FALSE)
	offer_mob()
