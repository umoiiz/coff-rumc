...
/mob/living/silicon/ai/Topic(href, href_list)
	...
	if(href_list["switchcamera"])
		...
		if(name == string)
			target += src
		if(!length(target))
			to_chat(src, span_warning("目标不在空间站上任何活动摄像头附近."))
			return
...
/mob/living/silicon/ai/proc/toggle_camera_light()
	if(camera_light_on)
		...
		light_cameras()
		to_chat(src, span_notice("摄像头灯光已开启."))
...
/mob/living/silicon/ai/proc/clean_artillery_refs()
	...
	to_chat(src, span_notice("通知: 与所连接的迫击炮的连接已断开."))
...
/datum/action/control_vehicle/action_activate()
	...
	if(new_vehicle.controlled)
		to_chat(ai, span_warning("已经有其他东西在控制这辆载具"))
		return
...
/datum/action/innate/squad_message/can_use_action()
	. = ..()
	if(owner.stat)
		to_chat(owner, span_warning("你当前的状态下无法下达命令."))
		return FALSE
...
/datum/action/innate/squad_message/action_activate()
	...
	if(filter_result)
		to_chat(owner, span_warning("该消息包含了一个在IC聊天中被禁止的词语! 请考虑查阅服务器规则.\n<span replaceRegex='show_filtered_ic_chat'>\"[text]\"</span>"))
...
/mob/living/silicon/ai/proc/ai_ping(atom/A, cooldown = COOLDOWN_AI_PING_NORMAL)
	...
	if(!COOLDOWN_FINISHED(src, last_pinged_marines)) //delay between alerts, both for balance and to prevent chat spam from overeager AIs
		to_chat(src, span_alert("你必须等待一段时间才能再次发布警报"))
		return
	...
	for(var/mob/M in receivers)
		...
		if(istype(A, /obj/effect/xenomorph/acid)) //special check for acid
			...
			to_chat(M, span_alert("AI遥测显示,<b>[pingedacid.acid_t]</b>位于<b>[newdistance]</b>单位距离处,坐标: [AREACOORD_NO_Z(A)],正<b>被[pingedacid.name]融化</b>!"))
			return
		if(newdistance <= AI_PING_RADIUS && newdistance != 0)
			...
			to_chat(M, span_alert("AI遥测显示,<b>[A.name]</b>位于<b>[generaldirection]</b>方向<b>[newdistance]</b>单位距离处."))
