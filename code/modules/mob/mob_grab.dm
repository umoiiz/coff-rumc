...
/obj/item/grab/on_thrown(mob/living/carbon/user, atom/target)
	...
	if(user.grab_state < GRAB_NECK)
		to_chat(user, span_warning("你需要更好的抓握!"))
		return
...
/mob/living/proc/advance_grab_state()
	...
	switch(grab_state)
		if(GRAB_AGGRESSIVE)
			...
			visible_message(span_danger("[src]强行抓住[victim]!"),
				span_danger("你强行抓住[victim]!"),
				span_hear("你听到激烈的扭打声!"), ignored_mob = victim)
			to_chat(victim, span_userdanger("[src]强行抓住你!"))
		...
		if(GRAB_NECK)
			...
			visible_message(span_danger("[src]掐住了[victim]的脖子!"),
				span_danger("你掐住了[victim]的脖子!"),
				span_hear("你听到激烈的扭打声!"), ignored_mob = victim)
			to_chat(victim, span_userdanger("[src]掐住了你的脖子!"))
			victim.drop_all_held_items()
		...
		if(GRAB_KILL)
			...
			visible_message(span_danger("[src]正在勒住[victim]!"),
				span_danger("你正在勒住[victim]!"),
				span_hear("你听到激烈的扭打声!"), ignored_mob = victim)
			to_chat(victim, span_userdanger("[src]正在勒住你!"))
			victim.drop_all_held_items()
			ENABLE_BITFIELD(victim.restrained_flags, RESTRAINED_NECKGRAB)
			if(!victim.buckled && !victim.density)
				victim.Move(loc)
