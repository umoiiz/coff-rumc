...
/mob/new_player/proc/vote_on_poll_handler(datum/poll_question/poll, href_list)
	...
	if(IsAdminAdvancedProcCall())
		...
		to_chat(src, span_danger("处理你的投票时发生了严重错误. 请联系管理员, 他们应该已收到相关消息"))
		return
	var/admin_rank
	if(client.holder)
	...
	if(query_validate_poll_vote.NextRow())
		...
		if(vote_id && !poll.allow_revoting)
			to_chat(usr, span_danger("重复投票已禁用, 且你已对此投票做出回复."))
			qdel(query_validate_poll_vote)
			return
	else
...
/mob/new_player/proc/vote_on_poll_text(href_list, admin_rank, sql_poll_id, vote_id)
	...
	if(!reply_text || (length(reply_text) > 2048))
		to_chat(src, span_danger("你输入的文本为空或过长. 请修正文本后重新提交."))
		return
...
/mob/new_player/proc/vote_on_poll_rating(datum/poll_question/poll, list/href_list, admin_rank, sql_poll_id)
	...
	for(var/h in href_list)
		...
		if (!option)
			to_chat(src, span_warning("无效投票已从你的选票中剔除, 请重新投票 ."))
		sql_votes += list(list(
