...
/mob/verb/memory()
	...
	if(mind)
		...
		to_chat(src, "游戏似乎丢失了你的意识数据, 所以我们无法向你显示你的笔记.")
...
/mob/verb/add_memory(msg as message)
	...
	if(mind)
		...
		to_chat(src, "由于某种原因你没有意识数据, 所以无法向其添加笔记.")
...
/mob/verb/respawn()
	...
	if(DEATHTIME_CHECK(usr))
		if(check_other_rights(usr.client, R_ADMIN, FALSE))
			if(tgui_alert(usr, "你通常没有资格进行此重生. 你确定要用你的管理员权限绕过它吗?", "绕过重生", list("Yes", "No"), 0) != "Yes")
	...
	to_chat(usr, span_notice("你现在可以重生了, 享受你的新生活吧!<br><b>请确保扮演不同的角色, 并请正确进行角色扮演.</b>"))
...
/mob/proc/eord_respawn()
	...
	if(isliving(usr))
		...
		if(liver.health >= liver.get_crit_threshold())
			to_chat(src, span_notice("你只能在死亡或濒死时使用此功能."))
			return
