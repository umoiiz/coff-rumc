/mob/dead/observer/Login()
	...
	if(length(GLOB.offered_mob_list))
		to_chat(src, span_boldnotice("有可用的生物可以接管! 幽灵 > 接管提供的生物"))
...
/mob/dead/observer/proc/revived_while_away()
	...
	to_chat(src, assemble_alert(
		title = "已复活",
		subtitle = "你在断线时被复活了.",
		message = "你在断线时被某人复苏了. [isnull(can_reenter_corpse) ? "You're currently unable to re-enter your body." : "You will re-enter your body in a few seconds."]",
		color_override = "red"
	))
