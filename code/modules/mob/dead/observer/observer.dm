...
/mob/dead/observer/Topic(href, href_list)
	...
	if(href_list["reentercorpse"])
		...
		if(!istype(target))
			to_chat(usr, span_warning("无效的目标."))
			return
		...
		if(LAZYFIND(HS.candidates, ghost.client))
			to_chat(ghost, span_warning("你已经在成为异形的队列中了."))
			return

		switch(tgui_alert(ghost, "你想做什么?", "有可用的潜藏幼虫来源", list("Join as Larva", "Cancel"), 0))
			if("Join as Larva")
...
/mob/dead/observer/verb/reenter_corpse()
	...
	if(isnull(can_reenter_corpse))
		to_chat(src, span_warning("你无法重新进入你的身体."))
		return FALSE
	...
	if(old_mob.key)
		to_chat(src, span_warning("另一个意识在你的身体里...它正在抵抗你."))
		return FALSE
...
/mob/dead/observer/verb/toggle_HUDs()
	...
	var/hud_choice = tgui_input_list(usr, "选择要切换的HUD", "切换HUD", list("Medical HUD", "Security HUD", "Squad HUD", "Xeno Status HUD", "Order HUD"))
...
/mob/dead/observer/verb/teleport()
	...
	var/area/newloc = tgui_input_list(usr, "选择要传送到的区域.", "传送", get_sorted_areas())
	if(!newloc)
		return
...
/mob/dead/observer/verb/offered_mobs()
	...
	if(!length(GLOB.offered_mob_list))
		to_chat(src, span_warning("目前没有提供任何生物."))
		return

	var/mob/living/L = tgui_input_list(usr, "选择你想接管的生物.", "提供的生物", sortNames(GLOB.offered_mob_list))
	if(isnull(L))
		return

	if(!istype(L))
		to_chat(src, span_warning("生物已被接管."))
		return
	...
	switch(tgui_alert(usr, "接管名为 [L.real_name][L.job ? " | Job: [L.job]" : ""] 的生物", "提供的生物", list("Yes", "No", "Follow")))
		if("Yes")
			L.take_over(src)
		if("Follow")
			ManualFollow(L)
...
/mob/dead/observer/verb/toggle_ghostsee()
	...
	to_chat(src, span_notice("你 [(ghost_vision ? "now" : "no longer")] 拥有幽灵视觉."))
...
/mob/dead/observer/verb/observe()
	...
	var/mob/target = tgui_input_list(usr, "请选择一个生物:", "观察", GLOB.mob_living_list)
	if(!target)
		return
...
/mob/dead/observer/verb/dnr()
	...
	if(!isnull(can_reenter_corpse) && tgui_alert(usr, "你确定吗? 你将无法被复活.", "确认", list("Yes", "No")) == "Yes")
	...
	to_chat(usr, span_warning("你已经无法被复活."))
...
/mob/dead/observer/verb/toggle_inquisition()
	...
	if(inquisitive_ghost)
		to_chat(src, span_notice("你现在会检查你点击的一切."))
	else
		to_chat(src, span_notice("你不再检查你点击的东西."))
...
/mob/dead/observer/verb/join_valhalla()
	...
	if(is_banned_from(ckey, ROLE_VALHALLA))
		to_chat(usr, span_notice("你被禁止进入瓦尔哈拉!"))
		return
	...
	if(stat != DEAD)
		to_chat(usr, span_boldnotice("你必须死亡才能使用此功能!"))
		return

	var/choice = tgui_input_list(usr, "你即将前往瓦尔哈拉那阴森可怖的城墙. 这将使你无法被复活. 异形还是陆战队员?", "加入瓦尔哈拉", list("Xenomorph", "Marine"))
	...
	if(choice == "Xenomorph")
		var/mob/living/carbon/xenomorph/xeno_choice = tgui_input_list(usr, "你即将前往瓦尔哈拉那阴森可怖的城墙. 你想要哪种异形?", "加入瓦尔哈拉", GLOB.all_xeno_types)
		if(!xeno_choice)
			return
	...
	var/datum/job/valhalla_job = tgui_input_list(usr, "你即将前往瓦尔哈拉那阴森可怖的城墙. 你想要哪种职业?", "加入瓦尔哈拉", GLOB.jobs_fallen_marine)
	if(!valhalla_job)
		return
...
/mob/dead/observer/point_to(atom/pointed_atom)
	if(!..())
		return FALSE
	visible_message(span_deadsay("[span_name("[src]")] 指向 [pointed_atom]."))
