...
/mob/living/start_pulling(atom/movable/AM, force = move_force, suppress_message = FALSE)
	...
	if(AM.pulledby)
		if(!suppress_message)
			AM.visible_message(span_danger("[src]已将[AM]从[AM.pulledby]的抓握中拉出."),
				span_danger("[src]已将你从[AM.pulledby]的抓握中拉出."), null, null, src)
			to_chat(src, span_notice("你将[AM]从[AM.pulledby]的抓握中拉出!"))
