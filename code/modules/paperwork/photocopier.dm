...
/obj/machinery/photocopier/proc/do_insertion(obj/item/O, mob/user)
	O.forceMove(src)
	to_chat(user, span_notice("你把[O]插入了[src]."))
...
/obj/machinery/photocopier/proc/remove_photocopy(obj/item/O, mob/user)
	...
	to_chat(user, span_notice("你从[src]中取出了[O]."))
...
/obj/machinery/photocopier/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/paper))
		...
		if(toner > 0)
			to_chat(user, span_warning("这个墨盒还没准备好更换!用完剩余的碳粉."))

		if(!user.temporarilyRemoveItemFromInventory(I))
			return
		qdel(I)
		toner = 40
		to_chat(user, span_notice("你把[I]插入了[src]."))
		updateUsrDialog()
