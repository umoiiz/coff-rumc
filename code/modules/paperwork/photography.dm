...
/obj/item/camera/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/camera_film))
		if(pictures_left)
			to_chat(user, span_notice("[src]里面还有一些胶片!"))
			return
...
/obj/item/camera/proc/printpicture(mob/user, datum/picture/picture) //Normal camera proc for creating photos
	...
	if(in_range(src, user)) //needed because of TK
		user.put_in_hands(p)
		pictures_left--
		to_chat(user, span_notice("[pictures_left]剩余照片数."))
		var/customise = "No"
		if(can_customise)
			customise = tgui_alert(user, "你想自定义照片吗?", "自定义", list("Yes", "No"))
		if(customise == "Yes")
...
/obj/item/camera/oldcamera
	name = "旧相机"
	desc = "一台旧的,略有磨损的数码相机,上面用胶带粘着一个廉价照片打印机.颜色是好看的蓝色."
