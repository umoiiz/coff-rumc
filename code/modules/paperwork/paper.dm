...
/obj/item/paper/attack(mob/living/carbon/M, mob/living/carbon/user)
	if(user.zone_selected == "eyes")
		user.visible_message(span_notice("你将纸张展示给[M]."), \
			span_notice("[user]举起一张纸展示给[M]."))
		examine(M)
		return
	...
	if(user.zone_selected == "mouth") // lipstick wiping
		...
		user.visible_message(span_warning("[user]开始用\the [src]擦掉[H]的口红."), \
			span_notice("你开始擦掉[H]的口红."))
		if(!do_after(user, 1 SECONDS, NONE, H, BUSY_ICON_FRIENDLY))
			return
		user.visible_message(span_notice("[user]用\the [src]擦掉了[H]的口红."), \
			span_notice("你擦掉了[H]的口红."))
		H.makeup_style = null
		H.update_body()
...
/obj/item/paper/proc/burnpaper(obj/item/P, mob/user)
	...
	user.visible_message("[class][user]将\the [P]举到\the [src]前,看起来像是[user.p_theyre()]想烧掉它!</span>", \
		"[class]You hold \the [P] up to \the [src], burning it slowly.")

	if(!do_after(user, 2 SECONDS, NONE, src))
		to_chat(user, span_warning("你必须稳稳地握住\the [P]才能烧掉\the [src]."))
		return

	if(!P.heat)
		return
	user.visible_message("[class][user]直接烧穿了\the [src],将其化为灰烬.灰烬在空中飘荡,然后落在地板上堆成一团.</span>", \
	...
	if(istype(I, /obj/item/paper) || istype(I, /obj/item/photo))
		if(istype(I, /obj/item/paper/carbon))
			...
			if(!C.iscopy && !C.copied)
				to_chat(user, span_notice("先取下复写副本."))
				return
		if(loc != user)
			return
		...
		user.dropItemToGround(src)
		to_chat(user, span_notice("你将\the [I]夹到[src]上."))
		...
		to_chat(user, span_notice("你用自己的橡皮图章在纸上盖了章."))
