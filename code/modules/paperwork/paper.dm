...
/obj/item/paper/attack(mob/living/carbon/M, mob/living/carbon/user)
	if(user.zone_selected == "eyes")
		user.visible_message(span_notice("你把纸展示给[M]."), \
...
/obj/item/paper/proc/burnpaper(obj/item/P, mob/user)
	...
	user.visible_message("[class][user]把\the [P]举到\the [src]前,看起来[user.p_theyre()]想烧掉它!</span>", \
		"[class]You hold \the [P] up to \the [src], burning it slowly.")

	if(!do_after(user, 2 SECONDS, NONE, src))
		to_chat(user, span_warning("你必须稳稳地拿着\the [P]才能烧掉\the [src]."))
		return

	if(!P.heat)
		return
	user.visible_message("[class][user]直接烧穿了\the [src],把它化为灰烬.灰烬在空中飘荡,最后落在地上堆成一堆.</span>", \
...
/obj/item/paper/attackby(obj/item/I, mob/user, params)
	...
	if(istype(I, /obj/item/paper) || istype(I, /obj/item/photo))
		...
		user.dropItemToGround(src)
		to_chat(user, span_notice("你把\the [I]夹到[src]上."))
