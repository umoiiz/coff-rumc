...
/obj/item/weapon/gun/proc/check_inactive_hand(mob/user)
	if(user)
		...
		if(in_hand != src && !master_gun) //It has to be held.
			to_chat(user, span_warning("你必须拿着[src]才能这样做!"))
			return
	return TRUE
...
/proc/get_active_firearm(mob/user)
	...
	if(!istype(G))
		to_chat(user, span_warning("你必须手持枪械才能这样做!"))
		return
...
/obj/item/weapon/gun/verb/toggle_auto_eject()
	...
	balloon_alert(usr, "自动卸下[CHECK_BITFIELD(reciever_flags, AMMO_RECIEVER_AUTO_EJECT) ? "enabled" : "disabled"].")
...
/obj/item/weapon/gun/proc/toggle_auto_aim_mode(mob/living/carbon/human/user) //determines whether toggle_aim_mode activates at the end of gun/wield proc
	...
	if(!HAS_TRAIT(src, TRAIT_GUN_AUTO_AIM_MODE))
		to_chat(user, span_notice("你将在持枪时立即瞄准.</b>"))
		ADD_TRAIT(src, TRAIT_GUN_AUTO_AIM_MODE, GUN_TRAIT)
	else
		to_chat(user, span_notice("你将在不精确瞄准的情况下持枪.</b>"))
...
/obj/item/weapon/gun/proc/toggle_aim_mode(mob/living/carbon/human/user)
	...
	if(!(item_flags & WIELDED) && !(deploy_flags & IS_DEPLOYED))
		to_chat(user, span_notice("你需要先持枪才能瞄准."))
		return
	if(!user.wear_id)
		to_chat(user, span_notice("你没有想要避免射击的特定友军.</b>"))
		return
	to_chat(user, span_notice("你稳定你的呼吸..."))
	...
	if(!user.marksman_aura)
		if(!do_after(user, aim_time, (deploy_flags & IS_DEPLOYED) ? NONE : IGNORE_USER_LOC_CHANGE, (deploy_flags & IS_DEPLOYED) ? loc : src, BUSY_ICON_BAR))
			to_chat(user, span_warning("<b>你的注意力被打断了!</b>"))
			return
	if(!(item_flags & WIELDED) && !(deploy_flags & IS_DEPLOYED))
		to_chat(user, span_notice("你需要先持枪才能瞄准."))
		return
	...
	if( (gunattachment) && (/datum/action/item_action/aim_mode in gunattachment.actions_types) )
		...
		gunattachment.modify_fire_delay(aim_fire_delay)
		gunattachment.modify_auto_burst_delay(aim_fire_delay)
	to_chat(user, span_notice("你调整好瞄准线, 允许你射击时越过友军.</b>"))
