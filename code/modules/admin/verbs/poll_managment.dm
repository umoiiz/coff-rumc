...
/datum/admins/proc/poll_parse_href(list/href_list, datum/poll_question/poll)
	...
	if(length(error_state))
		if(poll.edit_ready)
			to_chat(usr, span_danger("并非所有编辑都已应用,因为存在以下错误:\n[error_state.Join("\n")]"))
		else
...
/datum/poll_question/proc/cleaR_POLLS_votes()
	...
	qdel(query_cleaR_POLLS_votes)
	poll_votes = 0
	to_chat(usr, span_danger("投票 [poll_type == POLLTYPE_TEXT ? "responses" : "votes"] 已清除."))
...
/datum/admins/proc/poll_option_parse_href(list/href_list, datum/poll_question/poll, datum/poll_option/option)
	...
	if(length(error_state))
		if(new_option)
			to_chat(usr, span_danger("选项未添加,因为存在以下错误:\n[error_state.Join("\n")]"))
			qdel(option)
		else
			to_chat(usr, span_danger("并非所有编辑都已应用,因为存在以下错误:\n[error_state.Join("\n")]"))
		return
