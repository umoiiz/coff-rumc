...
/datum/admin_help/New(msg_raw, client/C, is_bwoink, tickettier)
	...
	if(is_bwoink)
		...
		if(admin_number_present <= 0)
			to_chat(C, span_notice("没有在线活跃管理员, 你的管理员求助已通过TGS发送给可用管理员. 这可能使用IRC或Discord."))
...
/datum/admin_help/proc/MessageNoRecipient(msg)
	...
	to_chat(initiator,
		type = MESSAGE_TYPE_ADMINPM,
		html = span_adminnotice("私信至-<b>[tier == TICKET_ADMIN ? "Admins" : "Mentors"]</b>: [span_linkify("[msg]")]"))
...
/datum/admin_help/proc/Reopen(irc)
	...
	if(state == AHELP_ACTIVE)
		to_chat(usr, span_warning("此工单已经开启."))
		return
	var/ref
	if(irc)
	...
	if(GLOB.ahelp_tickets.CKey2ActiveTicket(initiator_ckey))
		to_chat(usr, span_warning("该用户已有活跃工单, 无法重新开启此工单."))
		return
	...
	to_chat(initiator, span_adminhelp("你的工单已重新开启."))
...
/datum/admin_help/proc/Tier(irc)
	if(tier_cooldown > world.time)
		to_chat(usr, span_warning("请稍等片刻再更改等级."))
		return
	if(!irc && tier == TICKET_ADMIN && !check_rights(R_ADMINTICKET, FALSE))
		return
	var/ref
	...
	if(tier == TICKET_MENTOR)
		...
		to_chat(initiator, span_adminhelp("你的工单已升级为管理员求助."))
	else if(tier == TICKET_ADMIN)
		tier = TICKET_MENTOR
		msg = "a mentor ticket"
		...
		to_chat(initiator, span_adminhelp("你的工单已升级为导师求助."))
		if(!irc)
			for(var/client/X in GLOB.admins)
				if(!is_mentor(X))
					continue
...
/datum/admin_help/proc/Mark()
	...
	if(marked)
		if(marked == usr.client.key)
			...
			to_chat(initiator, span_adminhelp("你的工单已取消标记."))
			return
		...
		to_chat(initiator, span_adminhelp("你的工单已被另一位管理员标记."))
		return
	marked = usr.client.key
	if(tier == TICKET_MENTOR)
...
/datum/admin_help/proc/Close(silent, irc)
	...
	if(!silent)
		...
		to_chat(initiator, span_adminhelp("你的工单已关闭."))
		if(tier == TICKET_MENTOR)
...
/datum/admin_help/proc/Resolve(silent, irc)
	...
	if(tier == TICKET_MENTOR)
		to_chat(initiator, span_adminhelp("你的导师工单已解决, 如果你还需要询问什么, 请随时再发送一个."))
	else if(tier == TICKET_ADMIN)
		to_chat(initiator, span_adminhelp("你的工单已由管理员解决. 管理员求助动词将很快返还给你."))
...
/datum/admin_help/proc/Reject(irc)
	...
	if(initiator)
		...
		if(tier == TICKET_MENTOR)
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red' size='2'><b>- 导师求助被拒绝! -</b></font>")
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "你的问题可能毫无意义. 请尝试更详细地描述它.")
		else if(tier == TICKET_ADMIN)
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red' size='4'><b>- 管理员求助被拒绝! -</b></font>")
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red'><b>你的管理员求助被拒绝了.</b> 管理员求助动词已返还给你, 以便你可以再试一次.</font>")
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "请在管理员求助中尽量保持冷静, 清晰且描述详细, 不要假设管理员已经看到任何相关事件, 并清楚说明你举报的任何人的名字.")
...
/datum/admin_help/proc/ICIssue(irc)
	...
	if(initiator)
		if(tier == TICKET_MENTOR)
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red' size='4'><b>- 导师求助被标记为IC! -</b></font><br>")
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red'>你很可能询问了工作人员无法透露的重要游戏内信息.</font>")
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red'>请随时再次询问, 但请记住, 对回合至关重要的信息不会被透露.</font>")
		else if(tier == TICKET_ADMIN)
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red' size='4'><b>- 管理员求助被标记为IC! -</b></font><br>")
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red'>无论你的疑问是什么, 你都必须通过IC手段来查明, 工作人员不会透露任何相关内容.</font>")
			to_chat(initiator,
				type = MESSAGE_TYPE_ADMINPM,
				html = "<font color='red'>你的问题已被管理员判定为角色内问题, 目前不需要他们干预, 他们也不能在不以角色外方式介入回合的情况下解决它. 你应该根据规则在游戏中找到问题的解决办法.</font>")
...
/client/verb/adminhelp(msg as message)
	...
	if(prefs.muted & MUTE_ADMINHELP || is_banned_from(ckey, "Adminhelp"))
		to_chat(src, span_warning("错误: 你无法发送管理员求助 (已禁言)."))
		return
	...
	if(current_ticket)
		if(alert(usr, "You already have a ticket open. Is this for the same issue?",,"Yes","No") != "No")
			if(current_ticket)
				...
				to_chat(usr, span_warning("未找到工单, 正在创建新工单..."))
		else
...
/client/verb/mentorhelp(msg as message)
	...
	//handle muting and automuting
	if(prefs.muted & MUTE_ADMINHELP || is_banned_from(ckey, "Adminhelp"))
		to_chat(src, span_warning("错误: 你无法发送导师求助 (已禁言)."))
		return
