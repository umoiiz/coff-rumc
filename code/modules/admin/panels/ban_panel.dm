/client/proc/mute(client/C, mute_type, force = FALSE)
	...
	to_chat(C, span_boldannounce("你已被 [muteunmute] 从 [mute_string]."))
...
/datum/admins/proc/ban_parse_href(list/href_list)
	...
	if(length(error_state))
		to_chat(usr, span_danger("封禁未 [edit_id ? "edited" : "created"] 因为存在以下错误:\n[error_state.Join("\n")]"))
		return
	if(edit_id)
...
/datum/admins/proc/create_ban(player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, severity, reason, list/roles_to_ban)
	...
	if(applies_to_admins)
		...
		if(query_check_adminban_count.NextRow())
			...
			if(adminban_count >= max_adminbans)
				to_chat(usr, span_danger("你已经记录了 [max_adminbans] 次管理员封禁或更多. 请勿滥用此功能!"))
				qdel(query_check_adminban_count)
				return
		qdel(query_check_adminban_count)
	...
	if(C)
		build_ban_cache(C)
		to_chat(C, span_boldannounce("你已被 [usr.client.key] [applies_to_admins ? "admin " : ""]封禁, 来自 [roles_to_ban[1] == "Server" ? "the server" : " Roles: [roles_to_ban.Join(", ")]"].\nReason: [reason]</span><br>[span_danger("This ban is [isnull(duration) ? "permanent." : "temporary, it will be removed in [time_message]."] The round ID is [GLOB.round_id].")]<br><span class='danger'>要申诉此封禁请前往 [appeal_url]"))
		if(GLOB.admin_datums[C.ckey] || GLOB.deadmins[C.ckey])
			is_admin = TRUE
	...
	for(var/client/i in GLOB.clients - C)
		if(i.address == player_ip || i.computer_id == player_cid)
			build_ban_cache(i)
			to_chat(i, span_boldannounce("你已被 [usr.client.key] [applies_to_admins ? "admin " : ""]封禁, 来自 [roles_to_ban[1] == "Server" ? "the server" : " Roles: [roles_to_ban.Join(", ")]"].\nReason: [reason]</span><br>[span_danger("This ban is [isnull(duration) ? "permanent." : "temporary, it will be removed in [time_message]."] The round ID is [GLOB.round_id].")]<br><span class='danger'>要申诉此封禁请前往 [appeal_url]"))
...
/datum/admins/proc/unban(ban_id, player_key, player_ip, player_cid, role, page, admin_key)
	...
	if(C)
		build_ban_cache(C)
		to_chat(C, span_boldannounce("[usr.client.key]已为你的密钥移除了来自[role]的封禁."))
	for(var/client/i in GLOB.clients - C)
		if(i.address == player_ip || i.computer_id == player_cid)
			build_ban_cache(i)
			to_chat(i, span_boldannounce("[usr.client.key]已为你的IP或CID移除了来自[role]的封禁."))
...
/datum/admins/proc/edit_ban(ban_id, player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, reason, mirror_edit, old_key, old_ip, old_cid, old_applies, admin_key, page, list/changes)
	...
	if(applies_to_admins && (applies_to_admins != old_applies))
		...
		if(query_check_adminban_count.NextRow())
			...
			if(R_EVERYTHING && !(R_EVERYTHING & rank.can_edit_rights)) //edit rights are a more effective way to check hierarchical rank since many non-headmins have R_PERMISSIONS now
				max_adminbans = MAX_ADMINBANS_PER_HEADMIN
			if(adminban_count >= max_adminbans)
				to_chat(usr, span_danger("你已经记录了[max_adminbans]条或更多管理员封禁. 不要滥用此功能!"))
				qdel(query_check_adminban_count)
				return
		qdel(query_check_adminban_count)
	...
	if(C)
		build_ban_cache(C)
		to_chat(C, span_boldannounce("[usr.client.key]已为你的密钥编辑了封禁的[changes_keys_text]."))
	for(var/client/i in GLOB.clients - C)
		if(i.address == old_ip || i.computer_id == old_cid)
			build_ban_cache(i)
			to_chat(i, span_boldannounce("[usr.client.key]已为你的IP或CID编辑了封禁的[changes_keys_text]."))
