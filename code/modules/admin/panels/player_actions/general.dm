...
/datum/player_action/send_to_lobby/act(client/user, mob/target, list/params)
	if(!target.client)
		to_chat(user, span_warning("[target]似乎没有活跃客户端."))
		return
...
/datum/player_action/cryo/act(client/user, mob/target, list/params)
	...
	if(tgui_alert(user, "冷冻[key_name(living_target)]?", "低温休眠", list("Yes", "No")) != "Yes")
		return
	...
	if(target_client && tgui_alert(user, "他们已连接客户端,你确定吗?", "低温休眠", list("Yes", "No")) != "Yes")
		return

	var/old_name = living_target.real_name

	var/lobby
	if(target_client?.mob?.mind && tgui_alert(user, "你还想将他们送回大厅吗?", "低温休眠", list("Yes", "No")) == "Yes")
		lobby = TRUE
		var/mob/new_player/new_player_mob = new()
...
/datum/player_action/access_playtimes/act(client/user, mob/target, list/params)
	...
	if(!istype(target_client))
		to_chat(user, span_warning("目标已不再有效."))
		return
