...
/datum/admins/proc/edit_rights_topic(list/href_list)
	...
	if(check_rights(R_DBRANKS, FALSE))
		if(!skip)
			if(!SSdbcore.Connect())
				to_chat(usr, span_danger("无法连接到数据库, 更改仅为临时性的."))
				use_db = FALSE
			else
...
/datum/admins/proc/add_admin(admin_ckey, admin_key, use_db)
	...
	if(use_db)
		...
		if(query_admin_in_db.NextRow())
			qdel(query_admin_in_db)
			to_chat(usr, span_danger("[admin_key]已在管理员数据库中列出. 如果他们没有出现在管理员列表中, 请检查管理选项卡."))
			return FALSE
		qdel(query_admin_in_db)
...
/datum/admins/proc/remove_rank(admin_rank)
	...
	for(var/datum/admin_rank/R in GLOB.admin_ranks)
		if(R.name == admin_rank && (!(R.rights & usr.client.holder.rank.can_edit_rights) == R.rights))
			to_chat(usr, span_adminprefix("你没有编辑此等级拥有的所有权限的权限, 不允许删除等级."))
			return
	...
	if(query_admins_with_rank.NextRow())
		qdel(query_admins_with_rank)
		to_chat(usr, span_danger("错误: 在等级仍在使用时尝试删除等级; 告诉程序员, 这不应该发生."))
		return
	qdel(query_admins_with_rank)
