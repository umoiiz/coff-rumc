/datum/admins/proc/stickyban(action, data)
	...
	switch(action)
		...
		if("add")
			...
			message_admins(span_adminnotice("[ADMIN_TPMONTY(usr)] 已粘性封禁 [ckey].\nReason: [ban["message"]]"))
		...
		if("remove_alt")
			...
			if(!ban)
				to_chat(usr, span_adminnotice("错误: 该封禁已消失."))
				return
			...
			message_admins(span_adminnotice("[ADMIN_TPMONTY(usr)]已将[alt]从[ckey]的粘性封禁中解除关联."))
		...
		if("edit")
			...
			ban = get_stickyban_from_ckey(ckey)
			if(!ban)
				to_chat(usr, span_warning("错误: 该封禁已消失."))
				return
			ban["message"] = "[reason]"
			...
			message_admins(span_warning("[ADMIN_TPMONTY(usr)]已将[ckey]的粘性封禁原因从[oldreason]修改为[reason]"))
		...
		if ("unexempt")
			...
			if (!key)
				to_chat(usr, span_warning("错误: [alt]未从[ckey]的粘性封禁中获得豁免!"))
				return
			...
			if (!key)
				to_chat(usr, span_warning("错误: [alt]从[ckey]的粘性封禁中获得的豁免已消失."))
				return
			...
			message_admins(span_warning("[ADMIN_TPMONTY(usr)]已取消[alt]从[ckey]的粘性封禁中获得的豁免."))
		...
		if ("timeout")
			...
			message_admins(span_warning("[ADMIN_TPMONTY(usr)]已将[ckey]的粘性封禁设为超时."))
		...
		if ("untimeout")
			...
			message_admins(span_warning("[ADMIN_TPMONTY(usr)]已取消[ckey]的粘性封禁的超时状态."))
