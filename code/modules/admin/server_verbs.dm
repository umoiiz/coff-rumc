ADMIN_VERB(restart, R_SERVER, "Restart", "Restarts the server after a short pause.", ADMIN_CATEGORY_ROUND)
	...
	to_chat(world, span_danger("正在重启世界!</span> <span class='notice'>发起者: [user.ckey]"))
...
ADMIN_VERB(shutdown_server, R_SERVER, "Shutdown Server", "Shuts the server down.", ADMIN_CATEGORY_SERVER)
	...
	if(!CONFIG_GET(flag/allow_shutdown))
		to_chat(usr, span_danger("服务器运营者尚未启用此功能."))
		return
	...
	if(timeouts[user.ckey] && timeouts[user.ckey] + 2 MINUTES > world.time)
		to_chat(user, span_danger("你的关闭尝试被中止后, 必须等待2分钟才能再次尝试."))
		return
	...
	if(!SSticker)
		if(tgui_alert(usr, "游戏计时器不存在,将跳过常规检查.", "继续关闭服务器?", list("Continue", "Cancel"), 0) != "Continue")
			return
	else
		...
		if(SSticker.current_state == GAME_STATE_PLAYING || SSticker.current_state == GAME_STATE_SETTING_UP)
			if(tgui_alert(usr, "当前回合正在进行中,是否继续关闭?", "继续关闭服务器?", list("Continue", "Cancel"), 0) != "Continue")
				return
			waitforroundend = TRUE

	to_chat(user, span_danger("警告: 需要延迟确认.你将在30秒后被再次要求确认."))
	...
	if(tgui_alert(user, "你确定要关闭服务器吗?只有拥有服务器远程访问权限的人才能重新开启它.", "关闭服务器?", list("Yes!", "Cancel."), 0) != "Yes!")
		...
		shuttingdown = null
		return
	to_chat(world, span_danger("服务器正在关闭 [waitforroundend ? "after this round" : "in 30 seconds!"]</span> <span class='notice'>发起者: [usr.key]"))
	...
	to_chat(world, span_danger("服务器正在关闭[waitforroundend ? " after this round. " : ""].</span> <span class='notice'>发起者: [shuttingdown]"))
...
ADMIN_VERB(toggle_ooc, R_SERVER, "Toggle OOC", "Toggles OOC for non-admins.", ADMIN_CATEGORY_CHAT)
	...
	if(GLOB.ooc_allowed)
		to_chat(world, span_boldnotice("OOC频道已在全局启用!"))
	else
		to_chat(world, span_boldnotice("OOC频道已在全局禁用!"))
...
ADMIN_VERB(toggle_deadchat, R_SERVER, "Toggle Deadchat", "Toggles deadchat for non-admins.", ADMIN_CATEGORY_CHAT)
	...
	if(GLOB.dsay_allowed)
		to_chat(world, span_boldnotice("死亡聊天已在全局启用!"))
	else
		to_chat(world, span_boldnotice("死亡聊天已在全局禁用!"))
...
ADMIN_VERB(toggle_deadooc, R_SERVER, "Toggle Dead OOC", "Toggles OOC for dead non-admins.", ADMIN_CATEGORY_CHAT)
	...
	if(GLOB.dooc_allowed)
		to_chat(world, span_boldnotice("死亡玩家OOC已在全局启用!"))
	else
		to_chat(world, span_boldnotice("死亡玩家OOC已在全局禁用!"))
...
ADMIN_VERB(start, R_SERVER, "Start Round", "Starts the round early.", ADMIN_CATEGORY_ROUND)
	...
	if(SSticker.setup_failed)
		if(tgui_alert(user, "之前的设置失败.你是否想跳过检查再试一次?胜利条件检查也将暂停.", "开始回合", list("Yes", "No"),  0) != "Yes")
			return
...
ADMIN_VERB(toggle_join, R_SERVER, "Toggle Joining", "Players can still log into the server, but marines won't be able to join the game as a new mob.", ADMIN_CATEGORY_SERVER)
	...
	if(GLOB.enter_allowed)
		...
		to_chat(world, span_boldnotice("新玩家不再可以加入游戏."))
...
ADMIN_VERB(toggle_respawn, R_SERVER, "Toggle Respawn", "Allows players to respawn.", ADMIN_CATEGORY_SERVER)
	...
	if(GLOB.respawn_allowed)
		...
		to_chat(world, span_boldnotice("你不再可以重生."))
...
ADMIN_VERB(set_respawn_time, R_SERVER, "Set Respawn Timer", "Sets the global respawn timer.", ADMIN_CATEGORY_SERVER)
	var/time = tgui_input_number(user, "计时器应该设置为多少刻?")
	if(time < 0)
		return
...
ADMIN_VERB(delay_start, R_SERVER, "Delay Round Start", "Delay the start of the round", ADMIN_CATEGORY_ROUND)
	...
	var/newtime = tgui_input_number(usr, "设置新的时间(秒).设置为-1表示无限期延迟.", "设置延迟", round(SSticker.GetTimeLeft()), 9999, -1, 0)
...
ADMIN_VERB(delay_end, R_SERVER, "Delay Round End", "Delay the round end", ADMIN_CATEGORY_ROUND)
	...
	if(SSticker.admin_delay_notice)
		...
		if(!reason)
			return
		if(SSticker.admin_delay_notice)
			to_chat(user, span_warning("与此同时已经有人延迟了回合结束."))
			return
...
ADMIN_VERB(change_ground_map, R_SERVER, "Change Ground Map", "Change Ground Map for the next round.", ADMIN_CATEGORY_SERVER)
	...
	var/chosenmap = tgui_input_list(user, "选择要切换到的地面地图", "切换地面地图", maprotatechoices, timeout = 0)
	if(!chosenmap)
		return

	var/datum/map_config/VM = maprotatechoices[chosenmap]
	if(!SSmapping.changemap(VM, GROUND_MAP))
		to_chat(user, span_warning("切换地面地图失败."))
		return
...
ADMIN_VERB(toggle_cdn, R_SERVER, "Toggle CDN", "Toggle the Content Delivery Network for asset download.", ADMIN_CATEGORY_SERVER)
	...
	if(tgui_alert(usr, "你确定要切换CDN资源传输吗?", "确认", list("Yes", "No"), 0) != "Yes")
		return
