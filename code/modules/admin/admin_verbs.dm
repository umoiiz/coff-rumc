...
ADMIN_VERB_AND_CONTEXT_MENU(rejuvenate, R_ADMIN, "Rejuvenate", ADMIN_VERB_NO_DESCRIPTION, ADMIN_CATEGORY_MAIN, mob/living/L in world)
	if(tgui_alert(user, "你确定要 rejuvenate [key_name(L)] 吗?", "确认", list("Yes", "No")) != "Yes")
		return
...
/datum/admins/proc/browse_server_logs(path = "data/logs/")
	...
	switch(tgui_alert(usr, "查看(游戏内), 打开(使用你系统的文本编辑器), 下载", path, list("View", "Open", "Download"), 0))
		if("View")
...
/datum/admins/proc/browse_folders(root = "data/logs/", max_iterations = 100)
	...
	for(var/i = 0, i < max_iterations, i++)
		...
		if(path != root)
			choices.Insert(1, "/")
		var/choice = tgui_input_list(usr, "选择一个要访问的文件夹:", "服务器日志", sortList(choices), timeout = 0)
		switch(choice)
			if(null)
				return FALSE
			if("/")
...
/datum/admins/proc/browse_files(root = "data/logs/", max_iterations = 20, list/valid_extensions = list("txt", "log", "htm", "html", "json"))
	...
	for(var/i = 0, i < max_iterations, i++)
		...
		if(path != root)
			choices.Insert(1, "/")
		var/choice = tgui_input_list(usr, "选择一个要访问的文件:", "下载", sortList(choices), timeout = 0)
		switch(choice)
			if(null)
				return
			if("/")
...
ADMIN_VERB_ONLY_CONTEXT_MENU(object_say, R_ADMIN, "Osay", atom/movable/target in world)
	...
	message_admins(span_adminnotice("[key_name_admin(user)]在[AREACOORD(target)]做出了[target]。说 \"[message]\""))
...
/client/proc/private_message(whom, msg)
	...
	if(!holder && !current_ticket)
		to_chat(src,
			type = MESSAGE_TYPE_ADMINPM,
			html = span_warning("你无法再回复此工单,如有需要请使用Adminhelp指令另开一个。"))
		if(msg)
			to_chat(src,
				type = MESSAGE_TYPE_ADMINPM,
...
ADMIN_VERB(mcdb, R_BAN, "Open MCDB", "Opens the MCDB in your browser", ADMIN_CATEGORY_MAIN)
	if(!CONFIG_GET(string/dburl))
		to_chat(user, span_warning("数据库 URL 未设置."))
		return
...
ADMIN_VERB_AND_CONTEXT_MENU(show_traitor_panel, R_ADMIN, "Show Objective Panel", "Show a mobs objective panel.", ADMIN_CATEGORY_FUN, mob/target_mob in GLOB.mob_list)
	...
	if(!target_mind)
		to_chat(user, "这个生物没有意识!", confidential = TRUE)
		return
...
ADMIN_VERB(set_xeno_stat_buffs, R_ADMIN, "Set Xeno Buffs", "Allows you to change stats for all xenos. It is a multiplicator buff, so input 100 to put everything back to normal", ADMIN_CATEGORY_MAIN)
	var/multiplicator_buff_wanted = tgui_input_number(user, "输入将乘以异形属性的百分比系数", "100 是正常属性, 200 是生命值, 再生和近战攻击翻倍")
...
ADMIN_VERB(cmd_admin_create_predator_report, R_ADMIN, "Report: Yautja AI", "Create a predator ship AI report", ADMIN_CATEGORY_MAIN)
	var/input = tgui_input_text(user, "这是来自捕食者飞船 AI 的消息. 在发送之前请与在线工作人员确认.", "什么?", timeout = 0)
	if(!input)
		return FALSE
