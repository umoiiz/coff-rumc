...
/client/proc/vv_do_basic(datum/target, href_list)
	...
	if(check_rights(R_ADMIN, FALSE))
		if(href_list[VV_HK_EXPOSE])
			...
			if (!C)
				return
			if(!target)
				to_chat(usr, span_warning("你尝试暴露给[C]的对象已不存在(被置空或已硬删除)"), confidential = TRUE)
				return
			...
			to_chat(C, "[holder.fakekey ? "an Administrator" : "[usr.client.key]"]已授予你查看变量窗口的权限", confidential = TRUE)
	...
	if(href_list[VV_HK_ADDCOMPONENT])
		...
		if(QDELETED(src))
			to_chat(usr, "那个东西已不存在!", confidential = TRUE)
			return
		...
		message_admins(span_notice("[key_name_admin(usr)]已将[result] [datumname]添加到[key_name_admin(target)]."))
	if(href_list[VV_HK_REMOVECOMPONENT] || href_list[VV_HK_MASS_REMOVECOMPONENT])
		...
		names += sort_list(subtypesof(/datum/element), GLOBAL_PROC_REF(cmp_typepaths_asc))
		var/path = tgui_input_list(usr, "选择要移除的组件/元素.此处列出的所有元素可能并不在该数据上.", "移除元素", names)
		if(isnull(path))
			return
		if(!usr || path == "---Components---" || path == "---Elements---")
			return
		if(QDELETED(src))
			to_chat(usr, "那个东西已不存在!")
			return
		...
		message_admins(span_notice("[key_name_admin(usr)]已[mass_remove? "mass" : ""]从[mass_remove? target.type : key_name_admin(target)]移除了[path]组件."))
