...
/client/proc/vv_parse_text(O, new_var)
	if(O && findtext(new_var,"\["))
		var/process_vars = tgui_alert(usr,"字符串中检测到\[\], 是否作为变量处理?","处理变量?",list("Yes","No"))
		if(process_vars == "Yes")
...
/client/proc/mod_list(list/L, atom/O, original_name, objectvar, index, autodetect_class = FALSE)
	...
	if(L.len > 1000)
		var/confirm = tgui_alert(usr, "你尝试编辑的列表非常长, 继续可能会导致服务器崩溃.", "警告", list("Continue", "Abort"))
		if(confirm != "Continue")
			return
	...
	if (index == null)
		return
	var/assoc = 0
	var/prompt = tgui_alert(usr, "你想编辑键还是其分配的值?", "关联列表", list("Key", "Assigned Value", "Cancel"))
	if (prompt == "Cancel")
		return
...
/client/proc/modify_variables(atom/O, param_var_name = null, autodetect_class = 0)
	...
	if(param_var_name)
		if(!(param_var_name in O.vars))
			to_chat(src, "此datum([O])中不存在名为[param_var_name]的变量", confidential = TRUE)
			return
