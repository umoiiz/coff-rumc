...
//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
	...
	if(href_list["rename"])
		...
		if(!istype(M))
			to_chat(usr, "这只能用于/mob类型的实例", confidential = TRUE)
			return
		...
		if(is_ic_filtered(new_name) || is_soft_ic_filtered(new_name) && tgui_alert(usr, "你选择的名称包含IC聊天过滤器限制的词语.确认这个新名称吗?", "IC聊天过滤器冲突", list("Confirm", "Cancel")) == "Cancel")
			return
		...
		if(!istype(A))
			to_chat(usr, "这只能对/atom类型的实例执行", confidential = TRUE)
			return
