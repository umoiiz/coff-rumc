...
ADMIN_VERB(queen_report, R_FUN, "Queen Mother Report", "Play a Queen mother report to xenos.", ADMIN_CATEGORY_FUN)
	var/customname = tgui_input_text(user, "你希望这份报告的名称是什么?", "报告名称", "Queen Mother Directive", encode = FALSE)
	var/input = tgui_input_text(user, "这应该是一条来自异形种族统治者女王之母的消息.", "女王之母报告", "", multiline = TRUE, encode = FALSE)
	if(!input || !customname)
		return
...
ADMIN_VERB(ai_report, R_FUN, "AI Report", "Create an AI report to players", ADMIN_CATEGORY_FUN)
	var/customname = tgui_input_text(user, "你希望 AI 被称为什么?.", "AI 报告", "AI", encode = FALSE)
	var/input = tgui_input_text(user, "这应该是一条来自飞船 AI 的消息.", "AI 报告", multiline = TRUE, encode = FALSE)
	if(!input || !customname)
		return

	var/paper
	switch(tgui_alert(user, "你想在通信控制台打印一份纸质文件吗?", "AI 报告", list("Yes", "No", "Cancel")))
		if("Yes")
			paper = TRUE
		if("No")
			paper = FALSE
...
ADMIN_VERB(command_report, R_FUN, "Command Report", "Create a custom command report", ADMIN_CATEGORY_FUN)
	var/customname = tgui_input_text(user, "为报告选择一个名称.", "名称", "TGMC Update", encode = FALSE)
	if(!customname)
		return
	var/customsubtitle = tgui_input_text(user, "为报告选择一个副标题.", "副标题", "", encode = FALSE)
	var/input = tgui_input_text(user, "请输入任何你想要的内容. 任何内容. 认真的.", "什么?", "", multiline = TRUE, encode = FALSE)
	if(!input)
		return
	var/override = tgui_input_list(user, "为报告选择一个颜色.", "颜色", faction_alert_colors - "default", default = "blue")

	if(tgui_alert(user, "你想要在通讯控制台打印一份纸质文件吗?", null, list("Yes", "No")) == "Yes")
		print_command_report(input, "[SSmapping.configs[SHIP_MAP].map_name] Update", announce = FALSE)

	switch(tgui_alert(user, "是否应该向全体人员宣布?", "宣布", list("Yes", "No", "Cancel")))
		if("Yes")
			priority_announce(input, customname, customsubtitle, sound = 'sound/AI/commandreport.ogg', color_override = override);
		if("No")
			priority_announce("新的公告已在所有通讯控制台上可用.", "收到一条机密传输", type = ANNOUNCEMENT_PRIORITY, sound = 'sound/AI/commandreport.ogg')
		else
			return
...
ADMIN_VERB_AND_CONTEXT_MENU(narrate_direct, R_FUN, "Direct Narrate", ADMIN_VERB_NO_DESCRIPTION, ADMIN_CATEGORY_MAIN, mob/M in GLOB.mob_list)
	var/msg = tgui_input_text(user, "输入你希望目标看到的文本.", "直接叙述", multiline = TRUE, encode = FALSE)
	if(!msg)
		return
...
ADMIN_VERB(custom_info, R_FUN, "Change Custom Info", "Set a custom info to show to everyone and new joining players", ADMIN_CATEGORY_FUN)
	var/new_info = tgui_input_text(user, "设置玩家在加入时或通过OOC标签页获取的自定义信息.", "自定义信息", GLOB.custom_info, multiline = TRUE, encode = FALSE)
...
/client/verb/custom_info()
	...
	if(!GLOB.custom_info)
		to_chat(src, span_notice("当前没有设置自定义信息."))
		return
...
ADMIN_VERB(sound_file, R_SOUND, "Play Imported Sound", "Play a sound imported from anywhere on your computer.", ADMIN_CATEGORY_FUN, S as sound)
	...
	var/style = tgui_alert(user, "全局还是本地播放声音?", "播放导入的声音", list("Global", "Local"), timeout = 0)
	switch(style)
		if("Global")
...
ADMIN_VERB(sound_web, R_SOUND, "Play Internet Sound", "Play a sound using a link to a website.", ADMIN_CATEGORY_FUN)
	...
	if(!ytdl && !cobalt)
		to_chat(user, span_warning("yt-dlp和cobalt.tools均未配置,操作不可用."))
		return
	...
	if(findtext(web_sound_input, ":") && !findtext(web_sound_input, GLOB.is_http_protocol))
		to_chat(user, span_warning("不允许使用非http(s) URI."))
		to_chat(user, span_warning("对于像ytsearch:这样的yt-dlp快捷方式,请使用来自网站的相应完整url."))
		return
	...
	if(title)
		...
		switch(tgui_alert(user, "向玩家显示这首歌的标题和链接吗?\n[title]", "播放网络声音", list("Yes", "No", "Cancel")))
			if("Yes")
	...
	if(web_sound_url && !findtext(web_sound_url, GLOB.is_http_protocol))
		to_chat(user, span_warning("已阻止: 内容URL未使用http(s)协议"))
		to_chat(user, span_warning("媒体提供商返回的内容URL未使用HTTP或HTTPS协议"))
		return

	var/list/targets
	var/style = tgui_input_list(user, "你想要全局播放还是向异形/陆战队员播放?", null, list("Globally", "Xenos", "Marines", "Locally"))
	switch(style)
		if("Globally")
	...
	var/to_show_text
	var/anon = tgui_alert(user, "显示是谁播放了这首歌吗?", "标注你自己?", list("No", "Yes", "Cancel"))
	switch(anon)
		if("Yes")
			if(show)
...
ADMIN_VERB(force_distress, R_FUN, "Distress Beacon", "Call a distress beacon manually.", ADMIN_CATEGORY_FUN)
	if(!SSticker?.mode)
		to_chat(user, span_warning("请先等待本回合开始."))

	if(SSticker.mode.waiting_for_candidates)
		to_chat(user, span_warning("请等待当前信标完成最终确认."))
		return
	...
	var/choice = tgui_input_list(user, "你想要呼叫哪种求救信号?", null, list_of_calls)
	if(!choice)
		return
	...
	var/max = tgui_input_number(user, "最大生物数量应该是多少?", "最大生物数量", SSticker.mode.picked_call.mob_max)
	if(!max || max < 1)
		return

	SSticker.mode.picked_call.mob_max = max

	var/min = tgui_input_number(user, "最小生物数量应该是多少?", "最小生物数量", SSticker.mode.picked_call.mob_min)
	if(!min || min < 1)
		min = 0
	...
	if(tgui_alert(user, "你想要向服务器全体人员宣布求救信标吗? 这将会向所有玩家显示求救信标.", "宣布求救信标?", list("Yes", "No")) != "Yes")
		is_announcing = FALSE
...
ADMIN_VERB(drop_bomb, R_FUN, "Drop Bomb", "Cause an explosion of varying strength at your location.", ADMIN_CATEGORY_FUN)
	var/choice = tgui_input_list(user, "你想要产生哪种爆炸?", "投放炸弹", list("CAS: Widow Maker", "CAS: Banshee", "CAS: Keeper", "CAS: Fatty", "CAS: Napalm", "Small Bomb", "Medium Bomb", "Big Bomb", "Custom Bomb"))
	switch(choice)
		if("CAS: Widow Maker")
...
ADMIN_VERB(drop_ob, R_FUN, "Drop OB", "Cause an OB explosion of varying strength at your location", ADMIN_CATEGORY_FUN)
	...
	switch(mode)
		...
		if("Custom HE")
			...
			OBShell.explosion_power = tgui_input_number(user, "墙壁清除爆炸的爆炸威力应该有多大?", "设置清除威力", 1425, EXPLOSION_MAX_POWER, 1)
			if(isnull(OBShell.explosion_power))
				return
			OBShell.explosion_falloff = tgui_input_number(user, "墙壁清除爆炸的衰减应该有多大?", "设置清除衰减", 90, EXPLOSION_MAX_POWER, 1)
			if(isnull(OBShell.explosion_falloff))
				return
			warhead = OBShell
		if("Custom Cluster")
			var/obj/structure/ob_ammo/warhead/cluster/OBShell = new
			OBShell.cluster_amount = tgui_input_number(user, "应该发射多少轮齐射?", "设置集束数量", 25, 100)
			if(isnull(OBShell.cluster_amount))
				return
			OBShell.cluster_power = tgui_input_number(user, "爆炸应该有多强?", "设置爆炸威力", 240, EXPLOSION_MAX_POWER, 1)
			if(isnull(OBShell.cluster_power))
				return
			OBShell.cluster_falloff = tgui_input_number(user, "爆炸的衰减应该有多大?", "设置爆炸衰减", 40, EXPLOSION_MAX_POWER, 1)
			if(isnull(OBShell.cluster_falloff))
				return
			warhead = OBShell
		if("Custom Incendiary")
			var/obj/structure/ob_ammo/warhead/incendiary/OBShell = new
			OBShell.flame_intensity = tgui_input_number(user, "火力应该有多密集?", "设置火力强度", 36)
			if(isnull(OBShell.flame_intensity))
				return
			OBShell.flame_duration = tgui_input_number(user, "火焰应该持续多久?", "设置火焰持续时间", 40)
			...
			if(isnull(OBShell.flame_colour))
				return
			OBShell.smoke_radius = tgui_input_number(user, "烟雾应该扩散多远?", "设置烟雾半径", 17, 255, 1, 0)
			if(isnull(OBShell.smoke_radius))
				return
			OBShell.smoke_duration = tgui_input_number(user, "烟雾应该持续多久?", "设置烟雾持续时间", 20, 360, 1, 0)
			if(isnull(OBShell.smoke_duration))
				return
			warhead = OBShell
		if("Custom Plasmaloss")
			var/obj/structure/ob_ammo/warhead/plasmaloss/OBShell = new
			OBShell.smoke_radius = tgui_input_number(user, "烟雾的半径应该是多少格?", "设置烟雾半径", 25, 255, 1, 0)
			if(isnull(OBShell.smoke_radius))
				return
			OBShell.smoke_duration = tgui_input_number(user, "烟雾应该持续多久?", "设置烟雾持续时间", 30, 360, 1, 0)
			if(isnull(OBShell.smoke_duration))
				return
			warhead = OBShell
		else
...
ADMIN_VERB_ONLY_CONTEXT_MENU(offer, R_ADMIN, "Offer Mob", mob/living/L in GLOB.mob_living_list)
	if(L.client)
		if(tgui_alert(user, "这个生物体内有玩家, 你确定要继续吗?", "提供生物", list("Yes", "No")) != "Yes")
			return
		L.ghostize(FALSE)

	else if(L in GLOB.offered_mob_list)
		switch(tgui_alert(user, "这个生物已被提供, 你想重新公告吗?", "提供生物", list("Yes", "Remove", "Cancel")))
			if("Remove")
	...
	else if(tgui_alert(user, "你确定要提供这个生物吗?", "提供生物", list("Yes", "No")) != "Yes")
		return
...
/datum/admins/proc/create_ifriend(mob/living/friend_owner, seek_confirm = FALSE)
	...
	if(!isobserver(C.mob))
		if(is_mentor(C) && tgui_alert(usr, "没有管理员帮助你将无法回到原来的身体. 你确定吗?", "确认", list("Yes", "No")) != "Yes")
			return
...
ADMIN_VERB(run_weather, R_FUN, "Run Weather", "Triggers a weather on the z-level you choose.", ADMIN_CATEGORY_FUN)
	...
	var/z_level = tgui_input_number(user, "目标Z层级?", "Z轴层级", T?.z)
	if(!isnum(z_level))
		return
...
ADMIN_VERB(adjust_gravity, R_FUN, "Adjust Gravity", "Adjusts gravity/jump components of all mobs.", ADMIN_CATEGORY_FUN)
	var/choice = tgui_input_list(user, "你想把重力设置为多少?", "重力调整", list("Standard gravity", "Low gravity", "John Woo", "Exceeding orbital velocity"))
	switch(choice)
		if("Standard gravity")
		...
		if("John Woo")
			to_chat(GLOB.mob_living_list, span_userdanger("你感觉重力变弱了, 还有想飞起来的冲动."))
			for(var/mob/living/living_mob AS in GLOB.mob_living_list)
...
ADMIN_VERB(ai_squad, R_FUN, "Spawn AI squad", "Spawns a AI squad of your choice", ADMIN_CATEGORY_FUN)
	...
	if(!squad_choice)
		return
	var/quantity = tgui_input_number(user, "你想让小队里有多少个生物?", title = "小队规模", default = 5, max_value = length(GLOB.ai_squad_presets[squad_choice]), min_value = 1, timeout = 0, round_value = TRUE)
	if(!quantity)
		return
...
ADMIN_VERB(load_lazy_template, R_FUN, "Load/Jump Lazy Template", "Loads a lazy template and/or jumps to it.", ADMIN_CATEGORY_FUN)
	...
	var/teleport_to_template = tgui_input_list(user, "加载后跳转到模板?", "去哪里?", list("Yes", "No"))
	if(!choice)
		return

	choice = choices[choice]
	if(!choice)
		to_chat(user, span_warning("没有找到该按键对应的模板, 请报告此问题!"))
		return
	...
	if(teleport_to_template == "Yes")
		...
		to_chat(user, span_notice("模板已加载, 你已被移动到预留区域的左下角."))
