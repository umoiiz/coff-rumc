...
[span_admin("<span class='message'>[ADMIN_FULLMONTY(M)]")]</span><hr></span>"})
		...
		switch(href_list["secrets"])
			...
			if("gethumans")
				...
				to_chat(GLOB.alive_human_list, span_userdanger("[key_name_admin(usr, FALSE)]已将所有人类集体传送."))
			...
			if("getxenos")
				...
				to_chat(GLOB.xeno_mob_list, span_userdanger("[key_name_admin(usr, FALSE)]已将所有异形集体传送."))
			...
			if("getall")
				...
				to_chat(GLOB.mob_living_list, span_userdanger("[key_name_admin(usr, FALSE)]已将所有人集体传送."))
			...
			if("rejuvall")
				...
				to_chat(GLOB.mob_living_list, span_userdanger("[key_name_admin(usr, FALSE)]已使所有人集体恢复活力."))
				for(var/i in GLOB.mob_living_list)
					var/mob/living/L = i
					if(!L.client)
						continue
		...
		if(ismob(M))
			...
			if(tgui_alert(usr, "你确定要踢出[key_name(M)]吗?", "警告", list("Yes", "No"), 0) != "Yes")
				return
			if(!M?.client)
				to_chat(usr, span_warning("错误: [M]已不再拥有客户端!"))
				return
		...
		if(tgui_alert(usr, "你确定要使[L]恢复活力吗?", "恢复活力", list("Yes", "No"), 0) != "Yes")
			return
	...
	if(href_list["editrightsbrowsermanage"])
		...
		var/message = tgui_input_text(owner, "除了弹窗之外, 他们还会收到一条需要回复的消息. 你希望内容是什么?", "消息", timeout = 0)
		if(!message)
	...
	if(href_list["deny"])
		...
		priority_announce("求救信号已被封锁. 发射管正在重新校准.", "求救信号", sound = 'sound/AI/distressbeaconlocked.ogg')
	...
	if(href_list["distress"])
		...
		if(tgui_alert(usr, "你想把[key_name(M)]送到雷霆穹顶吗?", "确认", list("Yes", "No"), 0) != "Yes")
			return
		...
		if(C && tgui_alert(usr, "他们已连接客户端, 你确定吗?", "冷冻睡眠", list("Yes", "No"), 0) != "Yes")
			return
		...
		var/lobby
		if(C?.mob?.mind && tgui_alert(usr, "你还想把他们送回大厅吗?", "冷冻睡眠", list("Yes", "No"), 0) == "Yes")
			lobby = TRUE
			var/mob/new_player/NP = new()
			var/mob/N = C.mob
			NP.name = C.mob.name
		...
		switch(tgui_input_list(usr, "你想把它送到哪里?", "发送生物", list("Area", "Mob", "Key", "Coords"), timeout = 0))
			...
			if("Key")
				...
				target = C.mob.loc
			if("Coords")
				var/X = tgui_input_number(usr, "选择X坐标", "X坐标", 1, 255, 1, 0)
				var/Y = tgui_input_number(usr, "选择Y坐标", "Y坐标", 1, 255, 1, 0)
				var/Z = tgui_input_number(usr, "选择Z坐标", "Z坐标", 1, 10, 1, 0)
	...
	if(href_list["evac_authority"])
		...
		switch(href_list["evac_authority"])
			...
			if("use_dest")
				if(tgui_alert(usr, "你确定要现在摧毁[SSmapping.configs[SHIP_MAP].map_name]吗?", "自毁", list("Yes", "No"), 0) != "Yes")
					return
		...
		if(!paths)
			...
			return
		if(length(paths) > 5)
			to_chat(usr, span_warning("选择更少的对象类型, (最多5个)."))
			return
		...
		switch(where)
			if("inhand")
				if(!iscarbon(usr))
					to_chat(usr, "只有当你是一个碳基生物时才能在手中生成.")
					where = "onfloor"
				target = usr
			...
			if("inmarked")
				if(!marked_datum)
					to_chat(usr, span_warning("你没有标记任何物体. 正在放弃生成."))
					return
				else if(!istype(marked_datum, /atom))
					to_chat(usr, span_warning("你标记的物体不能用作目标. 目标必须是 /atom 类型."))
					return
				else
					target = marked_datum
		...
		switch(href_list["rank"])
			...
			if("skills")
				...
				var/newskillset = tgui_input_list(usr, "选择一个技能组.", "编辑军衔", sortList(skillnames))
				if(!newskillset)
					return
			...
			if("commstitle")
				change = tgui_input_text(usr, "输入一个通讯头衔 - \[补给 (头衔)\]", "编辑军衔")
			...
			if("chattitle")
				...
				change = tgui_input_text(usr, "输入一个聊天头衔 - 头衔 简 多伊 尖叫!", "编辑军衔")
			...
			if("idtitle")
				var/obj/item/card/id/C = locate(href_list["id"]) in GLOB.id_card_list
				change = tgui_input_text(usr, "输入一个ID头衔 - 简 多伊 (头衔)", "编辑军衔")
				if(isnull(change) || !istype(H) || !istype(C))
					return
			...
			if("idname")
				var/obj/item/card/id/C = locate(href_list["id"]) in GLOB.id_card_list
				change = tgui_input_text(usr, "输入一个ID名称 - 简 多伊 (头衔)", "编辑军衔")
				if(isnull(change) || !istype(H) || !istype(C))
					return
			...
			if("squad")
				...
				change = tgui_input_list(usr, "选择该陆战队员的新小队.", "更换小队", SSjob.squads)
				if(!change || !istype(H))
					return
		...
		if(GLOB.admin_approvals[approval_id] != -1)
			to_chat(usr, span_warning("该审批已用 '[GLOB.admin_approvals[approval_id]]' 回复过了"))
			return
		...
		if(!HAS_TRAIT(target, TRAIT_BANISHED))
			to_chat(usr, span_warning("目标已经未被放逐."))
			return
