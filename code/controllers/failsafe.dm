	...
	if(!Master || defcon == 0) //Master is gone/not responding and Failsafe just exited its loop
		...
		while (defcon > 0)
			...
			if (recovery_result == 1) //Exit emergency loop and delete self if it was able to recover MC
				...
				message_admins(span_boldannounce("在尝试重建损坏的MC时, 故障安全机制严重失败. 请手动修复MC或重启服务器. 故障安全机制现在退出."))
				message_admins(span_boldannounce("你可以尝试手动调用这两个过程:."))
				message_admins(span_boldannounce("/proc/recover_all_SS_and_recreate_master: 大多数功能应该仍能运行, 但预计会出现不稳定/运行时错误/损坏的东西."))
				message_admins(span_boldannounce("/proc/delete_all_SS_and_recreate_master: 大多数东西会损坏, 但移动和聊天等基本功能应该仍能工作."))
...
/datum/controller/failsafe/proc/emergency_loop()
	...
	if (. == 1) //We were able to create a new master
		...
		to_chat(GLOB.admins, span_adminnotice("故障安全机制在紧急状态 [defcon_pretty()] 下恢复了MC"))
	else
		log_game("FailSafe: Failsafe in emergency state and was unable to recreate MC while in defcon state [defcon_pretty()].")
		message_admins(span_boldannounce("故障安全机制处于紧急状态且主控制器已宕机, 在defcon等级 [defcon_pretty()] 下尝试重建MC失败."))
