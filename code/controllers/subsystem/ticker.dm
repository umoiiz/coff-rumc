...
/datum/controller/subsystem/ticker/proc/setup()
	...
	if(!mode.pre_setup() && !bypass_checks)
		QDEL_NULL(mode)
		to_chat(world, "<b>[GLOB.master_mode] 的预设置阶段出错.</b> 正在返回游戏前大厅.")
		SSjob.ResetOccupations()
		return FALSE
	...
	if(!mode.setup() && !bypass_checks)
		QDEL_NULL(mode)
		to_chat(world, "<b>[GLOB.master_mode] 的设置阶段出错.</b> 正在返回游戏前大厅.")
		SSjob.ResetOccupations()
		return FALSE
		...
		if(5) //every 5 ticks check if there is a slot available
			...
			if(living_player_count() < hpc)
				if(next_in_line?.client)
					to_chat(next_in_line, span_userdanger("一个位置已开放! 你大约有 20 秒的时间加入. <a href='byond://?src=[REF(next_in_line)];lobby_choice=latejoin;override=1'>\>\>加入游戏\<\<</a>"))
