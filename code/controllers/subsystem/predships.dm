...
/datum/clan_ui/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		if(CLAN_ACTION_CLAN_RENAME)
			...
			to_chat(user, span_notice("将[db_query.item[2]]的名称设为[input]."))
		...
		if(CLAN_ACTION_CLAN_SETCOLOR)
			...
			to_chat(user, span_notice("将[db_query.item[2]]的名称设为[color]."))
		...
		if(CLAN_ACTION_CLAN_SETHONOR)
			...
			to_chat(user, span_notice("将[db_query.item[2]]的荣誉从[db_query.item[4]]改为[input]."))
		...
		if(CLAN_ACTION_PLAYER_PURGE)
			...
			to_chat(user, span_notice("你已清除[db_query.item[1]]的战队资料."))
		...
		if(CLAN_ACTION_PLAYER_MOVECLAN)
			...
			var/input = tgui_input_list(user, "选择要将其加入的战队", "更改玩家的战队", clans)

			if(!input)
				return

			if(input == "Remove from clan" && db_query.item[4])
				to_chat(user, span_notice("已将[db_query.item[1]]从其战队中移除."))
				...
				to_chat(user, span_notice("已将[db_query.item[1]]从元老中移除."))
		...
		if(CLAN_ACTION_PLAYER_MODIFYRANK)
			...
			if(!db_query.item[4])
				to_chat(user, span_warning("该玩家不属于任何战队!"))
				return
			...
			if(user.client.has_clan_permission(CLAN_PERMISSION_ADMIN_MODIFY, warn = FALSE))
				var/input = tgui_input_list(user, "选择要将此用户更改到的军衔.", "选择军衔", ranks)
					...
					switch(chosen_rank.limit_type)
						if(CLAN_LIMIT_NUMBER)
							if(players_in_rank >= chosen_rank.limit)
								to_chat(user, span_danger("该槽位已满! (最多[chosen_rank.limit]个槽位)"))
								return
						if(CLAN_LIMIT_SIZE)
							...
							if(players_in_rank >= available_slots)
								to_chat(user, span_danger("该槽位已满! (每名玩家在战队中最多[chosen_rank.limit]个, 当前为[available_slots])"))
								return
			...
			to_chat(user, span_notice("将[db_query.item[1]]的军衔设为[chosen_rank.name]"))
