...
/datum/controller/subsystem/vote/proc/result(default_result)
	...
	if(restart)
		...
		if(!active_admins)
			...
			to_chat(world, "<span style='boltnotice'>注意:重启投票不会自动重启服务器,因为当前有活跃的管理员在线。</span>")
		...
		switch(vote_type)
			...
			if("groundmap")
				...
				if(!lower_admin && SSmapping.groundmap_voted)
					to_chat(usr, span_warning("下一张地面地图已经选定。"))
					return FALSE
			...
			if("custom")
				...
				forced_popup = tgui_alert(usr, "为所有人弹出窗口?", "弹出窗口?", list("Yes", "No")) == "Yes" ? TRUE : FALSE
			else
				return FALSE
		if(!length(choices))
